...
Usage:
    ...
    6. export OPLUS_USE_PREBUILT_BOOTIMAGE=gki/oki/ogki
       ./kernel_platform/oplus/build/oplus_build.sh -t user -p SM8850 -b //vendor/oplus/kernel/cpu:oplus_bsp_sched_ext
    针对依赖不同版本vmlinux的ko
"
}
...
function build_ddk_ko () {
    ...
    if [ -n "$OPLUS_USE_PREBUILT_BOOTIMAGE" ]; then
        for i in "${!build_flags[@]}"; do
            if [[ "${build_flags[$i]}" == *"OPLUS_USE_PREBUILT_BOOTIMAGE="* ]]; then
                build_flags[$i]="--action_env=OPLUS_USE_PREBUILT_BOOTIMAGE=$OPLUS_USE_PREBUILT_BOOTIMAGE"
            fi
        done
    fi
    temp_file=$(mktemp)
