...
def fetch_directory(url):
    ...
    """
    try:
        response = requests.get(url, auth=get_jfrog_config(), timeout=(3, 10))
...
def generate_and_check_urls(protocols, numbers, suffixes, output_file):
    ...
    with open(output_file, 'w', newline='') as csvfile:
        ...
        phone_local_list = [
            'phone-snapshot-local',
            'phone-snapshot2-local',
            'phone-oplus-snapshot-local'
        ]
        # 生成所有可能的组合
        for protocol in protocols:
            for number in numbers:
                for phone_local in phone_local_list:
                    for suffix in suffixes:
                        url = f"{protocol}://gpw{number}.myoas.com/artifactory/{phone_local}/{suffix}/"
                        # print(f"Checking URL: {url}")

                        try:
                            # 发送请求
                            response = requests.get(url, timeout=(3, 10), auth=get_jfrog_config())
                            if response.status_code != 200:
                                response = requests.get(url, timeout=(3, 10))
                            if response.status_code == 200:
...
def deduplicate_and_save(input_file, output_file):
    ...
    with open(input_file, mode='r', newline='', encoding='utf-8') as csvfile:
        ...
        for row in reader:
            if row:  # 确保行不为空
                parts = row[0].split('/')
                # 8 for project 9 for build_type 11 for release_type
                if len(parts) >= 12:
                    project = parts[8]
                    build_type = parts[9]
                    release_type = parts[11]
                    #print(f"project {project} build_type {build_type} release_type {release_type}")
                    # 提取编译日期
                    match = re.search(r'(\d{14})', parts[-1])
                    if match:
                    ...
                    if build_type == 'Daily':
                        key = (project, release_type)
                        if key not in seen or build_date > seen[key][1]:
                            seen[key] = (row[0], build_date)
