...
def ack_support_higher_equal_version_check(args, version):
    """
    判断当前内核版本是否大于等于指定的版本号

    :param args: 包含 full_kernel_version 的参数对象
    :param version: 目标版本号，格式如 "6.12"
    :return: 如果当前版本 >= 目标版本返回 True，否则返回 False
    """
    src_version = args.full_kernel_version

    # 从源版本字符串中提取主版本和次版本，例如从 "6.12.20" 提取 "6.12"
    src_match = re.match(r'^(\d+)\.(\d+)', src_version)
    if not src_match:
        return False

    src_major = int(src_match.group(1))
    src_minor = int(src_match.group(2))

    # 从目标版本字符串中提取主版本和次版本，例如从 "6.12" 提取 "6.12"
    ...
    # 比较版本号：主版本优先，主版本相同时比较次版本
    if src_major > version_major:
        return True
    elif src_major == version_major and src_minor >= version_minor:
        return True
    else:
        return False
...
def generate_abis_modversions(args, acks, output_dir):
    ...
    for ack in acks:
        ...
        if os.path.exists(ack_vmlinux):
            ...
            if ack_support_higher_equal_version_check(args,"6.12"):
...
def compare_same_line_files(base_file, new_file, output_file):
    ...
    if not os.path.exists(base_file):
        ...
        with open(output_file, "w") as output:
            pass
        print("file {} not exist".format(base_file))
        return
...
def check_current_status(args, file_path, src, dest, exclude_patterns=None, strict_mode=True):
    try:
...
def generate_modversions_from_src_to_dst(input_file, output_file, args):
    if not os.path.exists(input_file):
        print("input file {} no exist.".format(input_file))
        return
    ...
    # determine if the data is valid
    if not columns:
        print("No valid data found in {}.".format(args.out_put))
        return
...
def parse_cmd_args():
    ...
    parser.add_argument('--exit_on_error', type=str, help='exit_on_error', default="true")
    ...
    parser.add_argument('--out_put', type=str, help='output directory', default="out/oplus")
    parser.add_argument('--log_dir', type=str, help='log directory', default='LOGDIR/abi/')
    parser.add_argument('--exclude_file', type=str, nargs='*', help='exclude file patterns',
                        default=['*.ko', 'vmlinux'])
...
def find_ack_path(kernel_version, ack):
    ...
    oki_path_patterns = [
        ...
        'out_krn/target/product/vnd/obj/KERNEL_OBJ/*/common-*/',
...
def find_ko_path(args):
    src_dir_list_qcom = [
        ...
        pathlib.Path("out/msm-kernel-*/dist")
    ]
    src_dir_list_mtk = [
...
def prepare_source(args, src_and_targets, acks, out_path='out/oplus'):
    link_file_list = [
        ('kernel/oplus/config/modules.filter.*', '{}/'.format(out_path)),
        ('kernel/build/kernel/oplus/config/modules.filter.*', '{}/'.format(out_path)),
        ('kernel_platform/oplus/config/modules.filter.*', '{}/'.format(out_path)),
        ('kernel/oplus/config/ko_built_not_ddk_tmp_approval.csv', '{}/oki/ko_built_not_ddk_tmp_approval.csv'.format(out_path)),
        ('kernel_platform/oplus/config/ko_built_not_ddk_tmp_approval.csv', '{}/oki/ko_built_not_ddk_tmp_approval.csv'.format(out_path))
    ]
    src_and_dst_templates = [
        ('{source_path}/vmlinux.symvers', '{}/{{type}}/vmlinux.symvers'.format(out_path)),
        ('{source_path}/vmlinux', '{}/{{type}}/vmlinux'.format(out_path)),
    ]

    src_and_dst_path = [
        ('kernel/oplus/config/abi_symbols_tmp_approval.csv', '{}/{{type}}/abi_symbols_tmp_approval.csv'.format(out_path)),
        ('kernel_platform/oplus/config/abi_symbols_tmp_approval.csv', '{}/{{type}}/abi_symbols_tmp_approval.csv'.format(out_path)),
        ('kernel/build/kernel/oplus/config/abi_symbols_tmp_approval.csv', '{}/{{type}}/abi_symbols_tmp_approval.csv'.format(out_path)),
        ('kernel/oplus/config/src_img_approval.csv', '{}/{{type}}/src_img_approval.csv'.format(out_path)),
        ('kernel_platform/oplus/config/src_img_approval.csv', '{}/{{type}}/src_img_approval.csv'.format(out_path)),
        ('out/target/product/vnd/vendor_ramdisk/lib/modules/modules.load.recovery',
         '{}/{{type}}/modules.load.recovery'.format(out_path)),
        ('out/target/product/vnd/vendor_dlkm/lib/modules/system_dlkm.modules.blocklist',
         '{}/{{type}}/system_dlkm.modules.blocklist'.format(out_path))
    ]
...
def find_all_ko(args, acks):
    for ack in acks:
        ...
        out_put_path = pathlib.Path(args.out_put + '/' + ack)
        ack_path = find_ack_path(args.main_kernel_version, ack)
        ack_vmlinux = str(out_put_path / "vmlinux")
...
def abi_check_exit(args):
    if args.exit_on_error.lower() == "true":
        print("abi check exit  exit_on_error is {}".format(args.exit_on_error))
        sys.exit(1)
    else:
        print("abi check not exit exit_on_error is {}".format(args.exit_on_error))
...
def ddk_ko_check(args, output_dir):
    ...
    ack_log = str(pathlib.Path(args.log_dir) / 'oki')
    ...
    check_current_status(args, ko_built_without_ddk_list, ack_path, ack_log, args.exclude_file, False)
    print("\nddk ko check end...")

def symbols_crc_check(args, acks, output_dir):
    btf_hashs = []
    for ack in acks:
        print("\ngenerate {} ko modversions".format(ack))
        ack_path = str(pathlib.Path(output_dir) / ack)
        ack_log = str(pathlib.Path(args.log_dir) / ack)
        ...
        check_current_status(args, abi_required_and_remove_tmp_approval, ack_path, ack_log, args.exclude_file)
        ...
        check_current_status(args, abi_compare_recovery_ko_need_symbol, ack_path, ack_log, args.exclude_file, False)

        print("get modversions from vmlinux.symvers...")
        generate_modversions_from_src_to_dst(vmlinux_symvers, vmlinux_modversions_all_crc_symbols, args)
        ...
        check_current_status(args, symvers_ko_result, ack_path, ack_log, args.exclude_file)
        btf_hash = common.calculate_file_md5(btf_h)
        btf_hashs.append(btf_hash)
        print("copy log to dest...")
        copy_log_files_to_dest(ack_path, ack_log, args.exclude_file)
...
def main():
    ...
    prepare_source(args, src_and_targets, acks, args.out_put)
    find_all_ko(args, acks)

    numeric_suffixes = find_numeric_modules_filter(args.out_put)
    filter_list = create_files_from_numeric_suffixes(args.out_put, numeric_suffixes)
    copy_files_to_directories(args.out_put, filter_list, os.path.join(args.out_put, 'gki'))
    remove_filter_files(args.out_put, filter_list)

    generate_abis_modversions(args, acks + filter_list , args.out_put)
    if ack_support_higher_equal_version_check(args,"6.12"):
        ddk_ko_check(args, args.out_put)
    symbols_crc_check(args, acks + filter_list, args.out_put)
