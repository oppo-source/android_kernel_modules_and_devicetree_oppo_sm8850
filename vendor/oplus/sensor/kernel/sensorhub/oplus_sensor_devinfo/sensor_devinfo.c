...
static uint32_t g_alsps_reg_addr = 0;
static uint32_t g_alsps_reg_val = 0;
...
static void choose_special_gold_cct_value(int support_panel)
{
	...
	/*get 3000k gold ch*/
	ret = oplus_get_dts_feature(parent_node, "light", "panel_stage", &panel_stage);
	if (ret < 0) {
		panel_stage = PANEL_DEFAULT;
	}
	switch(panel_stage) {
	case PANEL_T0:
		ret = oplus_get_dts_feature(parent_node, als_panel, "gold_cct_3k_t0", gold_cct);
		break;
	case PANEL_EVT:
		ret = oplus_get_dts_feature(parent_node, als_panel, "gold_cct_3k_evt", gold_cct);
		break;
	case PANEL_DVT:
		ret = oplus_get_dts_feature(parent_node, als_panel, "gold_cct_3k_dvt", gold_cct);
		break;
	case PANEL_PVT:
		ret = oplus_get_dts_feature(parent_node, als_panel, "gold_cct_3k_pvt", gold_cct);
		break;
	default:
	...
	if (ret < 0) {
		DEVINFO_LOG("panel stage get gold_cct_3k fail, try get default gold_cct_3k\n");
		ret = oplus_get_dts_feature(parent_node, als_panel, "gold_cct_3k", gold_cct);
	}
	if (ret < 0) {
	...
	switch(panel_stage) {
	case PANEL_T0:
		ret = oplus_get_dts_feature(parent_node, als_panel, "gold_cct_6k_t0", gold_cct);
		break;
	case PANEL_EVT:
		ret = oplus_get_dts_feature(parent_node, als_panel, "gold_cct_6k_evt", gold_cct);
		break;
	case PANEL_DVT:
		ret = oplus_get_dts_feature(parent_node, als_panel, "gold_cct_6k_dvt", gold_cct);
		break;
	case PANEL_PVT:
		ret = oplus_get_dts_feature(parent_node, als_panel, "gold_cct_6k_pvt", gold_cct);
		break;
	default:
	...
	if (ret < 0) {
		DEVINFO_LOG("panel stage get gold_cct_6k fail, try get default gold_cct_6k\n");
		ret = oplus_get_dts_feature(parent_node, als_panel, "gold_cct_6k", gold_cct);
	}
	if (ret < 0) {
...
static void get_front_cct_feature(void)
{
	...
	if (support_panel > 0) {
		...
		switch(panel_stage) {
		case PANEL_T0:
			ret = oplus_get_dts_feature(parent_node, "light", "gold_cct_3k_t0", gold_cct);
			break;
		case PANEL_EVT:
			ret = oplus_get_dts_feature(parent_node, "light", "gold_cct_3k_evt", gold_cct);
			break;
		case PANEL_DVT:
			ret = oplus_get_dts_feature(parent_node, "light", "gold_cct_3k_dvt", gold_cct);
			break;
		case PANEL_PVT:
			ret = oplus_get_dts_feature(parent_node, "light", "gold_cct_3k_pvt", gold_cct);
			break;
		default:
		...
		switch(panel_stage) {
		case PANEL_T0:
			ret = oplus_get_dts_feature(parent_node, "light", "gold_cct_6k_t0", gold_cct);
			break;
		case PANEL_EVT:
			ret = oplus_get_dts_feature(parent_node, "light", "gold_cct_6k_evt", gold_cct);
			break;
		case PANEL_DVT:
			ret = oplus_get_dts_feature(parent_node, "light", "gold_cct_6k_dvt", gold_cct);
			break;
		case PANEL_PVT:
			ret = oplus_get_dts_feature(parent_node, "light", "gold_cct_6k_pvt", gold_cct);
			break;
		default:
...
static int sensor_feature_read_func(struct seq_file *s, void *v)
{
	...
	case CONFIG_ALSPS_REG:
		seq_printf(s, "%d %d", g_alsps_reg_addr, g_alsps_reg_val);
		break;
...
static ssize_t sensor_feature_write(struct file *filp, const char *ubuf, size_t cnt, loff_t *data)
{
	...
	case IS_IN_FACTORY_MODE:
		...
		case CCT_FACTORY_GAIN_NORMAL:
			...
			break;
		case CCT_DARK_COUNT_CALI:
			ret = si->send_factory_mode(SENSOR_TYPE_PWM_RGB, 6, &result);
			break;
		case CCT_DARK_DELAY_CALI:
			ret = si->send_factory_mode(SENSOR_TYPE_PWM_RGB, 7, &result);
			break;
		default:
	...
	case CONFIG_ALSPS_REG:
		...
		DEVINFO_LOG("buf %s, g_alsps_reg_addr = %u g_alsps_reg_val = %u\n", buf, g_alsps_reg_addr, g_alsps_reg_val);
		if (si && si->send_reg_config) {
			ret = si->send_reg_config(SENSOR_TYPE_FRONT_CCT, g_alsps_reg_addr, g_alsps_reg_val);
		}
		if (ret < 0) {
			DEVINFO_LOG("send alsps config fail\n");
		}
		break;
	default:
		DEVINFO_LOG("ivalid node type\n");
...
static struct proc_node sensor_feature_file[] = {
	...
	{"config_alsps_reg", CONFIG_ALSPS_REG},
};
