...
static int oplus_send_config_cmd_to_hub(int sensor_type, uint32_t reg, uint32_t val)
{
    uint32_t config_data[4] = {0};

    config_data[0] = reg;
    config_data[1] = val;
    return oplus_send_comm_to_hub(sensor_type, OPLUS_ACTION_CONFIG_REG, config_data, sizeof(config_data));
}
...
struct sensorhub_interface sensorhub_v2 = {
    ...
    .send_reg_config = oplus_send_config_cmd_to_hub,
    .send_cfg = NULL,
