...
#define CUST_ACTION_CONFIG_LOG_LEVEL 11
static struct sensor_fb_cxt *g_sensor_fb_cxt = NULL;
static log_center_t g_log_center;
...
struct sensor_fb_conf g_fb_conf[] = {
	...
	{ACCEL_SW_RESET_ID, "device_acc_sw_reset", SENSOR_DEBUG_DEVICE_TYPE},
...
{
	struct sensor_fb_cxt *sensor_fb_cxt = g_sensor_fb_cxt;
	sensor_log_module_t *module = &g_log_center.sensors[0];
	int i = 0;
	...
	if (sensor_type < SENSOR_TYPE_INVALID || sensor_type >= SENSOR_TYPE_SENSOR_MAX) {
		pr_err("oplus_log_configure_ap:Invalid sensor sensor_type:%d\n", sensor_type);
		return -1;
	}
	if (log_level >= MAX_LOG_LEVEL || log_level < 0) {
		pr_err("oplus_log_configure_ap:Invalid log level:%d\n", log_level);
		return -1;
	}
	spin_lock(&sensor_fb_cxt->rw_lock_log);
	if (sensor_type == SENSOR_TYPE_INVALID) {
		for (i = SENSOR_TYPE_ACCELEROMETER; i < SENSOR_TYPE_SENSOR_MAX; i++) {
			module = &g_log_center.sensors[i];
			module->log_level = log_level;
		}
	} else {
		module = &g_log_center.sensors[sensor_type];
		module->log_level = log_level;
	}
	spin_unlock(&sensor_fb_cxt->rw_lock_log);
	pr_info("oplus_log_configure_ap:sensor:%d is configured, log level:%s\n", sensor_type, log_level_to_string[log_level]);
	return 0;
}
static int oplus_log_configure_scp(int sensor_type, log_level_t log_level)
{
	int ret = 0;
	...
	if (sensor_type < SENSOR_TYPE_INVALID || sensor_type >= SENSOR_TYPE_SENSOR_MAX) {
		pr_err("oplus_log_configure_scp:Invalid sensor type:%d\n", sensor_type);
		return -1;
	}
	if (log_level >= MAX_LOG_LEVEL || log_level < 0) {
		pr_err("oplus_log_configure_scp:Invalid log level:%d\n", log_level);
		return -1;
	}
	memset(&cust_cmd, 0, sizeof(cust_cmd));
	cust_cmd.command = CUST_ACTION_CONFIG_LOG_LEVEL;
	cust_cmd.rx_len = 0;
	cust_cmd.tx_len = sizeof(cust_cmd.data[0]) * 2;
	cust_cmd.data[0] = sensor_type;
	cust_cmd.data[1] = log_level;
	if(sensor_fb_cxt->client == NULL) {
		pr_err("oplus_log_configure_scp:client is null\n");
		return -1;
	}
	...
	pr_info("oplus_log_configure_scp:sensor:%d is configured, log level:%s\n", sensor_type, log_level_to_string[log_level]);
	return 0;
}
static int oplus_log_init_all(void)
{
	int i = 0;
	struct sensor_fb_cxt *sensor_fb_cxt = g_sensor_fb_cxt;
	sensor_log_module_t *module = &g_log_center.sensors[0];
...
static int oplus_sensor_log_center_init(void)
{
	memset(&g_log_center, 0, sizeof(g_log_center));
	oplus_log_init_all();
	return 0;
}

static ssize_t log_config_show(struct device *dev,
                               ...
                               char *buf)
{
	...
	len += snprintf(buf + len, PAGE_SIZE - len, "kernel Sensor Configurations:\n");
	for (i = SENSOR_TYPE_ACCELEROMETER; i < SENSOR_TYPE_SENSOR_MAX; i++) {
		module = &g_log_center.sensors[i];
		len += snprintf(buf + len, PAGE_SIZE - len, "Sensor %d: Level %s\n", i, log_level_to_string[module->log_level]);
	}
	buf[PAGE_SIZE - 1] = '\0';
	return len;
}
static ssize_t log_config_store(struct device *dev,
                                struct device_attribute *attr,
                                const char *buf, size_t count)
{
	...
	if (sscanf(buf, "%d %d", &sensor_type, &log_level) != 2) {
        pr_err("log_config_store:Invalid input\n");
        return -1;
	}
	pr_info("log_config_store:sensor_type:%d, log_level:%d\n", sensor_type, log_level);
	ret = oplus_log_configure_scp(sensor_type, log_level);
	if (ret < 0) {
		pr_err("log_config_store:failed to configure scp log level\n");
		return -1;
	}
	ret = oplus_log_configure_ap(sensor_type, log_level);
	if (ret < 0) {
		pr_err("log_config_store:failed to configure ap log level\n");
		return -1;
	}
	return count;
}
...
DEVICE_ATTR(log_config, 0644, log_config_show, log_config_store);
...
static void sensor_feedback_remove(struct platform_device *pdev)
