...
const static struct fdt_property* oplus_get_dts_feature(int handle, char* node_name, char* feature_name)
{
	...
	for (index = 1; index <= SOURCE_NUM; index++) {
		snprintf(node, sizeof(node), "%s_%d", node_name, index);
		...
		if (!strncmp(device_name, sensorlist_info[handle].name, strlen(sensorlist_info[handle].name))) {
...
static void is_support_lb_algo(void)
{
	...
	for (index = 1; index <= SOURCE_NUM; index++) {
		snprintf(node, sizeof(node), "%s_%d", "/odm/light", index);
		offset = fdt_path_offset(fdt, node);
		if (offset < 0) {
			DEVINFO_LOG("get %s fail", node);
			return;
...
static void get_acc_gyro_cali_nv_adapt_q_flag(void)
{
	...
	if (1 == is_support) {
		...
		if (!strncmp(sensorlist_info[gyro].name, "bmi160", strlen(sensorlist_info[gyro].name))) {
...
static int read_oplus_custom(void *data)
{
	...
	if (!is_support_new_arch) {
		...
		memmove(data_v1, config_info.Sensor, 256);
	} else {
		...
		memmove(data_v2, config_info.Sensor, 256);
	}
...
int get_sensor_parameter(struct cali_data *data)
{
	...
	if (data) {
		memmove(data, g_cali_data, sizeof(struct cali_data));
	} else {
...
bool is_sensor_available(char *name)
{
	...
	for (handle = accel; handle < maxhandle; ++handle) {
		if (name && (!strncmp(sensorlist_info[handle].name, name, strlen(name)))) {
			find = true;
			break;
		}
	}
...
static void get_accgyro_cali_version(void)
{
	...
	if (accgyro_prop == NULL) {
		...
		snprintf(acc_cali_range, sizeof(acc_cali_range), "%u %u %u", acc_thrd_x, acc_thrd_y, acc_thrd_z);
	}
}
...
static void get_gold_rear_cct(void)
{
	...
	if (gold_prop == NULL) {
		snprintf(gold_rear_cct_factor, sizeof(gold_rear_cct_factor), "1001 1001 1001 1001 1001 1001");
		return;
	} else {
		gold_rear_cct_r = fdt32_to_cpu(*((uint32_t *)gold_prop->data));
		gold_rear_cct_g = fdt32_to_cpu(*((uint32_t *)gold_prop->data + 1));
...
static ssize_t mag_data_int2str(char *buf, size_t buf_size)
{
	ssize_t pos = 0;
	int len = 0;
	int ii = 0;
	...
	for (ii = 0; ii < SOURCE_NUM; ii++) {
		snprintf(node, sizeof(node), "/odm/msensor_%d", ii + 1);
		...
		if (prop) {
			...
			if (MAG_PARA_NUM == len / MAG_PARA_OFFSET) {
				for (jj = 0; jj < MAG_PARA_NUM; jj++) {
					...
					offset = snprintf(buf + pos, buf_size - pos, "%d", symble * value);
					if (offset <= 0) {
...
static ssize_t mag_para_read_proc(struct file *file, char __user *buf, size_t count, loff_t *off)
{
	...
	len = mag_data_int2str(page, sizeof(page));
...
static void choose_special_gold_cct_value(int support_panel)
{
	...
	DEVINFO_LOG("%s\n", als_panel);
	/*get 3000k gold ch*/
	offset = fdt_path_offset(fdt, als_panel);
	front_cct_feature = fdt_get_property(fdt, offset, "gold_cct_3k", &len);
	...
	/*get 6000k gold ch*/
	front_cct_feature = fdt_get_property(fdt, offset, "gold_cct_6k", &len);
	if (front_cct_feature == NULL) {
...
int get_msensor_parameter(int num)
{
	...
	snprintf(msensor, sizeof(msensor), "/odm/msensor_%d", num + 1);
	...
	if (prop == NULL) {
		...
		snprintf(project, sizeof(project), "%u", get_project());
		DEVINFO_LOG("project %s\n", project);

		for (index = 0; index < para_num; index++) {
			snprintf(mag_para, sizeof(mag_para), "/odm/msensor_%d/mag_para_%d", num + 1, index + 1);
	...
	if (!strncmp(libname, "mmc", strlen(libname)) || !strncmp(libname, "mxg", strlen(libname))) { /*Memsic parameter need analyze*/
		...
		for (index = 0; index < 9; index++) {
			...
			snprintf(float_buf, sizeof(float_buf), "%c%d.%d%d%d%d", fdt32_to_cpu(*(data_addr + 2 * index + 1)) ? '-' : ' ',
		...
		for (index = 1; index < para_len / 4; index++) {
			snprintf(para_buf[num], sizeof(para_buf[num]), "%s,%d", temp_buf, fdt32_to_cpu(*(data_addr + index)));
			strncpy(temp_buf, para_buf[num], sizeof(temp_buf));
		}
		snprintf(para_buf[num], sizeof(para_buf[num]), "\"%s\":[%u%s]", libname, fdt32_to_cpu(*data_addr), temp_buf);
	}
	return 0;
}
...
static int sensor_feature_read_func(struct seq_file *s, void *v)
{
	...
	case IS_SUPPROT_HWCALI:
		if (!strncmp(sensorlist_info[ps].name, "tcs3701", strlen(sensorlist_info[ps].name))) {
			seq_printf(s, "%d", 1);
		} else if (!strncmp(sensorlist_info[ps].name, "tmd2755x12", strlen(sensorlist_info[ps].name))) {
			seq_printf(s, "%d", 1);
		} else {
			seq_printf(s, "%d", 0);
		}
