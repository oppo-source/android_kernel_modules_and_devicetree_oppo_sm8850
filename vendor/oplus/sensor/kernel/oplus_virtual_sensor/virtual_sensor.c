...
EXPORT_SYMBOL_GPL(oplus_init_sensor_state);
...
EXPORT_SYMBOL_GPL(virtual_sensor_report_data);
	...
	int handle)
{
	...
	snprintf(cxt->wake_lock_name[index], 64, "virtual_sensor_wakelock-%d", index);
...
EXPORT_SYMBOL_GPL(virtual_sensor_register_control_path);
...
EXPORT_SYMBOL_GPL(virtual_sensor_data_report);
...
EXPORT_SYMBOL_GPL(virtual_sensor_flush_report);
...
EXPORT_SYMBOL_GPL(virtual_sensor_driver_add);

#ifdef _USE_VIRTUAL_SENSOR_AS_MODULE
static void virtual_sensor_work(struct work_struct *dwork)
{
	unsigned int wait_for_sensor = 4000;
	int err = 0;

	msleep_interruptible(wait_for_sensor);
	err = virtual_sensor_probe();
	if (err) {
		pr_err("failed to register virtual_sensor driver\n");
	} else {
		pr_err("virtual_sensor registered\n");
	}
...
static int __init virtual_sensor_init(void)
{
	pr_err("%s\n", __func__);
#ifdef _USE_VIRTUAL_SENSOR_AS_MODULE
	INIT_DELAYED_WORK(&vs_work, virtual_sensor_work);
	schedule_delayed_work(&vs_work, msecs_to_jiffies(100));
#else
