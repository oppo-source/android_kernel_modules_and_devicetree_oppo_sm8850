...
static const char *extend_i2s_prop[I2S_PROP_MAX] = {
	"I2S0", "I2S1", "I2S2", "I2S3", "I2S4", "I2S5", "ETDMIN", "ETDMOUT", "I2SOUT4", "I2SIN4", "I2SOUT0", "I2SIN0"
};
...
/*For tfa98xx default 4rd*/
static struct snd_soc_dai_link_component tfa98xx_dails_4rd[] = {
	{
		.of_node = NULL,
...
static void extend_codec_be_dailink(struct codec_prop_info *codec_info, struct snd_soc_dai_link *dailink, size_t size)
{
	...
	for (i = 0; i < size; i++) {
		if (strncmp(codec_info->codec_vendor, extend_pa_vendor[CODEC_PA_NXP], strlen(extend_pa_vendor[CODEC_PA_NXP])) &&
			strncmp(codec_info->codec_vendor, extend_pa_vendor[CODEC_PA_SIPA], strlen(extend_pa_vendor[CODEC_PA_SIPA]))&&
		...
		if (!strncmp(dailink[i].name, extend_i2s_prop[i2s_id], strlen(extend_i2s_prop[i2s_id]))
			...
			if (codec_info->dev_cnt == 1) {
				...
				pr_info("%s: use %s 4rd dailink replace\n", __func__, codec_info->codec_vendor);
				for (j = 0; j < codec_info->dev_cnt; j++) {
					tfa98xx_dails_4rd[j].name = codec_info->codec_name[j];
					tfa98xx_dails_4rd[j].dai_name = codec_info->codec_dai_name[j];
				}
				pr_info("%s: tfa98xx_dails[0] name:%s, dai_name:%s \n", __func__, tfa98xx_dails_4rd[0].name, tfa98xx_dails_4rd[0].dai_name);
				pr_info("%s: tfa98xx_dails[1] name:%s, dai_name:%s \n", __func__, tfa98xx_dails_4rd[1].name, tfa98xx_dails_4rd[1].dai_name);
				pr_info("%s: tfa98xx_dails[2] name:%s, dai_name:%s \n", __func__, tfa98xx_dails_4rd[2].name, tfa98xx_dails_4rd[2].dai_name);
