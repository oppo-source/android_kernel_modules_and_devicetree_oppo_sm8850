...
int typec_switch_to_fast_charger(int to_fast_charger)
{
	...
	if (switch_priv->vendor != DIO4483 && switch_priv->vendor != DIO4485 && switch_priv->vendor != WAS4783) {
...
static int typec_switch_usbc_analog_setup_switches(struct typec_switch_priv *switch_priv)
{
	...
	if (switch_priv->plug_state) {
		...
		// ZZZ No need?
		if ((jack_status & 0x2) && (switch_priv->vendor != DIO4480 && switch_priv->vendor != DIO4483 && switch_priv->vendor != DIO4485 && switch_priv->vendor != WAS4783)) {
		...
		if (switch_priv->vendor == DIO4480) {
			...
			if (jack_status == 0x01) {
				dev_info(dev, "V1.2 %s: error status,swap MIC_GND\n", __func__);
				typec_switch_write_register(switch_priv->regmap, 0x4e, 0x8f);
				typec_switch_write_register(switch_priv->regmap, 0x4e, 0x5a);
				typec_switch_write_register(switch_priv->regmap, 0x51, 0x90);
				typec_switch_write_register(switch_priv->regmap, 0x50, 0x45);
				...
				typec_switch_write_register(switch_priv->regmap, DIO4485_REG_FUN_EN,0x08);
...
static void typec_switch_shutdown(struct i2c_client *i2c) {
	...
	if (switch_priv->vendor == DIO4480 || switch_priv->vendor == DIO4483 || switch_priv->vendor == DIO4485) {
		typec_switch_write_register(switch_priv->regmap, DIO4485_REG_RESET, 0x01);//reset DIO4485
		return;
	}
