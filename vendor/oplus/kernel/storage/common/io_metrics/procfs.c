...
#define TOP_APP_MONITOR_BUF_LEN 256
static char top_app_monitor_buf[TOP_APP_MONITOR_BUF_LEN] = {""};
...
static int top_app_monitor_show(struct seq_file *seq_filp, void *data)
{
    struct file *file = (struct file *)seq_filp->private;
    seq_printf(seq_filp, "%s\n", top_app_monitor_buf);
...
static ssize_t top_app_monitor_write(struct file *file, const char __user *buf,
                                    size_t len, loff_t *ppos)
{
    len = len < (TOP_APP_MONITOR_BUF_LEN - 1) ? len : (TOP_APP_MONITOR_BUF_LEN - 1);
    if (copy_from_user(top_app_monitor_buf, buf, len)) {
        return -EFAULT;
    }
    top_app_monitor_buf[len] = '\0';
    io_metrics_print("%s(%d) write %s to %s/%s\n",
        current->comm, current->pid, top_app_monitor_buf,
        file->f_path.dentry->d_parent->d_iname,
        file->f_path.dentry->d_iname);
...
DEFINE_IO_METRICS_CONTROL(top_app_monitor);
...
static int create_all_proc_nodes(void)
{
    ...
    CREATE_IO_METRICS_CONTROL_NODE(top_app_monitor);
