...
static int kcompressd_enabled(void)
{
...
int schedule_bio_write(void *mem, struct bio *bio, compress_callback cb)
{
	...
	if(unlikely(!kcompressd_enabled()))
		return -EBUSY;
...
int kcompressd_init(void)
{
	...
	ret = mempool_resize(&fs_bio_set.bio_pool, MEMPOOL_NEW_SIZE);
	if (ret)
		pr_err("mempool_resize fs_bio_set.bio_pool failed, ret=%d\n", ret);
	else
