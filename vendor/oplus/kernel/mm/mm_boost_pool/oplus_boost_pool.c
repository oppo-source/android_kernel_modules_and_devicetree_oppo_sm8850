...
static int all_pool_nr_pages(struct dynamic_boost_pool *pool, bool dump)
{
	...
	if (dump) {
		pr_info("test:%d dmabuf_pool:%dMib erm_lru:%dMib erm_free:%dMib boost_pool:%dMib\n",
			boost_pool_test, M(nr_dmabuf_pool), M(nr_erm_lru), M(nr_erm_free), M(nr_boost_pool));
	}
...
static int dynamic_boost_pool_prefill_kworkthread(void *p)
{
	...
	while (true) {
		...
		if (boost_pool_test) {
			pr_info("reset boost_pool_test\n");
			boost_pool_test = false;
		}
...
static int dynamic_boost_pool_init_shrinker(void)
{
	...
	pool_shrinker = shrinker_alloc(0, "boost_pool_shrinker");
#else
	pool_shrinker = shrinker_alloc(0);
#endif
	if (!pool_shrinker)
		return -ENOMEM;

	pool_shrinker->count_objects = dynamic_boost_pool_shrink_count;
	pool_shrinker->scan_objects = dynamic_boost_pool_shrink_scan;
				...
				size_t count, loff_t *ppos)
{
	...
	if (task != NULL) {
		...
		if (strncmp(task->comm, "lmkd", 4) == 0) {
			boost_pool_test = true;
			pr_info("set boost_pool_test\n");
		}
	}
	rcu_read_unlock();
