...
static int all_pool_nr_pages(struct boost_pool *pool, bool dump)
{
	...
	if (dump) {
		pr_info("test:%d dmabuf_pool: %dMib erm_lru: %dMib erm_free: %dMib boost_pool: %dMib\n",
			boost_pool_test, P2M(nr_dmabuf_pool), P2M(nr_erm_lru), P2M(nr_erm_free), P2M(nr_boost_pool));
	}
	if (boost_pool_test) {
		nr_erm_lru = 0;
		nr_erm_free = 0;
			 ...
			 size_t count, loff_t *ppos)
{
	...
	if (boost_pool_nr_pages(boost_pool) < boost_pool->alloc)
...
int boost_pool_mgr_init(void)
{
	...
	pool_shrinker = shrinker_alloc(0, "boost_pool");
	if (!pool_shrinker) {
