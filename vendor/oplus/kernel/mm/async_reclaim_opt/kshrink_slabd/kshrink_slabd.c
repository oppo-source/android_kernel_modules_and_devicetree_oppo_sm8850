...
static bool file_and_cache_is_low(void)
{
	long nr_file = global_node_page_state(NR_ACTIVE_FILE) + global_node_page_state(NR_INACTIVE_FILE);

	if (nr_file < FILE_CACHE_LOW_LIMIT) {
		return true;
	}
	return false;
}
			...
			struct mem_cgroup *memcg, int priority, bool *bypass)
{
	...
	/* for non-root memcg, the memcg aware shrinker should do sync call */
	if (!mem_cgroup_disabled() && !mem_cgroup_is_root(memcg)) {
		*bypass = false;
		return;
	}

	if (is_shrink_slabd_task(current) || file_and_cache_is_low()) {
		*bypass = false;
		return;
	}
