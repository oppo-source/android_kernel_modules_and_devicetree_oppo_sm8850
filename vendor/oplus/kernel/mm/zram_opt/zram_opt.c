...
/*
 * Check if current task should use dynamic direct swappiness
 * Returns true if task is realtime, has max realtime priority, or is UX task
 */
static bool is_high_priority_task(void)
{
	return test_task_is_rt(current) || current->prio == MAX_RT_PRIO ||
		test_task_ux(current);
}

int tune_dynamic_direct_swappiness(void)
{
	...
	if (g_dynamic_direct_swappiness_threshold &&
			nr_inactive_file_pages >= (g_dynamic_direct_swappiness_threshold << 8) &&
...
#ifdef CONFIG_DYNAMIC_TUNING_SWAPPINESS
		*swappiness = tune_dynamic_direct_swappiness();
#else
...
static int register_zram_opt_vendor_hooks(void)
{
	...
	*/
	...
	ret = register_trace_android_rvh_kswapd_shrink_node(mem_cgroup_flush_kswapd, NULL);
	if (ret != 0) {
		pr_err("register_trace_android_rvh_kswapd_shrink_node failed! ret=%d\n", ret);
		goto out;
	}
	ret = register_trace_android_rvh_perform_reclaim(mem_cgroup_flush_reclaim, NULL);
	if (ret != 0) {
		...
		const char __user *buff, size_t len, loff_t *ppos)
{
	...
	if (check_swappiness(swappiness)) {
		pr_err("error swappiness %d\n", swappiness);
		return -EINVAL;
	}

	if (check_vm_threshold(size)) {
		pr_err("error threshold %d\n", size);
		return -EINVAL;
	}
		...
		char __user *buffer, size_t count, loff_t *off)
{
	...
	len = scnprintf(kbuf, PARA_BUF_LEN, "%d %d\n",
		g_dynamic_direct_swappiness, g_dynamic_direct_swappiness_threshold);
...
static int __init create_dynamic_direct_swappiness_proc(void)
{
	dynamic_direct_swappiness_entry = proc_create("oplus_mem/dynamic_direct_swappiness",
		0666, NULL, &proc_dynamic_direct_swappiness_ops);

	if (dynamic_direct_swappiness_entry) {
		pr_info("Register dynamic_direct_swappiness interface passed.\n");
		return 0;
	}

	pr_err("Register dynamic_direct_swappiness interface failed.\n");
	return -ENOMEM;
}

static void __exit destroy_dynamic_direct_swappiness_proc(void)
{
...
static int attach_lru_gen_caps(void)
{
...
static int attach_mem_cgroup_flush_stats(void)
{
...
static int __init zram_opt_init(void)
{
	...
	ret = attach_lru_gen_caps();
	if (ret != 0) {
		pr_err("failed to attach lru_gen_caps\n");
		return ret;
	}

	ret = attach_mem_cgroup_flush_stats();
	if (ret != 0) {
		pr_err("failed to attach mem_cgroup_flush_stats\n");
		return ret;
	}
...
module_param_named(dynamic_direct_swappiness, g_dynamic_direct_swappiness, int, S_IRUGO | S_IWUSR);
module_param_named(dynamic_direct_swappiness_threshold, g_dynamic_direct_swappiness_threshold, int, S_IRUGO | S_IWUSR);
#endif
#ifdef CONFIG_OPLUS_EXTRA_FREE_KBYTES
