// SPDX-License-Identifier: GPL-2.0-only
/*
 * Copyright (C) 2020-2022 Oplus. All rights reserved.
 */

#define pr_fmt(fmt) "uxmem_opt: " fmt

#include <linux/module.h>
#include <trace/hooks/vmscan.h>
#include <trace/hooks/mm.h>
#include <linux/swap.h>
#include <linux/proc_fs.h>
#include <linux/gfp.h>
#include <linux/types.h>
#include <linux/printk.h>
#include <linux/cgroup.h>
#include <linux/kernel.h>
#include <linux/version.h>
#include <linux/vmstat.h>
#include <linux/oom.h>
#include <linux/poll.h>
#include <linux/sched.h>
#include <linux/spinlock.h>
#include <uapi/linux/sched/types.h>
#include <linux/cpufreq.h>
#include <linux/mm.h>
#include <linux/mm_inline.h>
#include <linux/pagemap.h>
#include <linux/page-flags.h>
#include <linux/pageblock-flags.h>
#include <linux/debugfs.h>
#include <linux/memcontrol.h>
#include <linux/mm_types.h>
#include <linux/kasan.h>
#include <linux/page_owner.h>
#include <linux/page_ref.h>
#include <linux/mmzone.h>
#include <linux/kprobes.h>
#include <linux/delay.h>
#include "../../mm/internal.h"
#if IS_ENABLED(CONFIG_OPLUS_FEATURE_MM_OSVELTE)
#include "../mm_osvelte/mm-config.h"
#include "../mm_osvelte/sys-memstat.h"
#endif /* CONFIG_OPLUS_FEATURE_MM_OSVELTE */

#include <linux/sa_group.h>
#include <linux/sa_common.h>
#include <linux/osi_healthinfo.h>
#include "uxmem_opt.h"

#define UX_PAGE_POOL_NAME "ux_page_pool_fillthread"
#define MAX_UXMEM_POOL_ALLOC_RETRIES (5)

static const unsigned int orders[] = {0, 1};
#define NUM_ORDERS ARRAY_SIZE(orders)
static bool ezr_enabled;

/*
 * for movable type, 64MB order 0, 0MB order 1 by default(very few order 1 movable alloc)
 * for unmovable type, 32MB order 0, 8MB order 1 by default
 */
static unsigned int page_pool_nr_pages[NUM_ORDERS][POOL_MIGRATETYPE_TYPES_SIZE] = {
		{(SZ_32M >> PAGE_SHIFT), (SZ_64M >> PAGE_SHIFT)}, /* order 0 */
		{(SZ_8M >> PAGE_SHIFT), 0},  /* order 1 */
};

static struct page_pool *pools[NUM_ORDERS][POOL_MIGRATETYPE_TYPES_SIZE];

static struct task_struct *ux_page_pool_tsk = NULL;
static wait_queue_head_t kworkthread_waitq;
static unsigned int kworkthread_wait_flag;
static bool ux_page_pool_enabled = false;
static bool fillthread_enabled = false;

static struct per_cpu_pages *dma32_pcp_base = NULL;

static unsigned long ux_pool_alloc_fail = 0;
/* true by default, false when oplus_bsp_uxmem_opt.enable=N in cmdline */
bool enable = true;
module_param(enable, bool, S_IRUGO | S_IWUSR);

static void prep_new_page_dup(struct page *page, unsigned int order, gfp_t gfp_flags,
								unsigned int alloc_flags);
typedef void (*post_alloc_hook_t)(struct page *page, unsigned int order, gfp_t gfp_flags);
static post_alloc_hook_t post_alloc_hook_dup = NULL;
typedef void (*prep_compound_page_t)(struct page *page, unsigned int order);
static prep_compound_page_t prep_compound_page_dup = NULL;

#ifdef UXPAGEPOOL_DEBUG
static atomic_long_t g_alloc_pages_fast[NUM_ORDERS][POOL_MIGRATETYPE_TYPES_SIZE] = {{ATOMIC_LONG_INIT(0)}};
static atomic_long_t g_alloc_pages_fast_retry[NUM_ORDERS][POOL_MIGRATETYPE_TYPES_SIZE][MAX_UXMEM_POOL_ALLOC_RETRIES] = {{{ATOMIC_LONG_INIT(0)}}};
static atomic_long_t g_alloc_pages_slow[NUM_ORDERS][POOL_MIGRATETYPE_TYPES_SIZE] = {{ATOMIC_LONG_INIT(0)}};
static atomic_long_t fillthread_runtime_times[NUM_ORDERS][POOL_MIGRATETYPE_TYPES_SIZE] = {{ATOMIC_LONG_INIT(0)}};

static atomic_long_t g_alloc_fail_cnts[NUM_ORDERS][POOL_MIGRATETYPE_TYPES_SIZE] = {{ATOMIC_LONG_INIT(0)}};
static atomic_long_t g_alloc_pages_cnts[NUM_ORDERS][POOL_MIGRATETYPE_TYPES_SIZE] = {{ATOMIC_LONG_INIT(0)}};
#define PARA_BUF_LEN 1024
#else
#define PARA_BUF_LEN 512
#endif

static bool page_pool_fill(struct page_pool *pool);

static int order_to_index(unsigned int order)
{
	int i;

	for (i = 0; i < NUM_ORDERS; i++) {
		if (order == orders[i])
			return i;
	}
	return -1;
}

static void page_pool_wakeup_process(struct page_pool *pool)
{
	if (unlikely(!ux_page_pool_enabled))
		return;

	if (pool == NULL) {
		pr_err("%s: boost_pool is NULL!\n", __func__);
		return;
	}

	if (fillthread_enabled) {
		kworkthread_wait_flag = 1;
		wake_up_interruptible(&kworkthread_waitq);
	}
}

void __maybe_unused set_ux_page_pool_fillthread_cpus(void)
{
	struct cpumask mask;
	struct cpumask *cpumask = &mask;
	pg_data_t *pgdat = NODE_DATA(0);
	unsigned int cpu = 0, cpufreq_max_tmp = 0;
	struct cpufreq_policy *policy_max = NULL;

	for_each_possible_cpu(cpu) {
		struct cpufreq_policy *policy = cpufreq_cpu_get(cpu);

		if (policy == NULL)
			continue;

		if (policy->cpuinfo.max_freq >= cpufreq_max_tmp) {
			cpufreq_max_tmp = policy->cpuinfo.max_freq;
			policy_max = policy;
		}
	}

	cpumask_copy(cpumask, cpumask_of_node(pgdat->node_id));
	if (policy_max)
		cpumask_andnot(cpumask, cpumask, policy_max->related_cpus);

	if (!cpumask_empty(cpumask))
		set_cpus_allowed_ptr(current, cpumask);
}

static int ux_page_pool_fillthread(void *p)
{
	struct page_pool *pool;
	int i, j;
	int ret;
#ifdef UXPAGEPOOL_DEBUG
	unsigned long begin;
	int record_i, record_j;
#endif

	if (unlikely(!ux_page_pool_enabled))
		return -1;

	set_ux_page_pool_fillthread_cpus();

	while (!kthread_should_stop()) {
		ret = wait_event_interruptible(kworkthread_waitq,
						       (kworkthread_wait_flag == 1));
		if (ret < 0)
			continue;

		kworkthread_wait_flag = 0;

		for (i = 0; i < NUM_ORDERS; i++) {
			for (j = 0; j < POOL_MIGRATETYPE_TYPES_SIZE; j++) {
				pool = pools[i][j];
				if (pool == NULL)
					continue;
#ifdef UXPAGEPOOL_DEBUG
				if (pool->count < pool->low) {
					record_i = i;
					record_j = j;
				}

				begin = jiffies;
				pr_info("fill start >>>>>order:%d migratetype:%d low: %d high: %d count:%d gfp_mask %#x.\n",
					pool->order, j, pool->low, pool->high, pool->count, pool->gfp_mask);
#endif
				while (pool->count < pool->high) {
					if (!page_pool_fill(pool)) {
						/* sleep for 20ms if alloc fail */
						msleep_interruptible(20);
					}
				}
#ifdef UXPAGEPOOL_DEBUG
				pr_info("fill end   <<<<<order:%d migratetype:%d low: %d high: %d count:%d use %dms\n",
					pool->order, j, pool->low, pool->high, pool->count,
					jiffies_to_msecs(jiffies - begin));
#endif
			}
		}

#ifdef UXPAGEPOOL_DEBUG
		atomic_long_inc(&fillthread_runtime_times[record_i][record_j]);
#endif
	}
	return 0;
}

struct page_pool *ux_page_pool_create(gfp_t gfp_mask, unsigned int order,
		unsigned int nr_pages, int migratetype)
{
	struct page_pool *pool;

	pool = kmalloc(sizeof(*pool), GFP_KERNEL);
	if (!pool)
		return ERR_PTR(-ENOMEM);

	pool->gfp_mask = gfp_mask;
	pool->order = order;
	pool->count = 0;
	pool->high = nr_pages >> order;
	pool->low  = pool->high / 2;
	INIT_LIST_HEAD(&pool->items);
	spin_lock_init(&pool->lock);

	pr_info("%s order:%d migratetype:%d low: %d high: %d count:%d.\n",
		__func__, pool->order, migratetype, pool->low, pool->high, pool->count);

	return pool;
}

static void page_pool_add(struct page_pool *pool, struct page *page)
{
	unsigned long flags;

	spin_lock_irqsave(&pool->lock, flags);
	list_add_tail(&page->lru, &pool->items);
	pool->count++;
	spin_unlock_irqrestore(&pool->lock, flags);
}

static struct page *page_pool_remove(struct page_pool *pool)
{
	struct page *page;
	unsigned long flags;

	spin_lock_irqsave(&pool->lock, flags);
	page = list_first_entry_or_null(&pool->items, struct page, lru);

	if (page) {
		pool->count--;
		list_del(&page->lru);
	}
	spin_unlock_irqrestore(&pool->lock, flags);

	/* wakeup kthread on count < low*/
	if (pool->count < pool->low)
		page_pool_wakeup_process(pool);

	return page;
}

static bool page_pool_fill(struct page_pool *pool)
{
	struct page *page;
	gfp_t gfp_refill = pool->gfp_mask;

	if (pool == NULL) {
		pr_err("%s: pool is NULL!\n", __func__);
		return false;
	}

	page = alloc_pages(gfp_refill, pool->order);
	if (page == NULL)
		return false;

	page_pool_add(pool, page);
	return true;
}

/* fast path */
struct page *ux_page_pool_alloc_pages(unsigned int order, int migratetype)
{
	struct page *page = NULL;
	int retries = 0;
	struct page_pool *pool = NULL;
	int pool_migratetype;
	int order_ind = order_to_index(order);

	if (unlikely(!ux_page_pool_enabled) || (order_ind == -1))
		return NULL;

	if (migratetype > MIGRATE_MOVABLE)
		return NULL;

	pool_migratetype = (migratetype == MIGRATE_MOVABLE) ?
			POOL_MIGRATETYPE_MOVABLE : POOL_MIGRATETYPE_UNMOVABLE;

	pool = pools[order_ind][pool_migratetype];
	if (pool == NULL)
		return NULL;

#ifdef UXPAGEPOOL_DEBUG
	atomic_long_inc(&g_alloc_pages_cnts[order_ind][pool_migratetype]);
#endif

retry:
	/* Fast-path: Get a page from pool */
	page = page_pool_remove(pool);
	/* if erm is enabled, we don't need to retry */
	if (!page && retries < MAX_UXMEM_POOL_ALLOC_RETRIES && !ezr_enabled) {
		retries++;
		goto retry;
	}

#ifdef UXPAGEPOOL_DEBUG
	if (page)
		atomic_long_inc(&g_alloc_pages_fast[order][pool_migratetype]);
	else
		atomic_long_inc(&g_alloc_pages_slow[order][pool_migratetype]);

	if (retries != 0)
		atomic_long_inc(&g_alloc_pages_fast_retry[order][pool_migratetype][retries - 1]);
#endif

	if (!page) {
		ux_pool_alloc_fail += 1;
#ifdef UXPAGEPOOL_DEBUG
		atomic_long_inc(&g_alloc_fail_cnts[order_ind][pool_migratetype]);
#endif
	}

	return page;
}

static bool ux_page_pool_refill(struct page *page, unsigned int order)
{
	struct page_pool *pool;
	int order_ind = order_to_index(order);
	int pool_migratetype;
	int actual_migratetype;

	if (unlikely(!ux_page_pool_enabled) || (order_ind == -1))
		return false;

	actual_migratetype = get_pageblock_migratetype(page);
	if (actual_migratetype > MIGRATE_MOVABLE)
		return false;

	pool_migratetype = (actual_migratetype == MIGRATE_MOVABLE) ?
			POOL_MIGRATETYPE_MOVABLE : POOL_MIGRATETYPE_UNMOVABLE;

	pool = pools[order_ind][pool_migratetype];
	if (pool == NULL)
		return false;

	if (pool->count >= pool->high)
		return false;

	page_pool_add(pool, page);
	return true;
}

static ssize_t ux_page_pool_write(struct file *file,
		const char __user *buff, size_t len, loff_t *ppos)
{
	char kbuf[PARA_BUF_LEN] = {'\0'};
	char *str;
	int high_0 = 0, high_1 = 0;
	int pool_migratetype = 0;
	int ret;
	struct page_pool *pool;
	unsigned long flags;

	if (len > PARA_BUF_LEN - 1) {
		pr_err("len %ld is too long\n", len);
		return -EINVAL;
	}

	if (copy_from_user(&kbuf, buff, len))
		return -EFAULT;
	kbuf[len] = '\0';

	str = strstrip(kbuf);
	if (*str == 0) {
		pr_err("buff %s is invalid\n", kbuf);
		return -EINVAL;
	}

	if (strstr(str, "fillthread_pause")) {
		fillthread_enabled = false;
		return len;
	}

	if (strstr(str, "fillthread_resume")) {
		fillthread_enabled = true;
		return len;
	}

	ret = sscanf(str, "%d %d %d", &pool_migratetype, &high_0, &high_1);

	if (ret != 3) {
		pr_err("input para invalid, format is : migratetype high_order0 high_order1\n");
		return -EINVAL;
	}

	if (pool_migratetype >= POOL_MIGRATETYPE_TYPES_SIZE) {
		pr_err("pool_migratetype %d is invalid\n", pool_migratetype);
		return -EINVAL;
	}

	if (high_0 < 0 || high_0 > 81920)  {
		pr_err("high_0 %d is invalid\n", high_0);
		return -EINVAL;
	}

	if (high_1 < 0 || high_1 > 16384)  {
		pr_err("high_1 %d is invalid\n", high_1);
		return -EINVAL;
	}

	pool = pools[0][pool_migratetype];
	if (!pool) {
		pr_err("pool[0][%d] is NULL\n", pool_migratetype);
		return -EINVAL;
	}
	spin_lock_irqsave(&pool->lock, flags);
	pool->high = high_0;
	pool->low  = pool->high / 2;
	spin_unlock_irqrestore(&pool->lock, flags);
	pr_info("%s order:%d migratetype:%d low: %d high: %d count:%d.\n",
		__func__, pool->order, pool_migratetype, pool->low, pool->high, pool->count);

	pool = pools[1][pool_migratetype];
	if (!pool) {
		pr_err("pool[1][%d] is NULL\n", pool_migratetype);
		return -EINVAL;
	}
	spin_lock_irqsave(&pool->lock, flags);
	pool->high = high_1;
	pool->low  = pool->high / 2;
	spin_unlock_irqrestore(&pool->lock, flags);
	pr_info("%s order:%d migratetype:%d low: %d high: %d count:%d.\n",
		__func__, pool->order, pool_migratetype, pool->low, pool->high, pool->count);

	return len;
}

static ssize_t ux_page_pool_read(struct file *file,
		char __user *buffer, size_t count, loff_t *off)
{
	char kbuf[PARA_BUF_LEN] = {'\0'};
	int len = 0;
	int i, j;
	struct page_pool *pool;

	for (i = 0; i < NUM_ORDERS; i++) {
		for (j = 0; j < POOL_MIGRATETYPE_TYPES_SIZE; j++) {
			pool = pools[i][j];
			if (!pool)
				continue;
			len += snprintf(kbuf + len, PARA_BUF_LEN - len,
					"order:%d migratetype:%d low: %d high: %d count:%d.\n",
					pool->order, j, pool->low, pool->high, pool->count);
		}
	}

	len += snprintf(kbuf + len, PARA_BUF_LEN - len,
			"page_pool alloc fail count:%ld\n", ux_pool_alloc_fail);
	len += snprintf(kbuf + len, PARA_BUF_LEN - len,
			"page_pool fillthread status:%s\n",
			fillthread_enabled ? "running" : "not running");

#ifdef UXPAGEPOOL_DEBUG
	for (i = 0; i < NUM_ORDERS; i++) {
		for (j = 0; j < POOL_MIGRATETYPE_TYPES_SIZE; j++) {
			len += snprintf(kbuf + len, PARA_BUF_LEN - len,
				"order:%d, migratetype:%d, alloc_pages_cnts: %ld, alloc_fail_cnts: %ld\n",
				i, j, atomic_long_read(&g_alloc_pages_cnts[i][j]), atomic_long_read(&g_alloc_fail_cnts[i][j]));
		}
	}
#endif

	if (len == PARA_BUF_LEN)
		kbuf[len - 1] = '\0';

	if (len > *off)
		len -= *off;
	else
		return 0;

	if (copy_to_user(buffer, kbuf + *off, (len < count ? len : count)))
		return -EFAULT;

	*off += (len < count ? len : count);
	return (len < count ? len : count);
}

static const struct proc_ops ux_page_pool_fops = {
	.proc_write = ux_page_pool_write,
	.proc_read = ux_page_pool_read,
};

struct pglist_data *first_online_pgdat(void)
{
	return NODE_DATA(first_online_node);
}

struct pglist_data *next_online_pgdat(struct pglist_data *pgdat)
{
	int nid = next_online_node(pgdat->node_id);

	if (nid == MAX_NUMNODES)
		return NULL;
	return NODE_DATA(nid);
}

struct zone *next_zone(struct zone *zone)
{
	pg_data_t *pgdat = zone->zone_pgdat;

	if (zone < pgdat->node_zones + MAX_NR_ZONES - 1)
		zone++;
	else {
		pgdat = next_online_pgdat(pgdat);
		if (pgdat)
			zone = pgdat->node_zones;
		else
			zone = NULL;
	}
	return zone;
}

/* As prep_new_page is inlined, we duplicate one here */
static void __nocfi prep_new_page_dup(struct page *page, unsigned int order, gfp_t gfp_flags,
							unsigned int alloc_flags)
{
	post_alloc_hook_dup(page, order, gfp_flags);

	if (order && (gfp_flags & __GFP_COMP))
		prep_compound_page_dup(page, order);
}

static struct proc_dir_entry *ux_page_pool_entry;
static int ux_page_pool_init(void)
{
	int ret = 0;
	int i, j;
	bool err_cleanup_pools = false;
	struct proc_dir_entry *root_dir_entry = proc_mkdir("oplus_mem", NULL);
	struct zone *zone;
	struct kprobe post_alloc_hook_kp = {
		.symbol_name = "post_alloc_hook"
	};

	struct kprobe prep_compound_page_kp = {
		.symbol_name = "prep_compound_page"
	};
#if IS_ENABLED(CONFIG_OPLUS_FEATURE_MM_OSVELTE)
	struct config_oplus_bsp_uxmem_opt *config;
	config = oplus_read_mm_config(module_name_uxmem_opt);
#endif /* CONFIG_OPLUS_FEATURE_MM_OSVELTE */

	/* get some symbols address using kprobe */
	ret = register_kprobe(&post_alloc_hook_kp);
	if (ret) {
		pr_err("get post_alloc_hook addr from kprobe failed! ret=%d\n", ret);
		return ret;
	}
	post_alloc_hook_dup = (post_alloc_hook_t)post_alloc_hook_kp.addr;
	pr_info("successfully get post_alloc_hook addr:0x%px\n", post_alloc_hook_dup);
	unregister_kprobe(&post_alloc_hook_kp);

	ret = register_kprobe(&prep_compound_page_kp);
	if (ret) {
		pr_err("get prep_compound_page addr from kprobe failed! ret=%d\n", ret);
		return ret;
	}
	prep_compound_page_dup = (prep_compound_page_t)prep_compound_page_kp.addr;
	pr_info("successfully get prep_compound_page addr:0x%px\n", prep_compound_page_dup);
	unregister_kprobe(&prep_compound_page_kp);

#if IS_ENABLED(CONFIG_OPLUS_FEATURE_MM_OSVELTE)
	if (config && config->page_pool_order0_unmovable_mb) {
		page_pool_nr_pages[0][POOL_MIGRATETYPE_UNMOVABLE] = config->page_pool_order0_unmovable_mb * SZ_1M / PAGE_SIZE;
	}
	if (config && config->page_pool_order0_movable_mb) {
		page_pool_nr_pages[0][POOL_MIGRATETYPE_MOVABLE] = config->page_pool_order0_movable_mb * SZ_1M / PAGE_SIZE;
	}
	if (config && config->page_pool_order1_unmovable_mb) {
		page_pool_nr_pages[1][POOL_MIGRATETYPE_UNMOVABLE] = config->page_pool_order1_unmovable_mb * SZ_1M / PAGE_SIZE;
	}
	if (config && config->page_pool_order1_movable_mb) {
		page_pool_nr_pages[1][POOL_MIGRATETYPE_MOVABLE] = config->page_pool_order1_movable_mb * SZ_1M / PAGE_SIZE;
	}
#endif /* CONFIG_OPLUS_FEATURE_MM_OSVELTE */

	for (i = 0; i < NUM_ORDERS; i++) {
		struct page_pool *pool;
		pool = ux_page_pool_create((GFP_HIGHUSER | __GFP_ZERO |
			   __GFP_NOWARN | __GFP_NORETRY) & ~__GFP_RECLAIM, orders[i],
			   page_pool_nr_pages[i][POOL_MIGRATETYPE_UNMOVABLE], POOL_MIGRATETYPE_UNMOVABLE);
		if (IS_ERR(pool)) {
			ret = PTR_ERR(pool);
			pools[i][POOL_MIGRATETYPE_UNMOVABLE] = NULL;
			pr_err("create unmovable pool failed for order %d: %d\n", i, ret);
			err_cleanup_pools = true;
			break;
		}
		pools[i][POOL_MIGRATETYPE_UNMOVABLE] = pool;

		pool = ux_page_pool_create((GFP_HIGHUSER_MOVABLE | __GFP_ZERO |
			   __GFP_NOWARN | __GFP_NORETRY) & ~__GFP_RECLAIM, orders[i],
			   page_pool_nr_pages[i][POOL_MIGRATETYPE_MOVABLE], POOL_MIGRATETYPE_MOVABLE);
		if (IS_ERR(pool)) {
			ret = PTR_ERR(pool);
			pools[i][POOL_MIGRATETYPE_MOVABLE] = NULL;
			pr_err("create movable pool failed for order %d: %d\n", i, ret);
			err_cleanup_pools = true;
			break;
		}
		pools[i][POOL_MIGRATETYPE_MOVABLE] = pool;
	}

	if (err_cleanup_pools) {
		for (i = 0; i < NUM_ORDERS; i++) {
			for (j = 0; j < POOL_MIGRATETYPE_TYPES_SIZE; j++) {
				if (pools[i][j]) {
					kfree(pools[i][j]);
					pools[i][j] = NULL;
				}
			}
		}
		return ret;
	}

	ux_page_pool_enabled = true;

	init_waitqueue_head(&kworkthread_waitq);
	ux_page_pool_tsk = kthread_run(ux_page_pool_fillthread, NULL, UX_PAGE_POOL_NAME);
	if (IS_ERR_OR_NULL(ux_page_pool_tsk)) {
		pr_err("%s:run ux_page_pool_fillthread failed!\n", __func__);
		fillthread_enabled = false;
	} else {
		fillthread_enabled = true;
		page_pool_wakeup_process(pools[0][0]);
	}

	ux_page_pool_entry = proc_create((root_dir_entry ?
				"ux_page_pool" : "oplus_mem/ux_page_pool"),
				0666, root_dir_entry, &ux_page_pool_fops);

	/* record dma32_zone pageset if exist */
	for_each_zone(zone) {
		if (!strcmp(zone->name, "DMA32") && populated_zone(zone)) {
			pr_info("zone DMA32 detect\n");
			dma32_pcp_base = zone->per_cpu_pageset;
		}
	}

	return 0;
}

/*
inline int task_is_fg(struct task_struct *tsk)
{
	int cur_uid;

	cur_uid = task_uid(tsk).val;
	if (is_fg(cur_uid))
		return 1;
	return 0;
}
*/

static inline bool current_is_key_task(void)
{
	unsigned long im_flag = oplus_get_im_flag(current);

	return test_task_ux(current) || rt_task(current)
		|| test_bit(IM_FLAG_SURFACEFLINGER, &im_flag)
		|| test_bit(IM_FLAG_SYSTEMSERVER_PID, &im_flag)
		|| ta_task(current)
		|| (current->flags & PF_WQ_WORKER);
}

static void __nocfi get_page_from_uxmempool(void *data, gfp_t gfp_mask, int order, int alloc_flags,
		int migratetype, struct page **p_page)
{
	struct page *page = NULL;

	/* page allocated by erm, just return */
	if (*p_page)
		return;

	if (current_is_key_task() && !(gfp_mask & __GFP_DMA32)) {
		page = ux_page_pool_alloc_pages(order, migratetype);
		if (page) {
			if (!page_count(page)) {
				/* clear __GFP_DIRECT_RECLAIM because preempt is disabled in vh */
				prep_new_page_dup(page, order, gfp_mask & ~(__GFP_DIRECT_RECLAIM), ALLOC_WMARK_LOW);
			} else {
				if (gfp_mask & __GFP_SKIP_KASAN) {
					int i;
					for (i = 0; i != 1 << order; ++i)
						page_kasan_tag_reset(page + i);
				}

				if (order && (gfp_mask & __GFP_COMP))
					prep_compound_page_dup(page, order);
			}
		}
	}
	*p_page = page;
}

static void uxmempool_refill(void *data, struct page *page, int order,
		int migratetype, bool *bypass)
{
	struct zone *zone = page_zone(page);
	unsigned long mark = zone->_watermark[WMARK_LOW];
	long free_pages = zone_page_state(zone, NR_FREE_PAGES);

	free_pages -= zone->nr_reserved_highatomic;

	if ((migratetype <= MIGRATE_MOVABLE) && (zone_idx(zone) == ZONE_NORMAL)
			&& free_pages > mark) {
		if (ux_page_pool_refill(page, order))
			*bypass = true;
	}
}

#define KMALLOC_MAX_PAGES 8
#define UXMEM_POOL_MAX_PAGES 2
#define UXMEM_POOL_MAX_ORDER get_order(UXMEM_POOL_MAX_PAGES)

static void adjust_kvmalloc_flags(void *data, unsigned int order, gfp_t *alloc_flags)
{
	if ((order > UXMEM_POOL_MAX_ORDER) && test_task_ux(current))
		*alloc_flags &= ~__GFP_DIRECT_RECLAIM;
}


static void kvmalloc_check_use_vmalloc(void *data, size_t size,
		gfp_t *kmalloc_flags, bool *use_vmalloc)
{
	if ((size >= KMALLOC_MAX_PAGES * PAGE_SIZE) &&
				gfpflags_allow_blocking(*kmalloc_flags) && !test_task_ux(current))
		*use_vmalloc = true;
}

static void should_alloc_pages_retry(void *data, gfp_t gfp_mask,
		int order, int *alloc_flags, int migratetype,
		struct zone *preferred_zone, struct page **p_page, bool *should_alloc_retry)
{
	if (unlikely(test_task_ux(current)) && !in_interrupt() &&
		(preferred_zone->nr_reserved_highatomic >= (SZ_8M >> PAGE_SHIFT)) &&
		!(*alloc_flags & ALLOC_RESERVES) && !(gfp_mask & __GFP_NORETRY)) {
		*alloc_flags |= ALLOC_NON_BLOCK;
		*should_alloc_retry = true;
	} else {
		*should_alloc_retry = false;
	}
}

static void unreserve_highatomic_bypass(void *data, bool force,
		struct zone *zone, bool *skip_unreserve_highatomic)
{
	if (!force && (zone->nr_reserved_highatomic <= (SZ_16M >> PAGE_SHIFT)))
		*skip_unreserve_highatomic = true;
	else
		*skip_unreserve_highatomic = false;
}

static bool in_dma32_zone(struct per_cpu_pages *pcp)
{
	if (dma32_pcp_base)
		return pcp == this_cpu_ptr(dma32_pcp_base);
	return false;
}

static void fill_pcplist_from_uxmempool(void *data, unsigned int order,
		struct per_cpu_pages *pcp, int migratetype, struct list_head *list)
{
	struct page *page = NULL;

	if (current_is_key_task() && !in_dma32_zone(pcp)) {
		page = ux_page_pool_alloc_pages(order, migratetype);
	}

	if (page) {
		set_page_count(page, 0);
		list_add_tail(&page->lru, list);
		pcp->count += 1 << order;
	}
}

static unsigned long total_pool_pages(void)
{
	unsigned long total = 0;
	int i, j;
	struct page_pool *pool;

	for (i = 0; i < NUM_ORDERS; i++) {
		for (j = 0; j < POOL_MIGRATETYPE_TYPES_SIZE; j++) {
			pool = pools[i][j];
			if (pool)
				total += pool->count << orders[i];
		}
	}
	return total;
}

static void meminfo_adjust(void *data, unsigned long *totalram, unsigned long *freeram)
{
	if (unlikely(!ux_page_pool_enabled))
		return;

	/* make sure totalram is a kernel address */
	if ((unsigned long)totalram > PAGE_SIZE) {
		*freeram += total_pool_pages();
	}
}

static long read_uxmem_pool_usage(enum mtrack_subtype type)
{
	if (unlikely(!ux_page_pool_enabled))
		return 0;

	if (type == MTRACK_UXMEM_POOL_TOTAL)
		return total_pool_pages();
	return 0;
}

static struct mtrack_debugger uxmem_pool_mtrack_debugger = {
	.mem_usage = read_uxmem_pool_usage,
};

static void calc_alloc_flags(void *data, gfp_t gfp_mask, unsigned int *alloc_flags, bool *bypass)
{
	if (current == ux_page_pool_tsk)
		*bypass = true;
}

static void mem_available_adjust(void *data, unsigned long *available)
{
	if (unlikely(!ux_page_pool_enabled))
		return;

	*available += total_pool_pages();
}

static int register_uxmem_opt_vendor_hooks(void)
{
	int ret = 0;

	ret = register_trace_android_vh_alloc_pages_reclaim_bypass(get_page_from_uxmempool, NULL);
	if (ret != 0) {
		pr_err("register_trace_android_vh_alloc_pages_reclaim_bypass failed! ret=%d\n",
				ret);
		goto out;
	}

	ret = register_trace_android_vh_free_unref_page_bypass(uxmempool_refill, NULL);
	if (ret != 0) {
		pr_err("register_trace_android_vh_free_page failed! ret=%d\n", ret);
		goto out;
	}

	ret = register_trace_android_vh_adjust_kvmalloc_flags(adjust_kvmalloc_flags, NULL);
	if (ret != 0) {
		pr_err("register_trace_android_vh_adjust_kvmalloc_flags failed! ret=%d\n",
				ret);
		goto out;
	}

	ret = register_trace_android_vh_kvmalloc_node_use_vmalloc(kvmalloc_check_use_vmalloc, NULL);
	if (ret != 0) {
		pr_err("register_trace_android_vh_kvmalloc_node_use_vmalloc failed! ret=%d\n",
				ret);
		goto out;
	}

	ret = register_trace_android_vh_should_alloc_pages_retry(should_alloc_pages_retry, NULL);
	if (ret != 0) {
		pr_err("register_trace_android_vh_should_alloc_pages_retry failed! ret=%d\n", ret);
		goto out;
	}

	ret = register_trace_android_vh_unreserve_highatomic_bypass(unreserve_highatomic_bypass,
			NULL);
	if (ret != 0) {
		pr_err("register_trace_android_vh_unreserve_highatomic_bypass failed! ret=%d\n",
				ret);
		goto out;
	}

	ret = register_trace_android_vh_rmqueue_bulk_bypass(fill_pcplist_from_uxmempool, NULL);
	if (ret != 0) {
		pr_err("register_trace_android_vh_rmqueue_bulk_bypass failed! ret=%d\n", ret);
		goto out;
	}

	if (!ezr_enabled) {
		ret = register_trace_android_vh_si_meminfo_adjust(meminfo_adjust, NULL);
		if (ret != 0) {
			pr_err("register_trace_android_vh_si_meminfo_adjust failed! ret=%d\n", ret);
			goto out;
		}
	}

	ret = register_trace_android_vh_calc_alloc_flags(calc_alloc_flags, NULL);
	if (ret != 0) {
		pr_err("register_trace_android_vh_calc_alloc_flags failed! ret=%d\n", ret);
		goto out;
	}

	ret = register_trace_android_vh_si_mem_available_adjust(mem_available_adjust, NULL);
	if (ret != 0) {
		pr_err("register_trace_android_vh_si_mem_available_adjust failed! ret=%d\n", ret);
		goto out;
	}
out:
	return ret;
}

static void unregister_uxmem_opt_vendor_hooks(void)
{
	unregister_trace_android_vh_calc_alloc_flags(calc_alloc_flags, NULL);

	unregister_trace_android_vh_si_mem_available_adjust(mem_available_adjust, NULL);

	if (!ezr_enabled)
		unregister_trace_android_vh_si_meminfo_adjust(meminfo_adjust, NULL);

	unregister_trace_android_vh_rmqueue_bulk_bypass(fill_pcplist_from_uxmempool, NULL);

	unregister_trace_android_vh_unreserve_highatomic_bypass(unreserve_highatomic_bypass, NULL);

	unregister_trace_android_vh_should_alloc_pages_retry(should_alloc_pages_retry, NULL);

	unregister_trace_android_vh_kvmalloc_node_use_vmalloc(kvmalloc_check_use_vmalloc, NULL);

	unregister_trace_android_vh_adjust_kvmalloc_flags(adjust_kvmalloc_flags, NULL);

	unregister_trace_android_vh_free_unref_page_bypass(uxmempool_refill, NULL);

	unregister_trace_android_vh_alloc_pages_reclaim_bypass(get_page_from_uxmempool, NULL);
}

static int __init uxmem_opt_init(void)
{
	int ret = 0;

#if IS_ENABLED(CONFIG_OPLUS_FEATURE_MM_OSVELTE)
	struct config_ezreclaimd *config_ezr;
	struct config_oplus_bsp_uxmem_opt *config;

	config_ezr = oplus_read_mm_config(module_name_ezreclaimd);
	if (config_ezr && config_ezr->enable)
		ezr_enabled = true;

	config = oplus_read_mm_config(module_name_uxmem_opt);
	if (config && !config->enable) {
		pr_info("%s is disabled in config\n", module_name_uxmem_opt);
		return 0;
	}
#endif /* CONFIG_OPLUS_FEATURE_MM_OSVELTE */

	if (!enable) {
		pr_err("oplus_bsp_uxmem_opt is disabled in cmdline\n");
		return 0;
	}

	if (IS_ENABLED(CONFIG_PAGE_POISONING)) {
		pr_err("oplus_bsp_uxmem_opt conflict with CONFIG_PAGE_POISONING\n");
		return 0;
	}

	ret = ux_page_pool_init();
	if (ret != 0) {
		pr_err("uxmem_opt init failed!\n");
		return ret;
	}

	ret = register_uxmem_opt_vendor_hooks();
	if (ret != 0)
		return ret;

	register_mtrack_debugger(MTRACK_UXMEM_POOL, &uxmem_pool_mtrack_debugger);
	pr_info("uxmem_opt_init succeed!\n");
	return 0;
}

static void __exit uxmem_opt_exit(void)
{
	int i, j;
	struct page_pool *pool;
	struct page *page;

	unregister_uxmem_opt_vendor_hooks();
	proc_remove(ux_page_pool_entry);

	if (ux_page_pool_tsk) {
		fillthread_enabled = false;
		kthread_stop(ux_page_pool_tsk);
		ux_page_pool_tsk = NULL;
	}

	/* cleanup pools */
	for (i = 0; i < NUM_ORDERS; i++) {
		for (j = 0; j < POOL_MIGRATETYPE_TYPES_SIZE; j++) {
			pool = pools[i][j];
			if (!pool)
				continue;

			/* drain all pages in pool */
			while ((page = page_pool_remove(pool)) != NULL)
				__free_pages(page, pool->order);

			kfree(pool);
			pools[i][j] = NULL;
		}
	}
	ux_page_pool_enabled = false;
	pr_info("uxmem_opt_exit succeed!\n");
}

module_init(uxmem_opt_init);
module_exit(uxmem_opt_exit);

MODULE_LICENSE("GPL v2");
