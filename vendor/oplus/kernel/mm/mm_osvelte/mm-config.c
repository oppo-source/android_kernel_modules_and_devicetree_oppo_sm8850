...
#define MAX_UXMEM_POOL_SIZE 250
...
static void config_uxmem_opt_show(struct seq_file *m, struct config_data *cd)
{
	...
	seq_printf(m, "[%s]\n", cd->module_name);
	seq_printf(m, " enable: %d\n", config->enable);
	seq_printf(m, " order 0 unmovable pool size(MB): %d\n", config->page_pool_order0_unmovable_mb);
	seq_printf(m, " order 0 movable pool size(MB): %d\n", config->page_pool_order0_movable_mb);
	seq_printf(m, " order 1 unmovable pool size(MB): %d\n", config->page_pool_order1_unmovable_mb);
	seq_printf(m, " order 1 movable pool size(MB): %d\n", config->page_pool_order1_movable_mb);
}
...
static void parse_uxmem_opt_dt(const struct device_node *root)
{
	...
	err = of_property_read_u32(node, "page_pool_order0_unmovable_mb", &config->page_pool_order0_unmovable_mb);
	if (err) {
		osvelte_logi("read page_pool_order0_unmovable_mb fail(%d)\n", err);
	} else if (config->page_pool_order0_unmovable_mb > MAX_UXMEM_POOL_SIZE) {
		osvelte_loge("page_pool_order0_unmovable_mb %d too large, set to %d\n", config->page_pool_order0_unmovable_mb, MAX_UXMEM_POOL_SIZE);
		config->page_pool_order0_unmovable_mb = MAX_UXMEM_POOL_SIZE;
	}

	err = of_property_read_u32(node, "page_pool_order0_movable_mb", &config->page_pool_order0_movable_mb);
	if (err) {
		osvelte_logi("read page_pool_order0_movable_mb fail(%d)\n", err);
	} else if (config->page_pool_order0_movable_mb > MAX_UXMEM_POOL_SIZE) {
		osvelte_loge("page_pool_order0_movable_mb %d too large, set to %d\n", config->page_pool_order0_movable_mb, MAX_UXMEM_POOL_SIZE);
		config->page_pool_order0_movable_mb = MAX_UXMEM_POOL_SIZE;
	}

	err = of_property_read_u32(node, "page_pool_order1_unmovable_mb", &config->page_pool_order1_unmovable_mb);
	if (err) {
		osvelte_logi("read page_pool_order1_unmovable_mb fail(%d)\n", err);
	} else if (config->page_pool_order1_unmovable_mb > MAX_UXMEM_POOL_SIZE) {
		osvelte_loge("page_pool_order1_unmovable_mb %d too large, set to %d\n", config->page_pool_order1_unmovable_mb, MAX_UXMEM_POOL_SIZE);
		config->page_pool_order1_unmovable_mb = MAX_UXMEM_POOL_SIZE;
	}

	err = of_property_read_u32(node, "page_pool_order1_movable_mb", &config->page_pool_order1_movable_mb);
	if (err) {
		osvelte_logi("read page_pool_order1_movable_mb fail(%d)\n", err);
	} else if (config->page_pool_order1_movable_mb > MAX_UXMEM_POOL_SIZE) {
		osvelte_loge("page_pool_order1_movable_mb %d too large, set to %d\n", config->page_pool_order1_movable_mb, MAX_UXMEM_POOL_SIZE);
		config->page_pool_order1_movable_mb = MAX_UXMEM_POOL_SIZE;
	}
