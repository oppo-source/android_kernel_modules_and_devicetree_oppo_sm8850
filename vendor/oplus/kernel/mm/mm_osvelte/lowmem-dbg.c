...
/* lmkd use this for now, remove in the future */
...
int osvelte_lowmem_dbg_init(struct kobject *root, struct proc_dir_entry *proc_root)
{
	struct lowmem_dbg_cfg *cfg = &g_cfg;
	...
	if (total_ram >= PAGES(SZ_4G + SZ_8G))
		cfg->watermark_low = PAGES(SZ_1G + SZ_512M + SZ_256M);
	...
	proc_create("lowmem_dbg", 0444, proc_root, &lowmem_dbg_proc_ops);
