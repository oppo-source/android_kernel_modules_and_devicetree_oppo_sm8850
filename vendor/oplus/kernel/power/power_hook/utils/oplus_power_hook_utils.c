...
static struct tracepoints_table *interest_tracepoints_table;
...
static void lookup_tracepoints(struct tracepoint *tp, void *ignore)
{
	...
	for (i = 0; i < tpt_size; i++) {
		if (&interest_tracepoints_table[i] &&
			interest_tracepoints_table[i].name && tp->name &&
			!strncmp(interest_tracepoints_table[i].name, tp->name,
				strlen(interest_tracepoints_table[i].name))) {
			pr_debug("[oplus_power_hook] tracepoint %s found\n",
					interest_tracepoints_table[i].name);
			interest_tracepoints_table[i].tp = tp;
		}
	}
}
...
void tracepoint_cleanup(int index)
{
	...
	for (i = 0; i < index; i++) {
		tpt = &interest_tracepoints_table[i];
		if (tpt->registered) {
...
void find_and_register_tracepoint_probe(struct tracepoints_table *tp_table, unsigned int array_size)
{
	...
	interest_tracepoints_table = tp_table;
	tpt_size = array_size;
	...
	for (i = 0; i < array_size; i++) {
		...
		if (tpt->tp) {
			pr_debug("[oplus_power_hook] register %s tracepoint function %ps\n",
					tpt->name ? tpt->name : "NULL", tpt->func);
			ret = tracepoint_probe_register(tpt->tp, tpt->func,  NULL);
			if (ret) {
				pr_info("[oplus_power_hook] couldn't activate tracepoint %pf, ret=%d\n",
									tpt->func, ret);
				tracepoint_cleanup(i);
			} else
				tpt->registered = true;
		} else {
			pr_info("[oplus_power_hook] Tracepoint %s not found (tp is NULL)\n",
				tpt->name ? tpt->name : "NULL");
		}
	}
}
