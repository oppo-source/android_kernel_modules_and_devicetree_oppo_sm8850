...
#include "suspend_resume_hook/oplus_suspend_resume_hook.h"
...
#if IS_ENABLED(CONFIG_OPLUS_FEATURE_SUSPEND_RESUME_HOOK)
	ret = oplus_suspend_resume_hook_init();
	if (ret < 0) {
		pr_info("[power_hook] module failed to init suspend resume hook.\n");
		return ret;
	}
#endif
...
#if IS_ENABLED(CONFIG_OPLUS_FEATURE_SUSPEND_RESUME_HOOK)
	oplus_suspend_resume_hook_exit();
#endif
