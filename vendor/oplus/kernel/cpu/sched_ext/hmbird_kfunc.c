...
#include "hmbird_CameraScene/hmbird_CameraBoost.h"
...
/* Called in sched_switch_hanlder, protected by rq lock.*/
static DEFINE_PER_CPU(bool, executed);
bool sched_hook_executed(int cpu)
{
	if (this_cpu_read(executed)) {
		this_cpu_write(executed, false);
		return true;
	}
	return false;
}

__bpf_kfunc void hb_bpf_sched_hook_excuting(int cpu)
{
	this_cpu_write(executed, true);
}
...
__bpf_kfunc int hb_bpf_task_prio_type(struct task_struct *p)
{
	return task_prio_type(p);
}
...
__bpf_kfunc unsigned int hb_bpf_cfg_busy_pct_get(void)
{
	return cfg_busy_pct_get();
}

__bpf_kfunc unsigned int hb_bpf_cfg_hmbird_feat_get(void)
{
	return cfg_hmbird_feat_get();
}
...
__bpf_kfunc void hb_bpf_kick_cpu_if_idle(int cpu)
{
	hmbird_II_kick_cpu_if_idle(cpu);
}

__bpf_kfunc void hb_bpf_get_topapp_boost_tbl(int *enable, struct topapp_boost_ctl *tbc)
{
	hmbird_get_topapp_boost_tbl(enable, tbc);
}
...
__bpf_kfunc void hb_bpf_boost_soft_min_freq(int order)
{
	boost_soft_min_freq(order);
}
...
BTF_ID_FLAGS(func, hb_bpf_sched_hook_excuting);
...
BTF_ID_FLAGS(func, hb_bpf_task_prio_type);
...
BTF_ID_FLAGS(func, hb_bpf_cfg_busy_pct_get);
BTF_ID_FLAGS(func, hb_bpf_cfg_hmbird_feat_get);
...
BTF_ID_FLAGS(func, hb_bpf_kick_cpu_if_idle);
BTF_ID_FLAGS(func, hb_bpf_get_topapp_boost_tbl);
...
BTF_ID_FLAGS(func, hb_bpf_boost_soft_min_freq);
