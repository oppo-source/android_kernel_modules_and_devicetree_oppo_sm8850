...
/*
 * QoS MIN boost via sysctl
 *
 * Write a non-zero value (kHz) to request MIN boost on all present CPUs.
 * Write 0 to cancel (set to FREQ_QOS_MIN_DEFAULT_VALUE).
 *
 * Note: hmbird_qos_request_min() has an internal auto-reset (default 5s).
 */
static unsigned int sysctl_qos_min_boost_khz;
static int cfg_qos_min_boost(const struct ctl_table *table,
			     ...
			     loff_t *ppos)
{
	...
	mutex_lock(&mutex);
	val = sysctl_qos_min_boost_khz;

	ret = proc_dointvec(&tmp, write, buffer, lenp, ppos);
	if (ret)
		goto out;

	if (!write || val == sysctl_qos_min_boost_khz)
		goto out;

	cpumask_copy(&mask, cpu_present_mask);
	if (val == 0)
		hmbird_qos_request_min(&mask, FREQ_QOS_MIN_DEFAULT_VALUE);
	else
		hmbird_qos_request_min(&mask, val);

	sysctl_qos_min_boost_khz = val;
out:
	mutex_unlock(&mutex);
	return ret;
}
...
#define TOPAPP_BOOST_TBL_SIZE		2
#define TOPAPP_BOOST_MIN_DEMAND		256
...
static struct topapp_boost_ctl topapp_boost_tbl[TOPAPP_BOOST_TBL_SIZE];
static int top_app_boost;
static DEFINE_RAW_SPINLOCK(topapp_boost_lock);

void hmbird_get_topapp_boost_tbl(int *enable, struct topapp_boost_ctl *tbc)
{
	int i;

	raw_spin_lock(&topapp_boost_lock);
	for (i = 0; i < TOPAPP_BOOST_TBL_SIZE && i < top_app_boost; i++) {
		tbc[i].demand = topapp_boost_tbl[i].demand;
		tbc[i].cpu = topapp_boost_tbl[i].cpu;
		tbc[i].cpus.bits[0] = topapp_boost_tbl[i].cpus.bits[0];
	}
	*enable = top_app_boost;
	raw_spin_unlock(&topapp_boost_lock);
}

/*
 * topapp_boost sysctl format:
 *   "<enable>,<demand0>:<cpu0>,<demand1>:<cpu1>"
 * Example:
 *   "2,256:4,512:7"
 *
 * Rules:
 * - enable in [1..TOPAPP_BOOST_TBL_SIZE]
 * - demand must be strictly increasing and >= TOPAPP_BOOST_MIN_DEMAND
 * - cpu's cluster_id must be strictly increasing (e.g. little -> big -> super)
 *
 * NOTE: BPF reads the table once in its init(). If you change sysctl at runtime,
 * you must re-attach/refresh the BPF program to take effect.
 */
static int parse_topapp_boost_cfg_and_set(const char *input)
{
	char *token, *input_copy, *cur;
	int tbl_idx = 0, enable = 0, cpus[TOPAPP_BOOST_TBL_SIZE], ret = 0;
	u16 demands[TOPAPP_BOOST_TBL_SIZE];
	u16 prev_demand, demand;
	int prev_cpu, cpu, prev_cluster_id, cluster_id;
	...
	token = strsep(&cur, ",");
	if (!token || sscanf(token, "%d", &enable) != 1 ||
		!enable || enable > TOPAPP_BOOST_TBL_SIZE) {
		ret = -EINVAL;
		goto out;
	}

	while ((token = strsep(&cur, ",")) != NULL && tbl_idx < TOPAPP_BOOST_TBL_SIZE) {
		if (sscanf(token, "%hu:%d", &demands[tbl_idx], &cpus[tbl_idx]) != 2) {
			ret = -EINVAL;
			goto out;
		}

		cluster_id = topology_cluster_id(cpus[tbl_idx]);
		if (demands[tbl_idx] < TOPAPP_BOOST_MIN_DEMAND ||
			cpus[tbl_idx] < 0 || cpus[tbl_idx] >= MAX_NR_CPUS ||
			cluster_id < 0 || cluster_id >= nr_cluster) {
			ret = -EINVAL;
			goto out;
		}

		if (tbl_idx > 0) {
			prev_demand = demands[tbl_idx - 1];
			demand = demands[tbl_idx];
			prev_cpu = cpus[tbl_idx - 1];
			cpu = cpus[tbl_idx];
			prev_cluster_id = topology_cluster_id(prev_cpu);
			cluster_id = topology_cluster_id(cpu);
			if (demand <= prev_demand || cluster_id <= prev_cluster_id) {
				ret = -EINVAL;
				goto out;
			}
		}
	...
	raw_spin_lock(&topapp_boost_lock);
	for (tbl_idx = 0; tbl_idx < enable; tbl_idx++) {
		topapp_boost_tbl[tbl_idx].demand = demands[tbl_idx];
		topapp_boost_tbl[tbl_idx].cpu = cpus[tbl_idx];
		cluster_id = topology_cluster_id(cpus[tbl_idx]);
		topapp_boost_tbl[tbl_idx].cpus.bits[0] = hmbird_cluster[cluster_id].cpus.bits[0];
	}
	top_app_boost = enable;
	raw_spin_unlock(&topapp_boost_lock);
...
static ssize_t print_topapp_boost_cfg(char *buf, size_t buf_size)
{
	int len = 0;
	int i;
	...
	len += scnprintf(buf + len, buf_size - len, "%d", top_app_boost);
	for (i = 0; i < top_app_boost && i < TOPAPP_BOOST_TBL_SIZE; i++) {
		len += scnprintf(buf + len, buf_size - len, ",%hu:%d:0x%lx",
				topapp_boost_tbl[i].demand, topapp_boost_tbl[i].cpu,
				topapp_boost_tbl[i].cpus.bits[0]);
	}

	return len;
}

static int cfg_topapp_boost(const struct ctl_table *table,
				int write, void *buffer, size_t *lenp,
				loff_t *ppos)
{
	char input[128] = { };
	...
	if (write) {
		...
		ret = parse_topapp_boost_cfg_and_set(input);
	} else {
		ret = print_topapp_boost_cfg(input, sizeof(input));
		if (ret < 0) {
			ret = -EFAULT;
			goto unlock;
		}
...
static unsigned int busy_pct = 95;
unsigned int cfg_busy_pct_get(void)
{
	return busy_pct;
}
static int cfg_busy_pct(const struct ctl_table *table,
				int write, void *buffer, size_t *lenp,
				loff_t *ppos)
{
	int ret = -EPERM;
	...
	if (!write) {
		...
		if (val > 100)
			goto unlock;
		trace_hmbird_busy_pct_update(val);
		ret = proc_dointvec(&tmp, write, buffer, lenp, ppos);
	} else {
		ret = proc_dointvec(&tmp, write, buffer, lenp, ppos);
		if (ret || val == busy_pct || val > 100) {
			HMBIRD_SYSCTL_WARN("cfg_busy_pct val %d not set, ret %d\n", val, ret);
			goto unlock;
		}
		busy_pct = val;
		trace_hmbird_busy_pct_update(val);
...
static unsigned int hmbird_feat = HMBIRD_FEATURE_MULTI_PREEMPT;
unsigned int cfg_hmbird_feat_get(void)
{
	return hmbird_feat;
}
static int cfg_hmbird_feat(const struct ctl_table *table,
				int write, void *buffer, size_t *lenp,
				loff_t *ppos)
{
	int ret = -EPERM;
	...
	if (!write) {
		val = hmbird_feat;
		trace_hmbird_feature_update(val);
		ret = proc_dointvec(&tmp, write, buffer, lenp, ppos);
	} else {
		ret = proc_dointvec(&tmp, write, buffer, lenp, ppos);
		if (ret || val == hmbird_feat) {
			HMBIRD_SYSCTL_WARN("cfg_hmbird_feat val %d not set, ret %d\n", val, ret);
			goto unlock;
		}
		hmbird_feat = val;
		trace_hmbird_feature_update(val);
				...
				loff_t *ppos)
{
	...
	hex = cfg_type == PREFER_CPU || cfg_type == PREFER_CLUSTER || cfg_type == PREFER_CPU2PRI;
	...
	if (!write) {
		...
		case PREFER_IDLE:
			...
			break;
		case PREFER_PREEMPT2PRI:
			if (pid_and_val[1] == 1) {
				hmbird_sched_prop_set_prefer_preempt(task, 1);
				hmbird_sched_prop_set_prefer_pri(task, PREFER_PRI1, 1);
			}
			else if (pid_and_val[1] == 0) {
				hmbird_sched_prop_set_prefer_preempt(task, 0);
				hmbird_sched_prop_set_prefer_pri(task, 0, 0);
			}
			else {
				ret = -EINVAL;
				HMBIRD_SYSCTL_WARN("illegal value %d\n", pid_and_val[1]);
		...
		case PREFER_PREEMPT:
			...
			break;
		case PREFER_PRI:
			if (pid_and_val[1] < 0 || pid_and_val[1] > MAX_NR_PRI) {
				ret = -EINVAL;
				HMBIRD_SYSCTL_WARN("illegal value %d\n", pid_and_val[1]);
			}
			if (pid_and_val[1])
				hmbird_sched_prop_set_prefer_pri(task, pid_and_val[1] - 1, 1);
			else
				hmbird_sched_prop_set_prefer_pri(task, pid_and_val[1], 0);
			break;
		case PREFER_CLUSTER:
		...
		case PREFER_CPU2PRI:
			if (pid_and_val[1] < 0 || pid_and_val[1] > 0xff) {
				...
				hmbird_sched_prop_set_prefer_cpu(task, pid_and_val[1], 1);
				hmbird_sched_prop_set_prefer_pri(task, PREFER_PRI0, 1);
			}
			else {
				hmbird_sched_prop_set_prefer_cpu(task, 0, 0);
				hmbird_sched_prop_set_prefer_pri(task, 0, 0);
...
struct ctl_table hmbird_table[] = {
	...
	{
		.procname	= "qos_min_boost_khz",
		.data		= &sysctl_qos_min_boost_khz,
		.maxlen		= sizeof(unsigned int),
		.mode		= 0644,
		.proc_handler	= cfg_qos_min_boost,
		.extra1		= SYSCTL_ZERO,
		.extra2		= SYSCTL_INT_MAX,
	},
	{
		.procname	= "scx_enabled",
		.data		= &__hb_ops_enabled.counter,
		.maxlen		= sizeof(unsigned int),
		.mode		= 0444,
	...
	{
		.procname	= "topapp_boost",
		.data		= &topapp_boost_tbl,
		.maxlen		= 128,
		.mode		= 0644,
		.proc_handler	= cfg_topapp_boost,
	},
	{
	...
	{
		.procname	= "prefer_preempt2pri",
		.data		= (int *)PREFER_PREEMPT2PRI,
		.maxlen		= sizeof(int),
		.mode		= 0644,
		.proc_handler	= cfg_task_sched_prop_handler,
	},
	{
		.procname	= "prefer_cpu2pri",
		.data		= (int *)PREFER_CPU2PRI,
		.maxlen		= sizeof(int),
		.mode		= 0644,
		.proc_handler	= cfg_task_sched_prop_handler,
	},
