...
{
	static u64 cached_val[MAX_NR_CLUSTER] = {0};
	...
	if (!hg_policy) {
		for_each_possible_cpu(cpu) {
			...
			hmbird_II_output_systrace("C|9999|clus[%d]_soft_limit|%llu\n", cluster_id, val);
		}
	} else {
		struct cpufreq_policy *policy = hg_policy->policy;
		u64 max_unit_k = hg_policy->tunables->soft_freq_max / 1000;
		u64 min_unit_k = hg_policy->tunables->soft_freq_min / 1000;
		u64 max_unit_kern = hg_policy->tunables->soft_freq_max_kern > -1 ? hg_policy->tunables->soft_freq_max_kern / 1000 : 0;
		u64 min_unit_kern = hg_policy->tunables->soft_freq_min_kern > -1 ? hg_policy->tunables->soft_freq_min_kern / 1000 : 0;
		cluster_id = topology_cluster_id(policy->cpu);
		val = max_unit_kern * 1000000000000 + min_unit_kern * 100000000 + max_unit_k * 10000 + min_unit_k;
		if (cluster_id >=  MAX_NR_CLUSTER || cluster_id < 0)
			return;
...
{
	struct cpufreq_policy *policy = hg_policy->policy;
	int soft_freq_max = hg_policy->tunables->soft_freq_max_kern > -1 ? hg_policy->tunables->soft_freq_max_kern : hg_policy->tunables->soft_freq_max;
	int soft_freq_min = hg_policy->tunables->soft_freq_min_kern > -1 ? hg_policy->tunables->soft_freq_min_kern : hg_policy->tunables->soft_freq_min;
...
static int hmbird_gov_init(struct cpufreq_policy *policy)
{
	...
	tunables->soft_freq_max_kern = -1;
	tunables->soft_freq_min_kern = -1;
