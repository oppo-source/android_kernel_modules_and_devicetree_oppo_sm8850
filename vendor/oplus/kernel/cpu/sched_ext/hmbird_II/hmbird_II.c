...
static void (*addr_scx_task_iter_start)(struct scx_task_iter *iter);
static struct task_struct *(*addr_scx_task_iter_next_locked)(struct scx_task_iter *iter);
static void (*addr_scx_task_iter_unlock)(struct scx_task_iter *iter);
static bool (*addr_dequeue_task)(struct rq *rq, struct task_struct *p, int flags);
static void (*addr_sched_deq_and_put_task)(struct task_struct *p, int queue_flags, struct sched_enq_and_set_ctx *ctx);
static void (*addr_sched_enq_and_set_task)(struct sched_enq_and_set_ctx *ctx);
static void (*addr_check_class_changing)(struct rq *rq, struct task_struct *p, const struct sched_class *prev_class);
static void (*addr_check_class_changed)(struct rq *rq, struct task_struct *p, const struct sched_class *prev_class, int oldprio);
static struct mutex *addr_scx_ops_enable_mutex;

LOOKUP_KERNEL_SYMBOL(scx_task_iter_start);
LOOKUP_KERNEL_SYMBOL(scx_task_iter_next_locked);
LOOKUP_KERNEL_SYMBOL(scx_task_iter_unlock);
LOOKUP_KERNEL_SYMBOL(dequeue_task);
LOOKUP_KERNEL_SYMBOL(sched_deq_and_put_task);
LOOKUP_KERNEL_SYMBOL(sched_enq_and_set_task);
LOOKUP_KERNEL_SYMBOL(check_class_changing);
LOOKUP_KERNEL_SYMBOL(check_class_changed);

static __nocfi void scx_task_iter_stop(struct scx_task_iter *iter)
{
	list_del_init(&iter->cursor.tasks_node);
	addr_scx_task_iter_unlock(iter);
}
static bool pre_switch_check = true;
...
int hmbird_prepare_and_check(void)
{
	...
	lookup_scx_task_iter_start();
	lookup_scx_task_iter_next_locked();
	lookup_scx_task_iter_unlock();
	lookup_dequeue_task();
	lookup_sched_deq_and_put_task();
	lookup_sched_enq_and_set_task();
	lookup_check_class_changing();
	lookup_check_class_changed();
	addr_scx_ops_enable_mutex = (struct mutex *)addr_kallsyms_lookup_name("scx_ops_enable_mutex");
	if (!addr_scx_task_iter_start || !addr_scx_task_iter_next_locked
		|| !addr_scx_task_iter_unlock || !addr_dequeue_task
		|| !addr_sched_deq_and_put_task || !addr_sched_enq_and_set_task
		|| !addr_check_class_changing || !addr_check_class_changed
		|| !addr_scx_ops_enable_mutex)
		pre_switch_check = false;
	return 0;
}
...
void __nocfi hmbird_change_scx_to_rt(void)
{
	struct scx_task_iter sti;
	struct task_struct *p;
	const struct sched_class *new_class, *old_class;
	struct sched_enq_and_set_ctx ctx;
	if (!pre_switch_check)
		return;
	addr_scx_task_iter_start(&sti);
	while ((p = addr_scx_task_iter_next_locked(&sti))) {
		old_class = p->sched_class;
		if (!rt_prio(p->prio) || old_class != &ext_sched_class)
			continue;
		new_class = addr_rt_sched_class;
		if (p->se.sched_delayed)
			addr_dequeue_task(task_rq(p), p, DEQUEUE_SLEEP | DEQUEUE_DELAYED);
		addr_sched_deq_and_put_task(p, DEQUEUE_SAVE | DEQUEUE_MOVE, &ctx);
		p->sched_class = new_class;
		addr_check_class_changing(task_rq(p), p, old_class);
		addr_sched_enq_and_set_task(&ctx);
		addr_check_class_changed(task_rq(p), p, old_class, p->prio);
	}
	scx_task_iter_stop(&sti);
}

static void scx_ops_enable_mutex_while_disabling(void *unused,
			struct mutex *mutex, unsigned long settime)
{
	if (likely(mutex != addr_scx_ops_enable_mutex))
		return;
	if (!settime)
		return;
	if (current != sched_ext_helper)
		return;
	hmbird_change_scx_to_rt();
}
...
int hmbird_II_init(void)
{
	...
	register_trace_android_vh_record_mutex_lock_starttime(scx_ops_enable_mutex_while_disabling, NULL);
#endif
	return 0;
}
