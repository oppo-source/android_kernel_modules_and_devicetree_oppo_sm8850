...
/* params_backup storage and operations */
#define PARAM_BACKUP_BUF_SIZE (2 * PAGE_SIZE)
static char g_params_backup_buf[PARAM_BACKUP_BUF_SIZE] = {0};
static DEFINE_MUTEX(params_backup_mutex);

static ssize_t params_backup_read(struct file *file, char __user *ubuf, size_t count, loff_t *ppos)
{
	size_t len;

	mutex_lock(&params_backup_mutex);
	len = strnlen(g_params_backup_buf, sizeof(g_params_backup_buf));
	if (*ppos >= len) {
		hmbird_pr_debug("params_backup: read position %lld >= length %zu\n", *ppos, len);
		mutex_unlock(&params_backup_mutex);
		return 0;
	}
	...
	if (copy_to_user(ubuf, g_params_backup_buf + *ppos, count)) {
		mutex_unlock(&params_backup_mutex);
		return -EFAULT;
	}
...
static ssize_t params_backup_write(struct file *file, const char __user *ubuf, size_t count, loff_t *ppos)
{
	mutex_lock(&params_backup_mutex);

	/* Handle empty write (clear operation) */
	if (count == 0) {
		memset(g_params_backup_buf, 0, sizeof(g_params_backup_buf));
		hmbird_pr_debug("params_backup: cleared buffer\n");
		mutex_unlock(&params_backup_mutex);
		return 0;
	}

	if (count >= sizeof(g_params_backup_buf)) {
		hmbird_pr_warn("params_backup: input too long, max %zu bytes\n",
			       sizeof(g_params_backup_buf) - 1);
		mutex_unlock(&params_backup_mutex);
		return -EINVAL;
	}

	if (copy_from_user(g_params_backup_buf, ubuf, count)) {
		mutex_unlock(&params_backup_mutex);
		return -EFAULT;
	}

	/* Ensure null termination */
	g_params_backup_buf[count] = '\0';

	/* Remove trailing newline if present */
	if (count > 0 && (g_params_backup_buf[count - 1] == '\n' || g_params_backup_buf[count - 1] == '\r')) {
		g_params_backup_buf[count - 1] = '\0';
		count--;
	}

	hmbird_pr_debug("params_backup: wrote %zu bytes: %s\n", count, g_params_backup_buf);

	mutex_unlock(&params_backup_mutex);
	return count;
}

static const struct proc_ops params_backup_fops = {
	.proc_read  = params_backup_read,
	.proc_write = params_backup_write,
};
...
static int create_stats_procs(void) {
	...
	p = proc_create(PARAMS_BACKUP_PROC, S_IRUGO | S_IWUGO,
			d_oplus_hmbird, &params_backup_fops);
	if (p == NULL) {
		hmbird_pr_debug("failed to proc_create %s\n", PARAMS_BACKUP_PROC);
		goto err_params_backup;
	}
...
{
	hmbird_pr_debug("doing remove_proc_entry %s\n", PARAMS_BACKUP_PROC);
	remove_proc_entry(PARAMS_BACKUP_PROC, d_oplus_hmbird);
