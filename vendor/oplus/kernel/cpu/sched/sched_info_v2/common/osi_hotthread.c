...
extern u64 high_load_switch;
extern s32 g_over_load;

struct hot_thread_struct  hot_thread_top[JANK_WIN_CNT][TOP_THREAD_CNT];
static u32 __read_mostly cpu_nums;
...
s32 find_in_plist(struct task_struct *p,  struct oplus_task_struct *ots)
{
...
/*
*  This Function run in account_process_tick, not sched_tick,
*  so There is no concurrent issue here
*/
static s32 insert_hot_thread(struct oplus_task_struct *ots, struct task_struct *p, u32 now_idx)
{
...
static void notify_rqlen_fn(struct work_struct *work)
{
	...
	rq_len = kmalloc_array(cpu_nums, sizeof(s32), GFP_KERNEL);
	if (!rq_len)
		return;
	memset(rq_len, 0, cpu_nums * sizeof(s32));

	for_each_possible_cpu(cpu) {
		rq_len[cpu] = per_cpu(percpu_rq_num, cpu).avg_num;
	}
	send_to_user(NOTIFY_CPU_RQ, cpu_nums, rq_len);
	kfree(rq_len);
}

static inline void count_rq_num(s32 cpu)
{
...
void jank_hotthread_update_tick(struct task_struct *p, u64 now)
{
	...
	u64 timestamp_prewin;
	u32 now_idx;
	u32 cpu, cluster_id;
	static s32 cal_rq_cnt;

	if (!g_over_load || unlikely(!p))
		return;
...
static s32  top_hotthread_dump_win(struct seq_file *m, void *v, u32 win_cnt)
{
	u32 i;
...
static s32 proc_top_hotthread_show(struct seq_file *m, void *v)
{
	return top_hotthread_dump_win(m, v, JANK_WIN_CNT/2);
}
static s32 proc_top_hotthread_open(struct inode *inode,
		struct file *file)
{
