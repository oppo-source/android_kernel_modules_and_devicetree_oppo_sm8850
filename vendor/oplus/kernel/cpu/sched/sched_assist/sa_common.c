...
#include "../../vendor/oplus/kernel/cpu/sched_ext/hmbird_II/hmbird_II_export.h"
#endif
...
unsigned int sum_num_ux_in_cpu(int cpu)
{
	struct oplus_rq *orq = get_oplus_rq(cpu_rq(cpu));

	return orq->nr_running;
}
EXPORT_SYMBOL_GPL(sum_num_ux_in_cpu);

inline unsigned long oplus_capacity_orig_of(int cpu)
{
	...
	return orq->cpu_capacity_orig;
#else
	return arch_scale_cpu_capacity(cpu);
#endif
}
EXPORT_SYMBOL_GPL(oplus_capacity_orig_of);

void oplus_update_capacity_orig(int cpu, unsigned long capacity)
{
	struct oplus_rq *orq = NULL;
	struct rq *rq = cpu_rq(cpu);
	...
	orq->cpu_capacity_orig = capacity;
}
EXPORT_SYMBOL_GPL(oplus_update_capacity_orig);
...
set:
	...
	*/
...
#ifdef CONFIG_HMBIRD_SCHED_BPF
	is_task_on_scx = task_on_scx(t);
#endif

	/* if ux is disabled, task only removed from list, not insert to list. */
	if (unlikely(!global_sched_assist_enabled) || is_task_on_scx) {
		...
		if (is_task_on_scx)
			hmbird_update_ux_state_locked(t, ux_state);
		goto out;
	}
	...
	if (!(ux_state & SCHED_ASSIST_UX_MASK)) {
		...
		ots->ux_priority = -1;
		ots->ux_nice = -1;
	} else if (task_on_rq_queued(t) && se->on_rq && (se->sched_delayed == 0)) {
		int prio, nice;
...
EXPORT_SYMBOL_GPL(ux_state_systrace_c);
...
void sched_assist_init_oplus_rq(void)
{
	...
	for_each_possible_cpu(cpu) {
		...
		orq->cpu_capacity_orig = arch_scale_cpu_capacity(cpu_of(rq));
...
static void enqueue_ux_thread(struct rq *rq, struct task_struct *p, int flags)
{
	struct oplus_rq *orq;
	struct oplus_task_struct *ots;
	unsigned long irqflag;
...
static void dequeue_ux_thread(struct rq *rq, struct task_struct *p, int flags)
{
	struct oplus_rq *orq;
	struct oplus_task_struct *ots;
	unsigned long irqflag;
...
void queue_ux_thread(struct rq *rq, struct task_struct *p, int enqueue, int flags)
{
	if (enqueue)
		enqueue_ux_thread(rq, p, flags);
	else
		dequeue_ux_thread(rq, p, flags);
}
...
void android_rvh_find_lowest_rq_handler(void *unused,
			struct task_struct *p, struct task_struct *exec_task, struct cpumask *local_cpu_mask, int ret, int *best_cpu)
#else
void android_rvh_find_lowest_rq_handler(void *unused,
...
void android_rvh_after_enqueue_task_handler(void *unused, struct rq *rq, struct task_struct *p, int flags)
{
	...
	if (test_bit(OTS_STATE_DDL_ENQUEUE_DELAYED, &ots->state))
		clear_bit(OTS_STATE_DDL_ENQUEUE_DELAYED, &ots->state);
#endif
}
