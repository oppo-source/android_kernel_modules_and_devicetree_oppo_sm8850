...
void update_ux_timeline_task_removal(struct oplus_rq *orq, struct oplus_task_struct *ots, __maybe_unused struct sched_entity *se, __maybe_unused bool is_curr)
{
	...
	/* skipped task is removed, reset skip_ots */
	if (orq->skip_ots == ots) {
		orq->skip_ots = NULL;
	}
...
int preempt_compare(struct oplus_task_struct *curr, struct oplus_task_struct *ots, u64 gran)
{
	...
	gran_vtime = calc_delta_fair(gran, ots->ux_priority);
...
void android_vh_sched_stat_runtime_handler(void *unused, struct task_struct *task, u64 delta_exec)
{
	struct rq *rq;
	struct oplus_rq *orq;
	struct oplus_task_struct *ots;
