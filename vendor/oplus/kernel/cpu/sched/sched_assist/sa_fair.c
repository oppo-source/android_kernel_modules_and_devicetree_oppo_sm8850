...
static inline int get_cls_for_silver_perf(struct task_struct *task, int cls_max)
{
	if (!global_silver_perf_core)
		return -1;

	return strict_ux_task(task) ? cls_max : 0;
}
...
inline int get_task_cls_for_scene(struct task_struct *task)
{
	...
	/* For the all-big-core architecture */
	cls_id = get_cls_for_silver_perf(task, cls_max);
	if (cls_id >= 0)
		return cls_id;
...
bool set_ux_task_to_prefer_cpu(struct task_struct *task, int *orig_target_cpu)
{
	...
	long vip_max_spare_cap = -1, max_spare_cap = -1;
	int ux_state = 0, subopt_ux_priority = 0, ux_priority = 0;
	...
	subopt_ux_priority = ux_state_to_priority(oplus_get_ux_state(task));
...
retry:
	...
	for_each_cpu(cpu, &search_cpus) {
		...
		if (walk_next_cls && (spare_cap > subopt_max_spare_cap)) {
		...
		ux_state = oplus_get_ux_state(rq->curr);
		if (ux_state & SCHED_ASSIST_UX_MASK) {
			ux_priority = ux_state_to_priority(ux_state);
			if (ux_priority < subopt_ux_priority) {
				subopt_ux_priority = ux_priority;
				subopt_cpu = cpu;
			}
			continue;
		}
	...
	/* 4 No cpu select, Preempt UX threads, Priority */
	if (subopt_cpu != -1) {
