 ...
 */
static void schedtune_enqueue_task(struct task_struct *p, int cpu)
{
	struct boost_groups *bg = &per_cpu(cpu_boost_groups, cpu);
 ...
 * NOTE: This function must be called while holding the lock on the CPU RQ
 */
static void schedtune_dequeue_task(struct task_struct *p, int cpu)
{
	struct boost_groups *bg = &per_cpu(cpu_boost_groups, cpu);
	unsigned long irq_flags;
	int idx;
...
void android_vh_cpu_cgroup_css_alloc_early_handler(void *unused, struct task_group *parent)
{
...
void android_vh_sched_move_task_handler(void *unused, struct task_struct *task)
{
	struct rq *rq;
...
void android_rvh_after_dequeue_task_handler(void *unused, struct rq *rq, struct task_struct *p, int flags)
{
	schedtune_dequeue_task(p, cpu_of(rq));
}

int register_schedtune_vendor_hooks(void)
{
	int ret = 0;

	/* register vendor hook in kernel/sched/core.c*/
	ret |= register_trace_android_vh_cpu_cgroup_css_alloc_early(android_vh_cpu_cgroup_css_alloc_early_handler, NULL);
	ret |= register_trace_android_vh_cpu_cgroup_css_alloc(android_vh_cpu_cgroup_css_alloc_handler, NULL);
	ret |= register_trace_android_vh_cpu_cgroup_css_free(android_vh_android_vh_cpu_cgroup_css_free_handler, NULL);
	ret |= register_trace_android_vh_sched_move_task(android_vh_sched_move_task_handler, NULL);
	ret |= register_trace_android_rvh_after_enqueue_task(android_rvh_after_enqueue_task_handler, NULL);
	ret |= register_trace_android_rvh_after_dequeue_task(android_rvh_after_dequeue_task_handler, NULL);
	if (ret) {
		pr_err("schedstune register vendor hooks failed!\n");
		return ret;
	}
...
static void unregister_schedtune_vendor_hooks(void)
{
	unregister_trace_android_vh_cpu_cgroup_css_alloc_early(android_vh_cpu_cgroup_css_alloc_early_handler, NULL);
	unregister_trace_android_vh_cpu_cgroup_css_alloc(android_vh_cpu_cgroup_css_alloc_handler, NULL);
	unregister_trace_android_vh_cpu_cgroup_css_free(android_vh_android_vh_cpu_cgroup_css_free_handler, NULL);
	unregister_trace_android_vh_sched_move_task(android_vh_sched_move_task_handler, NULL);
}
...
static inline void schedtune_init_cgroups(void)
{
	...
	ret = cgroup_add_legacy_cftypes(&cpu_cgrp_subsys, schedtune_legacy_files);
	if (ret < 0) {
		printk("add schedtune file fail");
		return ;
	}
	...
	pr_info("schedtune: configured to support %d boost groups",
		BOOSTGROUPS_COUNT);
	register_schedtune_vendor_hooks();
 ...
 */
static int __init schedtune_init(void)
{
...
static void __exit schedtune_exit(void)
{
	unregister_schedtune_vendor_hooks();
}
module_init(schedtune_init);
module_exit(schedtune_exit);
MODULE_DESCRIPTION("Oplus schedtune Vender Hooks Driver");
