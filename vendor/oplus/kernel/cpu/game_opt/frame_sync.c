...
struct gameopt_frame_data pre_data;
static DEFINE_SPINLOCK(pre_data_lock);
...
static inline long common_frame_notify(const struct gameopt_frame_data *data)
{
    long ret;
...
static inline long handle_ioctl_produce(struct gameopt_frame_data *data)
{
    data->mode = NOTIFY_FRAME_PRODUCE;
    produce_data = *data;
    read_pointer = &produce_data;
    return common_frame_notify(data);
}

static inline long handle_ioctl_consume(struct gameopt_frame_data *data)
{
    data->mode = NOTIFY_FRAME_CONSUME;
    consume_data = *data;
    read_pointer = &consume_data;
    return notify_wait_fd();
}

static inline long handle_ioctl_tlpred(struct gameopt_frame_data *data)
{
    tl_pred_data = *data;
...
static long sync_ctrl_ioctl(struct file* file, unsigned int cmd, unsigned long arg) {
    ...
    if (copy_from_user(&data, uarg, sizeof(data)))
        return -EFAULT;
    if (atomic_read(&gpu_ctrl_per_frame_enable)) {
        struct gameopt_frame_data pre_copy;
        spin_lock(&pre_data_lock);
        pre_copy = pre_data;
        spin_unlock(&pre_data_lock);
        gpu_ctrl_notify_frame_data(&data, &pre_copy);
    }
    spin_lock(&pre_data_lock);
    pre_data = data;
    spin_unlock(&pre_data_lock);
    ...
    switch (cmd) {
        case CMD_ID_GAMEOPT_EPOLL_PRODUCE:
            return handle_ioctl_produce(&data);
        case CMD_ID_GAMEOPT_EPOLL_CONSUME:
            return handle_ioctl_consume(&data);
        case CMD_ID_GAMEOPT_EPOLL_TLPRED:
            return handle_ioctl_tlpred(&data);
        default:
            return reset_wait_event_cnt();
    }
}
