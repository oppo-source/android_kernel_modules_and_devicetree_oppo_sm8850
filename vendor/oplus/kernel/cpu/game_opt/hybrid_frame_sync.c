...
static long frame_hybrid_write(unsigned long arg)
{
	void __user *uarg = (void __user *)arg;
	struct hybrid_frame_data *data = buffer_push_alloc();

	if (data && !copy_from_user(data, uarg, sizeof(struct hybrid_frame_data))) {
		frame_sync_handler(data);
		log_format_hfd(data);
		return wake_up_hybrid_frame_poll();
	} else {
		buffer_pop_back();
		return -EFAULT;
	}
...
static long frame_hybrid_read(unsigned long arg)
{
	void __user *uarg = (void __user *)arg;
	struct hybrid_frame_data *data = buffer_front();
	long ret = 0;

	if (data) {
		data->end = buffer_size() <= 1; /* the last buffer */
		log_format_hfd(data);
		if (!copy_to_user(uarg, data, sizeof(struct hybrid_frame_data))) {
			buffer_pop();
		} else {
			ret = -EFAULT;
		}
	...
	struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case CMD_ID_FRAME_HYBRID_WR:
		mutex_lock(&hfs_mutex);
		ret = frame_hybrid_write(arg);
		mutex_unlock(&hfs_mutex);
		break;

	case CMD_ID_FRAME_HYBRID_RD:
		mutex_lock(&hfs_mutex);
		ret = frame_hybrid_read(arg);
		mutex_unlock(&hfs_mutex);
		break;
