...
static ssize_t yield_opt_proc_write(struct file *file, const char __user *buf, size_t count, loff_t *ppos)
{
	...
	if ((enable_tmp != 0 && enable_tmp != 1 && enable_tmp != 2) ||
...
{
	if (!yield_opt_params.enable || current->pid != game_pid_yield || *skip)
		return;
	...
	if (yield_opt_params.enable == 2) {
		/*simple yield opt mode*/
		*skip = 1;
		sleep_now = div64_u64(MIN_YIELD_SLEEP, 1000);
...
static void init_yield_state(void)
{
	int cpu;
	struct sched_yield_state *ys;
