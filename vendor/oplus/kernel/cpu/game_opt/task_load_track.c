...
DEFINE_PER_CPU(u32, g_rq_tick_cnt);
	...
	bool running)
{
	...
	update_inst = read_sysreg_s(SYS_AMEVCNTR0_INST_RET_EL0);
	update_cycle = read_sysreg_s(SYS_AMEVCNTR0_CORE_EL0);

	if (running) {
		ipc->curr_inst_cnt += update_inst - ipc->last_update_inst;
		ipc->curr_cycle_cnt += update_cycle - ipc->last_update_cycle;
		ipc->curr_runtime_sum += now - ipc->last_update_time;
		if (ipc->curr_runtime_sum > (NSEC_PER_MSEC << 6)) {
			task_inst_avg = mul_u64_u64_div_u64(
				ipc->curr_inst_cnt, (1 << 10), ipc->curr_runtime_sum);
			task_ipc_avg = mul_u64_u64_div_u64(
				ipc->curr_inst_cnt, (1 << 10), ipc->curr_cycle_cnt);
			if (likely(ipc->task_inst_avg > 0)) {
				ipc->task_inst_avg = (ipc->task_inst_avg >> 1) + (task_inst_avg >> 1);
			} else {
				ipc->task_inst_avg = task_inst_avg;
			}
			if (likely(ipc->task_ipc_avg > 0)) {
				ipc->task_ipc_avg = (ipc->task_ipc_avg >> 1) + (task_ipc_avg >> 1);
			} else {
				ipc->task_ipc_avg = task_ipc_avg;
			}
			ipc->curr_inst_cnt = 0;
			ipc->curr_cycle_cnt = 0;
			ipc->curr_runtime_sum = 0;
			debug_trace_pr_val_str("inst", ipc->task_inst_avg);
			debug_trace_pr_val_str("ipc", ipc->task_ipc_avg);
		}
	} else {
		ipc->last_update_inst = update_inst;
		ipc->last_update_cycle = update_cycle;
...
void tlt_read_task_load(struct tlt_info *info)
{
	...
	struct game_task_struct *gts = NULL;
	struct freq_stat *stat;
	int size = min(info->size, TLT_INFO_PAGE_SIZE / 3);
...
void tlt_read_task_ipc_avg(struct tlt_info *info)
{
	struct task_struct *ts = NULL;
	struct game_task_struct *gts = NULL;
	int size = min(info->size, TLT_INFO_PAGE_SIZE / 2);

	for (int i = size - 1; i >= 0; i--) {
		debug_trace_pr_val_str("read_inst", info->data[i]);
		scoped_guard(rcu) {
			ts = find_task_by_vpid(info->data[i]);
			if (ts_to_gts(ts, &gts) && gts->demand.tracking) {
				info->data[i * 2] = gts->ipc.task_inst_avg;
				info->data[i * 2 + 1] = gts->ipc.task_ipc_avg;
			} else {
				info->data[i * 2] = 0;
				info->data[i * 2 + 1] = 0;
			}
		...
		struct task_struct *prev, struct task_struct *next, unsigned int prev_state)
{
	...
	if (ts_to_gts(prev, &pgts) && (pgts->demand.tracking || pgts->demand.handoff)) {
		...
		update_task_inst_avg(prev, pgts, now, true);
...
pts_unlock:
	...
	if (ts_to_gts(next, &ngts) && ngts->demand.tracking) {
		...
		update_task_inst_avg(next, ngts, now, false);
...
static void scheduler_tick_handler(void *unused, struct rq *rq)
{
	int cpu = cpu_of(rq);
	u64 now;
	u32 *tick_cnt = per_cpu_ptr(&g_rq_tick_cnt, cpu);
	struct task_struct *ts = rq->curr;
	struct game_task_struct *gts = NULL;
	...
	if (ts_to_gts(ts, &gts) && gts->demand.tracking) {
		now = sched_clock();
		update_task_inst_avg(ts, gts, now, true);
	}
}
...
void register_tlt_hooks(void)
{
	...
	register_trace_android_vh_scheduler_tick(scheduler_tick_handler, NULL);
}
...
void unregister_tlt_hooks(void)
{
	...
	unregister_trace_android_vh_scheduler_tick(scheduler_tick_handler, NULL);
}

/************************** proc ops ************************/

static int tlt_ioctl_cmd(unsigned int cmd, struct tlt_info *data, bool *to_user)
{
	switch (cmd) {
	...
	case CMD_ID_TLT_READ_TASK_INST_AVG:
		tlt_read_task_ipc_avg(data);
		*to_user = true;
		break;
	...
	struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	ret = tlt_ioctl_cmd(cmd, &data, &to_user);
