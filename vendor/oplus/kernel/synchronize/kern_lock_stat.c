...
static unsigned long register_time;
...
static __read_mostly u32 thres_array[LIMIT_GRP_TYPES][LOCK_TYPES][MAX_THRES]  = {
			...
			{0, 0, 0},
#endif
			{100 * NSEC_PER_MSEC, 150 * NSEC_PER_MSEC, 200 * NSEC_PER_MSEC},
			},
			/* RT */
			{
#ifdef INCLUDE_UNUSE
			...
			{0, 0, 0},
#endif
			{200 * NSEC_PER_MSEC, 350 * NSEC_PER_MSEC, 500 * NSEC_PER_MSEC},
			}
};
...
static ssize_t locking_debug_write(struct file *file, const char __user *user_buffer,
                                 size_t count, loff_t *offset)
{
	...
	if (count >= sizeof(buf)) {
		pr_err("[kern_lock_debug]:Invailid argument \n");
		return -EINVAL;
	}

	if (copy_from_user(buf, user_buffer, count)) {
		pr_err("[kern_lock_debug]:Copy from user failed\n");
		return -EFAULT;
	}
	buf[count] = '\0';

	if (kstrtouint(buf, 0, &val)) {
		pr_err("[kern_lock_debug]:kstrtouint failed\n");
		return -EINVAL;
	}

	WRITE_ONCE(g_opt_debug, val);
	return count;
}

static int locking_debug_show(struct seq_file *m, void *v)
{
	seq_printf(m, "%u\n", g_opt_debug);
	return 0;
}

static int locking_debug_open(struct inode *inode, struct file *file)
{
	return single_open(file, locking_debug_show, inode);
}

static const struct proc_ops locking_debug_fops = {
	.proc_open = locking_debug_open,
	.proc_read = seq_read,
	.proc_write = locking_debug_write,
	.proc_lseek = seq_lseek,
	.proc_release = single_release,
};
...
static int create_stats_procs(void)
{
	...
	p = proc_create("locking_debug", S_IRUGO | S_IWUGO,
			d_oplus_locking, &locking_debug_fops);
	if (NULL == p)
		goto err5;
...
err5:
	remove_proc_entry("locking_debug", d_oplus_locking);
err4:
...
static __always_inline void lk_contended(int type)
{
	...
	if (unlikely(!locking_opt_enable(LK_MONITOR_ENABLE)))
		return;
	...
	if (ots->lkinfo.waittime_stamp) {
		lk_info("kern_lock_stat : error start,times_stamp not 0,type = %d !\n", type);
		return;
	}
...
static __always_inline void lk_acquired(int type)
{
	...
	if (unlikely(!locking_opt_enable(LK_MONITOR_ENABLE))) {
		if (unlikely(ots->lkinfo.waittime_stamp))
	...
	if (ots->lkinfo.waittime_stamp) {
		...
		if(register_time < jiffies - 10*HZ)
			lk_info("kern_lock_stat :  error end,"
		  ...
		  u32 bitset)
{
	...
	/* need add more trace_android_vh_futex_wait_end() hook in futex_wait() */
