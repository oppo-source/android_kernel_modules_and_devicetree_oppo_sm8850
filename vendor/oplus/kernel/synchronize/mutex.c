...
static void mutex_set_inherit_ux(struct mutex *lock, struct task_struct *task)
{
	...
	inherit_val = calc_inherit_val(is_ux, is_rt, task);
	if (!inherit_val)
		return;

	owner = __mutex_owner(lock);

	if (!test_inherit_ux(owner, INHERIT_UX_MUTEX) || !test_ux_is_same(current, owner))
		set_inherit_ux(owner, INHERIT_UX_MUTEX, oplus_get_ux_depth(task), inherit_val);
}
