...
void locking_update_tick_hit(struct task_struct *prev, struct task_struct *next)
{
	struct oplus_task_struct *ots;
...
static void check_consecutive_tick_hits(struct rq *rq)
{
	...
	if ((rt_prio(curr_task->prio) || curr_task == rq->idle))
		return;
...
{
	check_consecutive_tick_hits(rq);
}
...
static int register_dstate_opt_vendor_hooks(void)
{
	...
	REGISTER_TRACE(android_vh_scheduler_tick, android_vh_scheduler_tick_handler, NULL, out6);
...
struct sched_assist_locking_ops sa_ops = {
	...
	.locking_tick_hit = locking_update_tick_hit,
};
