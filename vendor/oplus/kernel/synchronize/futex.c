...
static int calc_inherit_val(bool is_rt, int inherit_val, int waittype)
{
	if (waittype & LOCK_GC_COND)
		return SA_TYPE_HEAVY;
...
static int futex_set_inherit_ux_refs(struct task_struct *holder, struct task_struct *p, int waittype)
{
	bool set_ux = false, is_rt = false;
	int inherit_val, h_inherit_val;
	struct oplus_task_struct *ots, *h_ots;

	if (unlikely(!holder || !p))
		return 0;

	ots = get_oplus_task_struct(p);
	h_ots = get_oplus_task_struct(holder);

	if (unlikely(IS_ERR_OR_NULL(ots) || IS_ERR_OR_NULL(h_ots)))
		return 0;

	inherit_val = ots->ux_state & SCHED_ASSIST_UX_MASK;
	h_inherit_val = h_ots->ux_state & SCHED_ASSIST_UX_MASK;
	is_rt = test_task_is_rt(p);
	set_ux = test_set_inherit_ux(p);

	if (is_rt || (set_ux && inherit_val)) {
		int type = get_ux_state_type(holder);
		inherit_val = calc_inherit_val(is_rt, inherit_val, waittype);
		...
		if (type == UX_STATE_NONE || type == UX_STATE_STATIC) {
			...
			set_inherit_ux(holder, INHERIT_UX_FUTEX, ots->ux_depth, inherit_val);
			...
			lock_ux_state_systrace(current, p, holder,
				STATE_FUTEX_SET_UX, LOG_LOCK_SYSTRACE_LVL0);
			return INHERIT_SET;
		} else if (type == UX_STATE_INHERIT || type == UX_STATE_COMBINED) {
			if (holder->__state & TASK_NORMAL)
				atomic_long_inc((atomic_long_t*)&futex_set_blocked_ux_cnt);

			if ((inherit_val != h_inherit_val) && (inherit_val != 0))
				set_inherit_ux(holder, INHERIT_UX_FUTEX, ots->ux_depth, inherit_val|h_inherit_val);
			else
			...
			cond_trace_printk(locking_opt_debug(LK_DEBUG_FTRACE),
				...
				p->comm, p->pid, p->tgid);
			lock_ux_state_systrace(current, p, holder,
				STATE_FUTEX_INC_INHERIT_UX, LOG_LOCK_SYSTRACE_LVL0);
			return INHERIT_INC;
		} else {
			lock_ux_state_systrace(current, p, holder,
				STATE_FUTEX_NOT_SET_UX, LOG_LOCK_SYSTRACE_LVL0);
		}
	} else {
		lock_ux_state_systrace(current, p, holder,
			STATE_FUTEX_DO_NOTHING, LOG_LOCK_SYSTRACE_LVL0);
	}
...
static bool boost_holder(struct task_struct *holder, struct task_struct *waiter, int waittype)
{
	 ...
	 */
	if (atomic64_read(&(ots->lkinfo.ux_contrib))) {
		lock_ux_state_systrace(current, waiter, holder,
			STATE_FUTEX_UX_CONTRIB, LOG_LOCK_SYSTRACE_LVL0);
		cond_trace_printk(locking_opt_debug(LK_DEBUG_FTRACE),
	 ...
	 */
	if (futex_set_inherit_ux_refs(holder, waiter, waittype)) {
		atomic64_set(&(ots->lkinfo.ux_contrib), 1);
...
static bool set_holder(struct task_struct *waiter, struct task_struct *holder, int waittype)
{
	struct oplus_task_struct *ots;
	...
	if (waiter->tgid == holder->tgid) {
		...
		if (likely(!IS_ERR_OR_NULL(ots))) {
			...
			if (waittype != 0) {
				ots->lkinfo.waittype = waittype;
			}
			lock_ux_state_systrace(current, waiter, holder,
				STATE_FUTEX_SET_HOLDER, LOG_LOCK_SYSTRACE_LVL1);
			return true;
		}
	} else {
		lock_ux_state_systrace(current, waiter, holder,
			STATE_FUTEX_NOT_SET_HOLDER_OF_TGID, LOG_LOCK_SYSTRACE_LVL0);
	}
...
static void futex_notify_waiter(unsigned long pid)
{
	...
	if (!waiter) {
		lock_ux_state_systrace(current, NULL, current,
			STATE_FUTEX_WAITER_UNKNOWN_WHEN_NOTIFY, LOG_LOCK_SYSTRACE_LVL0);
		return;
	}
	is_target = (test_task_ux(waiter) || test_task_is_rt(waiter)) && !curr_is_ux_thread();

	if (!is_target) {
		lock_ux_state_systrace(current, waiter, current,
			STATE_FUTEX_WAITER_NOT_UX_WHEN_NOTIFY, LOG_LOCK_SYSTRACE_LVL0);
		goto out_put_waiter;
	}
	get_task_struct(holder);
	try_to_boost = set_holder(waiter, holder, 0);

	/* TODO: should check waiter's state? it should be sleeping */
	if (try_to_boost)
		boost_holder(holder, waiter, 0);
 ...
 *     bit8 ~ bit23 : Used to pass owner tid.
 *     bit24 ~ bit31 : Used to pass lock type of JUC/ART.
 * wake situation:
 *     N/A.
 **/
#define WAKE_MSG (64)
		...
		if (do_something) {
			...
			if ((info.cmd & MAGIC_MASK) == MAGIC_NUM) {
				...
				if (info.cmd & TYPE_BIT) {
					if (info.type & (LOCK_ART | LOCK_JUC | LOCK_GC_COND)) {
		  ...
		  u32 bitset)
{
	...
	holder = futex_find_task_by_pid((flags & ~(-1 << LOCK_TYPE_SHIFT)) >> FLAGS_OWNER_SHIFT);
	if (!holder)
		return;

	waittype = (flags & (-1 << LOCK_TYPE_SHIFT)) >> LOCK_TYPE_SHIFT;
	set_holder(current, holder, waittype);
}
		  ...
		  u32 bitset)
{
	...
	if (ots->lkinfo.holder) {
		...
		if (unlikely(atomic64_cmpxchg(&(ots->lkinfo.ux_contrib), 1, 0))) {
			cond_trace_printk(locking_opt_debug(LK_DEBUG_FTRACE),
	...
	if (ots->lkinfo.waittype != 0) {
		ots->lkinfo.waittype = 0;
	}
}
		    ...
		    struct plist_head *head, bool *already_on_hb)
{
	...
	if (*already_on_hb) {
		lock_ux_state_systrace(current, NULL, NULL,
			STATE_FUTEX_ALREADY_ON_HB, LOG_LOCK_SYSTRACE_LVL0);
		return;
	}
	if (!curr_is_ux_thread() && !curr_is_rt_thread()) {
		lock_ux_state_systrace(current, NULL, NULL,
			STATE_FUTEX_CURR_NOT_UX_WHEN_LISTADD, LOG_LOCK_SYSTRACE_LVL1);
		return;
	}
	ots = get_oplus_task_struct(current);
	if (unlikely(IS_ERR_OR_NULL(ots)))
		return;
	if (ots->lkinfo.holder) {
		boost_holder(ots->lkinfo.holder, current, ots->lkinfo.waittype);
		lock_ux_state_systrace(current, NULL, NULL,
			STATE_FUTEX_BOOST_HOLDER_WHEN_LISTADD, LOG_LOCK_SYSTRACE_LVL1);
	} else {
		lock_ux_state_systrace(current, NULL, NULL,
			STATE_FUTEX_NO_HOLDER_WHEN_LISTADD, LOG_LOCK_SYSTRACE_LVL1);
	}
	...
	*target_nr = 0;
	...
	plist_for_each_entry_safe(this, next, chain, list) {
		if (match_futex(&this->key, &key)) {
			...
			if ((ots->lkinfo.holder == current) && atomic64_cmpxchg(&(ots->lkinfo.ux_contrib), 1, 0)) {
				*target_nr += 1;
			}
			cond_trace_printk(locking_opt_debug(LK_DEBUG_FTRACE),
		  ...
		  struct task_struct *p)
{
	...
	if (atomic64_cmpxchg(&(ots->lkinfo.ux_contrib), 1, 0) && ots->lkinfo.holder) {
