...
static bool rwsem_list_add_ux(struct list_head *entry, struct list_head *head)
{
	...
	list_for_each_safe(pos, n, head) {
		waiter = (struct rwsem_waiter *)list_entry(pos, struct rwsem_waiter, list);
...
static bool test_ux_is_same(struct task_struct *a, struct task_struct *b)
{
	return (oplus_get_ux_state(a) & SCHED_ASSIST_UX_MASK) == (oplus_get_ux_state(b) & SCHED_ASSIST_UX_MASK);
}

static int calc_inherit_val(bool is_ux, bool is_rt, struct task_struct* task)
{
	if (is_rt)
		return SA_TYPE_LIGHT;

	if (is_ux)
		return oplus_get_ux_state(task);
...
static void rwsem_set_inherit_ux(struct rw_semaphore *sem)
{
	...
	inherit_type = calc_inherit_val(is_ux, is_rt, current);

	if (!inherit_type)
		return;

	/* set writer as ux task */
	if (!test_inherit_ux(owner, INHERIT_UX_RWSEM) || !test_ux_is_same(current, owner)) {
		/*
...
void pre_dump_waitlist(struct rwsem_waiter *curr, struct rw_semaphore *sem)
{
	...
	task = curr->task;
	lk_err("DBG_RWSEM [%s$%d]: [X] [%s] [%s]: task=%s, pid=%d, tgid=%d, prio=%d, timeout=%lu, handoff=%d, empty=%d, count=%lu\n",
		__func__, __LINE__,
	...
	list_for_each_entry_safe(waiter, tmp, &sem->wait_list, list) {
		...
		lk_err("DBG_RWSEM [%s$%d]: [%d] [%s] [%s]: task=%s, pid=%d, tgid=%d, prio=%d, timeout=%lu\n",
...
#ifdef RWSEM_DEBUG
	if (ret) {
		lk_err("DBG_RWSEM [%s$%d]: task=%s, pid=%d, tgid=%d, prio=%d, ux=%d, timeout=%lu(0x%x), t_m=%lu(0x%x), jiffies=%lu(0x%x)\n",
			__func__, __LINE__,
