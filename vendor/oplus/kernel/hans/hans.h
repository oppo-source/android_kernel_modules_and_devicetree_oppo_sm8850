...
#define HANS_TRANS_BINDER_REPLY (-100)
#define HANS_TRANS_BINDER_TRANSACTION (-200)
#define CHECK_KERN_SUPPORT_CGRPV2 (-8000)
#define SKIP_TRANS_BINDER_REPLY (-9000)
#define UNSKIP_TRANS_BINDER_REPLY (-9001)
...
/*global var: check trans binder reply or not*/
#ifndef _HANS_SKIP_TB_REPLY
#define _HANS_SKIP_TB_REPLY
extern int g_skip_trans_binder_reply;
#endif
...
{
	if (cgroup_task_frozen(task) || is_jobctl_frozen(task)) {
		return true;
	}
	/*BUG 7727344: if task->group_leader is NULL, unfreeze it to avoid some unknown problems*/
	if (NULL == task->group_leader) {
		return true;
	} else {
		return is_frozen_state_compatible(task->group_leader) || freezing(task->group_leader);
	}
}
