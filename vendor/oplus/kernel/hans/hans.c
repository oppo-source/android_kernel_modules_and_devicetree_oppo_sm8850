...
int g_skip_trans_binder_reply = 1;
...
void hans_strcpy(char *dst, const char *src,  size_t size)
{
...
int hans_report(enum message_type type, int caller_pid, int caller_uid, int target_pid, int target_uid, const char *rpc_name, int code)
{
        ...
        data.code = code;
        hans_strcpy(data.rpc_name, rpc_name, INTERFACETOKEN_BUFF_SIZE);
		...
		if (nla->nla_type == HANS_ATTR_MSG_GENL) {
			...
			case CPUCTL_TRANS:
				if (CHECK_KERN_SUPPORT_CGRPV2 == data->target_uid) {
					hans_kern_support_cgrpv2();
				} else if (SKIP_TRANS_BINDER_REPLY == data->target_uid) {
					g_skip_trans_binder_reply = 1;
					printk(KERN_ERR "%s: --> FROZEN_TRANS, g_skip_trans_binder_reply = %d\n", __func__, g_skip_trans_binder_reply);
				} else if (UNSKIP_TRANS_BINDER_REPLY == data->target_uid) {
					g_skip_trans_binder_reply = 0;
					printk(KERN_ERR "%s: --> FROZEN_TRANS, g_skip_trans_binder_reply = %d\n", __func__, g_skip_trans_binder_reply);
				} else {
					if (len < sizeof(struct hans_message)) {
