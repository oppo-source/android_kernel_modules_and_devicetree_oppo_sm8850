				...
				list_for_each_entry(w, &thread->todo, entry) {
					...
					if (w != NULL && w->type == BINDER_WORK_TRANSACTION && btrans != NULL) {
						...
						if (btrans->to_thread != NULL && btrans->to_thread == thread && (!(btrans->flags & TF_ONE_WAY))) {
							...
							if (from_uid != to_uid) {
								btrans_type = btrans->need_reply == 1 ? HANS_TRANS_BINDER_TRANSACTION : HANS_TRANS_BINDER_REPLY;
								if (g_skip_trans_binder_reply == 0) {
									binder_inner_proc_unlock(proc);
									hans_report(type, from_pid, from_uid, to_pid, to_uid, "FROZEN_TRANS_THREAD1", btrans_type);
									return;
								} else {
									if (btrans->need_reply == 1) {
										binder_inner_proc_unlock(proc);
										hans_report(type, from_pid, from_uid, to_pid, to_uid, "FROZEN_TRANS_THREAD1", btrans_type);
										return;
									}
								...
								printk(KERN_ERR "HANS binder: FROZEN_TRANS_THREAD1 internal uid %d:%d->%d\n", to_uid, from_pid, to_pid);
							}
							...
							if (from_uid != to_uid) {
								...
								if (g_skip_trans_binder_reply == 0) {
									binder_inner_proc_unlock(proc);
									hans_report(type, from_pid, from_uid, to_pid, to_uid, "FROZEN_TRANS_THREAD2", btrans_type);
									return;
								} else {
									if (btrans->need_reply == 1) {
										binder_inner_proc_unlock(proc);
								...
								printk(KERN_ERR "HANS binder: FROZEN_TRANS_THREAD2 internal uid %d:%d->%d\n", to_uid, from_pid, to_pid);
							}
						} else {
							spin_unlock(&btrans->lock);
						}
						...
						binder_inner_proc_unlock(proc);
						hans_report(type, -1, -1, -1, to_uid, "FROZEN_TRANS_THREAD3", btrans_type);
						return;
					}
				}
			}
			...
			if (btrans) {
				...
				if (btrans->to_thread != NULL
					...
					if (from_uid != to_uid) {
						...
						if (g_skip_trans_binder_reply == 0) {
							binder_inner_proc_unlock(proc);
							hans_report(type, from_pid, from_uid, to_pid, to_uid, "FROZEN_TRANS_STACK1", btrans_type);
							return;
						} else {
							if (btrans->need_reply == 1) {
								binder_inner_proc_unlock(proc);
						...
						printk(KERN_ERR "HANS binder: FROZEN_TRANS_STACK internal uid %d:%d->%d\n", to_uid, from_pid, to_pid);
					}
				} else {
					spin_unlock(&btrans->lock);
				}
			...
			if (w != NULL && w->type == BINDER_WORK_TRANSACTION && btrans != NULL) {
				...
				if (btrans->to_thread != NULL && (!(btrans->flags & TF_ONE_WAY))) {
					...
					if (from_uid != to_uid) {
						...
						if (g_skip_trans_binder_reply == 0) {
							binder_inner_proc_unlock(proc);
							hans_report(type, from_pid, from_uid, to_pid, to_uid, "FROZEN_TRANS_PROC1", btrans_type);
							return;
						} else {
							if (btrans->need_reply == 1) {
								binder_inner_proc_unlock(proc);
					...
					if (from_uid != to_uid) {
						...
						if (g_skip_trans_binder_reply == 0) {
							binder_inner_proc_unlock(proc);
							hans_report(type, from_pid, from_uid, to_pid, to_uid, "FROZEN_TRANS_PROC2", btrans_type);
							return;
						} else {
							if (btrans->need_reply == 1) {
								binder_inner_proc_unlock(proc);
						...
						printk(KERN_ERR "HANS binder: FROZEN_TRANS_PROC2 internal uid %d:%d->%d\n", to_uid, from_pid, to_pid);
					}
				} else {
					spin_unlock(&btrans->lock);
				}
				...
				/* under binder error/dead status, to_proc could be null, user "-1" instead uid/pid */
				binder_inner_proc_unlock(proc);
				hans_report(type, -1, -1, -1, to_uid, "FROZEN_TRANS_PROC3", btrans_type);
				return;
			}
		}
	}
			...
			if (w != NULL && w->type == BINDER_WORK_TRANSACTION && btrans != NULL) {
				...
				if (btrans->to_thread != NULL && (!(btrans->flags & TF_ONE_WAY))) {
					...
					if (from_uid != to_uid) {
						...
						if (g_skip_trans_binder_reply == 0) {
							binder_inner_proc_unlock(proc);
							hans_report(type, from_pid, from_uid, to_pid, to_uid, "FROZEN_TRANS_FG_PROC1", btrans_type);
							return;
						} else {
							if (btrans->need_reply == 1) {
								binder_inner_proc_unlock(proc);
					...
					if (from_uid != to_uid) {
						...
						if (g_skip_trans_binder_reply == 0) {
							binder_inner_proc_unlock(proc);
							hans_report(type, from_pid, from_uid, to_pid, to_uid, "FROZEN_TRANS_FG_PROC2", btrans_type);
							return;
						} else {
							if (btrans->need_reply == 1) {
								binder_inner_proc_unlock(proc);
				...
				/* under binder error/dead status, to_proc could be null, user "-1" instead uid/pid */
				binder_inner_proc_unlock(proc);
				hans_report(type, -1, -1, -1, to_uid, "FROZEN_TRANS_FG_PROC3", btrans_type);
				return;
			}
		}
	}
				       ...
				       enum message_type type, int uid)
{
	...
	for (n = rb_first(&proc->threads); n != NULL; n = rb_next(n)) {
		...
		if (!empty) {
			...
			if (found == true) {
				binder_inner_proc_unlock(proc);
				hans_report(type, -1, -1, -1, uid, "FROZEN_TRANS_THREAD", btrans_type);
				return;
			}
		}
		...
		if (btrans) {
			...
			if (btrans->to_thread == thread
				...
				if (g_skip_trans_binder_reply == 0) {
					binder_inner_proc_unlock(proc);
					hans_report(type, -1, -1, -1, uid, "FROZEN_TRANS_STACK", btrans_type);
					return;
				} else {
					if (btrans->need_reply == 1) {
						binder_inner_proc_unlock(proc);
	...
	if (!empty) {
		...
		if (found == true) {
			binder_inner_proc_unlock(proc);
			hans_report(type, -1, -1, -1, uid, "FROZEN_TRANS_PROC", btrans_type);
			return;
		}
	}
	...
	if (!empty) {
		...
		if (found == true) {
			binder_inner_proc_unlock(proc);
			hans_report(type, -1, -1, -1, uid, "FROZEN_TRANS_FG_PROC", btrans_type);
			return;
		}
	}
