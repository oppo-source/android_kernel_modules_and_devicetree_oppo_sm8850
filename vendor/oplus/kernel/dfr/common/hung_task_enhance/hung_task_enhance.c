...
static bool is_main_process(struct task_struct *t, const struct cred *tcred)
{
	return !strncmp(t->comm, "main", TASK_COMM_LEN) && (tcred->uid.val == 0) &&
		   (t->parent != 0 && !strncmp(t->parent->comm, "init", TASK_COMM_LEN));
}

static bool is_system_process(struct task_struct *t)
{
	return !strncmp(t->comm, "system_server", TASK_COMM_LEN) ||
		   !strncmp(t->comm, "surfaceflinger", TASK_COMM_LEN);
}

static bool is_binder_main_process(struct task_struct *t, const struct cred *tcred)
{
	return !strncmp(t->comm, "Binder:", TASK_COMM_LEN) && (t->group_leader->pid == t->pid) &&
		   (tcred->uid.val == 1000) && (t->parent != 0 && !strncmp(t->parent->comm, "main", TASK_COMM_LEN));
}

static bool is_init_process(struct task_struct *t, const struct cred *tcred)
{
	if (!strncmp(t->comm, "init", TASK_COMM_LEN) && (tcred->uid.val == 0) && (t->pid == 1))
		return true;
	if (!strncmp(t->comm, "init", TASK_COMM_LEN) && (tcred->uid.val == 0) &&
		(t->parent != 0) && !strncmp(t->parent->comm, "init", TASK_COMM_LEN) && (t->parent->pid == 1))
		return true;
	return false;
}

static bool is_watchdog_process(struct task_struct *t)
{
#if (!IS_ENABLED(CONFIG_OPLUS_DDK_MTK))
	return !strncmp(t->comm, "watchdog", TASK_COMM_LEN) && (!strncmp(t->group_leader->comm, "system_server", TASK_COMM_LEN));
#else
	return false;
#endif
}
...
static bool is_usersapce_key_process(struct task_struct *t)
{
	...
	return is_main_process(t, tcred) || is_system_process(t) || is_binder_main_process(t, tcred) ||
		is_init_process(t, tcred) || is_watchdog_process(t);
}

static const char *const ignore_process_list[] = {
	"mdss_dsi_event",
	"msm-core:sampli",
	"mdss_fb0",
	"mdss_fb_ffl0",
...
static const char *const ignore_display_list[] = {
	"crtc_commit",
	"crtc_event"
};
...
{
	for (int i = 0; i < ARRAY_SIZE(ignore_process_list); i++) {
		if (!strncmp(t->comm, ignore_process_list[i], TASK_COMM_LEN))
			return true;
	}
	for (int i = 0; i < ARRAY_SIZE(ignore_display_list); i++) {
		if (!strncmp(t->comm, ignore_display_list[i], DISP_TASK_COMM_LEN_MASK))
			return true;
	}
	return false;
}
