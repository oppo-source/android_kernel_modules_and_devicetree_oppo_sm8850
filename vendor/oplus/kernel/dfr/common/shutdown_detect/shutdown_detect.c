...
static struct file *get_reserve_partition_bdev(void) {
	...
	while (retry_wait_for_device--) {
		if (lookup_bdev(PATH_OPLUS_RESERVE_1, &dev)) {
			pr_err("failed to get bdev!\n");
			return NULL;
		}
		if (dev != 0) {
...
static void handle_shutdown_timeout(long val)
{
	switch (val) {
...
static bool should_dump_ubt(void)
{
#if IS_ENABLED (CONFIG_OPLUS_BSP_DFR_USERSPACE_BACKTRACE)
	return (shutdown_phase == SHUTDOWN_STAGE_INIT ||
