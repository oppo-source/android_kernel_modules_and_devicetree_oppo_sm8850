        ...
        size_t count,loff_t *off)
{
    ...
    size_t to_copy = min(count, (size_t)len);
    if (copy_to_user(buf, page + *off, to_copy))
        return -EFAULT;
        ...
        size_t count,loff_t *off)
{
    ...
    len = snprintf(page, sizeof(page), "=== shutdown_recorder_enable:%d ===\n",
    ...
    if (copy_to_user(buf,page + *off,(len < count ? len : count))) {
       return -EFAULT;
    }
