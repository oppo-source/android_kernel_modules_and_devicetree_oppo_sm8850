...
static int setup_dev_map_file(struct file *dev_map_file, struct file **dev_file_tmp)
{
	struct block_device *bdev = NULL;
#if LINUX_VERSION_CODE >= KERNEL_VERSION(6, 12, 0)
	*dev_file_tmp = bdev_file_open_by_path(TARGET_DEV_BLOCK, FMODE_READ | FMODE_WRITE, NULL, NULL);
	if (IS_ERR(*dev_file_tmp)) {
		pr_err("Failed to get dev_file_tmp\n");
		return -1;
	} else {
		bdev = file_bdev(*dev_file_tmp);
	}
...
static int record_combkey_write_operation(struct file *dev_map_file, unsigned int combkey_long_press)
{
	int ret = 0;
	struct kiocb kiocb;
...
static int record_combkey_longpress_flag(unsigned int flag)
{
	unsigned int combkey_longpress = flag;
	int ret = 0;
	struct file dev_map_file;
	struct file *dev_file_tmp = NULL;
	memset(&dev_map_file, 0, sizeof(struct file));
	ret = setup_dev_map_file(&dev_map_file, &dev_file_tmp);
	if (ret == -1) {
		return ret;
	}
	ret = record_combkey_write_operation(&dev_map_file, combkey_longpress);
	if (ret == 1) {
#if LINUX_VERSION_CODE >= KERNEL_VERSION(6, 12, 0)
		fput(dev_file_tmp);
#endif
...
static void combkey_long_press_callback(struct work_struct *work)
{
	...
	record_combkey_longpress_flag(1);
}
...
static void clear_combkey_long_press_flag_callback(struct work_struct *work)
{
	if (g_combkey_long_press_flag == 1) {
		...
		record_combkey_longpress_flag(0);
	} else {
...
static void handle_power_key_event(struct keyevent_notifier_param *param,
				   struct key_press_time_data *pwr_tm_data)
{
	...
	if (param->down == KEY_DOWN_VALUE) {
		...
		scnprintf(payload, sizeof(payload),
			...
			pwrkey_press_count, pwr_tm_data->curr_up_time, pwr_tm_data->curr_down_time,
			pwr_tm_data->key_handle_interval);
		oplus_kevent_fb(FB_TRI_STATE_KEY, BUTTON_DEBOUNCE_TYPE, payload);
#endif
	}
}

static void handle_volume_up_event(struct keyevent_notifier_param *param,
				   struct key_press_time_data *volup_tm_data)
{
	...
	if (param->down == KEY_DOWN_VALUE) {
		...
		scnprintf(payload, sizeof(payload),
			...
			volup_press_count, volup_tm_data->curr_up_time, volup_tm_data->curr_down_time,
			volup_tm_data->key_handle_interval);
		oplus_kevent_fb(FB_TRI_STATE_KEY, BUTTON_DEBOUNCE_TYPE, payload);
#endif
	}
}

static void handle_combination_key_logic(void)
{
	...
	if (is_pwrkey_down && is_volumup_down) {
		...
		schedule_delayed_work(&g_check_combkey_long_press_work,
				      msecs_to_jiffies(CHECK_COMBKEY_LONG_PRESS_MS));
	} else {
...
static void handle_power_key_only_logic(void)
{
	/* only power key pressed, start to calculate duration */
	if (is_pwrkey_down && !is_volumup_down) {
		pr_info("power key pressed.\n");
		schedule_delayed_work(&g_check_pwrkey_long_press_work,
				      msecs_to_jiffies(CHECK_PWRKEY_LONG_PRESS_MS));
	} else {
		pr_info("power key canceled.\n");
		schedule_delayed_work(&g_clear_combkey_long_press_flag_work,
				      msecs_to_jiffies(CLEAR_COMBKEY_LONG_PRESS_FLAG_MS));
		cancel_delayed_work(&g_check_pwrkey_long_press_work);
	}
}

static int combkey_monitor_notifier_call(struct notifier_block *nb,
					 unsigned long type, void *data)
{
	...
	if (param->keycode == KEY_POWER) {
		handle_power_key_event(param, &pwr_tm_data);
	} else if (param->keycode == KEY_VOLUMEUP) {
		handle_volume_up_event(param, &volup_tm_data);
	}

	handle_combination_key_logic();
	handle_power_key_only_logic();
