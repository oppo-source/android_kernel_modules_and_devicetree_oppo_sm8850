...
typedef bool (*param_handler_t)(char *value);

/* Individual parameter handlers */
static bool handle_state_param(char *value)
{
	int state;
...
static bool handle_timeout_param(char *value)
{
	int timeout;
...
static bool handle_log_param(char *value)
{
	int get_log;
...
static bool handle_panic_param(char *value)
{
	int is_panic;
...
static bool handle_systemserver_pid_param(char *value)
{
	int s_pid;
...
static bool handle_boot_completed_param(char *value)
{
...
static const struct param_entry param_table[] = {
	{ "state", handle_state_param },
	{ "timeout", handle_timeout_param },
	{ "log", handle_log_param },
	{ "panic", handle_panic_param },
	{ "systemserver_pid", handle_systemserver_pid_param },
	{ "boot-completed", handle_boot_completed_param },
};

#define PARAM_TABLE_SIZE ARRAY_SIZE(param_table)
...
static bool handle_param_setup(char *key, char *value)
{
	...
	for (i = 0; i < PARAM_TABLE_SIZE; i++) {
		if (!strcmp(key, param_table[i].key)) {
			return param_table[i].handler(value);
		}
	}
