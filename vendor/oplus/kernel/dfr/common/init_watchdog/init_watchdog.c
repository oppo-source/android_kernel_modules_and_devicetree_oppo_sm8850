...
static const char * const ignored_processes[] = {
	"shortc_thread",
	"bcc_kthread",
	"usbtemp_kthread",
	"hang_detect",
...
static bool is_ignore_process(struct task_struct *t)
{
	...
	for (int i = 0; i < ARRAY_SIZE(ignored_processes); i++) {
		if (!strncmp(t->comm, ignored_processes[i], TASK_COMM_LEN)) {
			return true;
		}
	}
	return false;
...
static bool should_skip_init_check(void)
{
...
static bool handle_init_kick_state(void)
{
...
static struct task_struct *get_init_task(void)
{
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(6, 1, 0)
#define IS_TASK_FROZEN(task) ((task)->__state == TASK_FROZEN)
#else
#define IS_TASK_FROZEN(task) ((task)->flags & PF_FROZEN)
#endif

#if LINUX_VERSION_CODE >= KERNEL_VERSION(5, 15, 0)
#define IS_TASK_D_STATE(task) (READ_ONCE((task)->__state) == TASK_UNINTERRUPTIBLE)
#else
#define IS_TASK_D_STATE(task) ((task)->state == TASK_UNINTERRUPTIBLE)
#endif

void check_init_process_state(struct task_struct *taskp, char *extra_info) {
    if (!g_init_watchdog_debug_enabled && IS_TASK_FROZEN(taskp)) {
    ...
    if (g_init_watchdog_debug_enabled || IS_TASK_D_STATE(taskp)) {
        pr_err(INIT_WATCHDOG_LOG_TAG "init process block in D-state more than 30s\n");
        snprintf(extra_info, EXTRA_INFO_BUF_LEN,
                 "init process block in D-state more than 30s, block_count is %u",
                 g_init_watchdog_data->block_count);
...
static void init_check_work_handler(struct work_struct *work)
{
	...
	if (should_skip_init_check() || handle_init_kick_state()) {
		return;
	}
	...
	check_init_process_state(taskp, extra_info);
}
