...
int send_oplus_uevent(struct device *device) {
  char *envp[3];
  char event_string[64];
  char match_string[] = "DEVPATH=/devices/virtual/misc/nfc";
  int ret_val = 0;

  if (!device) {
      pr_err("nfc: invalid parameters\n");
      return -EINVAL;
  }

  /*send uevent*/
  strscpy(event_string, "clkreq high", sizeof(event_string));
  envp[0] = (char *)&event_string;
  envp[1] = (char *)&match_string;
  envp[2] = NULL;

  ret_val = kobject_uevent_env(&device->kobj, KOBJ_CHANGE, envp);
  if (!ret_val) {
      pr_info("nfc:kobject_uevent_env %s\n", event_string);
  } else {
     pr_info("nfc:kobject_uevent_env fail,error=%d!\n", ret_val);
  }
  return ret_val;
}
...
int nfc_i2c_dev_suspend(struct device *device)
{
	...
	//#ifdef OPLUS_BUG_STABILITY
	//Add for: store count or clkreq high,10 *30 second, 5min senduevent to Nfc service
	if(gpio_is_valid(nfc_gpio->clkreq)) {
	    if (gpio_get_value(nfc_gpio->clkreq)) {
	        nfc_config->count_high_clkreq++;
               if (nfc_config->count_high_clkreq == 10) {
                  send_oplus_uevent(device);
               }
	    }
            pr_err("%s: clkreq = %d , count_high_clkreq = %d  \n",__func__ ,gpio_get_value(nfc_gpio->clkreq) , nfc_config->count_high_clkreq);
	}
	//#endif /*OPLUS_BUG_STABILITY*/
...
int nfc_i2c_dev_resume(struct device *device)
{
	...
	if(gpio_is_valid(nfc_gpio->clkreq)){
		pr_info("%s: clkreq = %d \n",__func__ ,gpio_get_value(nfc_gpio->clkreq));
        //#ifdef OPLUS_BUG_STABILITY
        //Add for: clkreq =0 ,init count of sys_idle_clkreq
        if (!gpio_get_value(nfc_gpio->clkreq)) {
             nfc_config->count_high_clkreq = 0;
        }
        //#endif /*OPLUS_BUG_STABILITY*/
	}
