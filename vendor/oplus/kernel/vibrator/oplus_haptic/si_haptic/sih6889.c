...
static void sih6889_get_vbat(sih_haptic_t *sih_haptic)
{
	...
	sih6889_load_reg_config(sih_haptic, REG_FUNC_VBAT);
...
static void __maybe_unused sih6889_vbat_comp(sih_haptic_t *sih_haptic)
{
	...
	*/
	comp_gain = SIH6889_STANDARD_VBAT * sih_haptic->chip_ipara.gain / curr_vbat;
...
static int sih6889_efuse_check(sih_haptic_t *sih_haptic)
{
	...
	if (crc4_result != crc4_value) {
		...
		hp_info("%s: crc4 check success, crc_result = 0x%02x, crc_write = 0x%02x\n",
			__func__, crc4_result, crc4_value);
	return ret;
}
...
static void sih6889_get_ram_data(sih_haptic_t *sih_haptic, uint8_t *buf, uint32_t size)
{
	i2c_read_bytes(sih_haptic, SIH6889_REG_RAMDATA, buf, size);
}
...
static void sih6889_set_ram_addr(sih_haptic_t *sih_haptic, uint32_t base_addr)
{
	...
	haptic_regmap_bulk_write(sih_haptic->regmapp.regmapping,
		SIH6889_REG_RAMADDRH, SIH_I2C_OPERA_BYTE_TWO, ram_addr);
}

static void sih6889_set_ram_data(sih_haptic_t *sih_haptic, uint8_t *data,
    uint32_t len)
{
    int i = 0;
    int cnt = 0;
...
static void sih6889_get_lra_resistance(sih_haptic_t *sih_haptic)
{
	...
	sih6889_load_reg_config(sih_haptic, REG_FUNC_RL);
...
static void sih6889_get_detect_f0(sih_haptic_t *sih_haptic)
{
	...
	sih6889_load_reg_config(sih_haptic, REG_FUNC_CONT);
...
#ifdef OPLUS_FEATURE_CHG_BASIC
static void sih6889_parse_dt(struct device *dev, sih_haptic_t *sih_haptic,
	struct device_node *np)
{
	...
	if (of_property_read_u32(np, "sih6889_boost_voltage", &max_boost_voltage))
	...
	val = of_property_read_u8_array(np, "sih6889_vmax",
							vmax, ARRAY_SIZE(vmax));
	if (val != 0) {
		hp_info("sih6889_vmax not found");
	} else {
	...
	val = of_property_read_u8_array(np, "sih6889_gain",
						gain, ARRAY_SIZE(gain));
	if (val != 0) {
		hp_info("sih6889_gain not found");
	} else {
...
static int sih6889_convert_level_to_vmax(sih_haptic_t *sih_haptic, struct vmax_map *map, int val)
{
	int i;
...
static void sih6889_f0_pre_init(sih_haptic_t *sih_haptic)
{
	if (DEVICE_ID_0815 == sih_haptic->device_id || DEVICE_ID_0809 == sih_haptic->device_id) {
		sih_haptic->detect.detect_f0 = SIH_F0_PRE_VALUE;
		sih_haptic->detect.tracking_f0 = SIH_F0_PRE_VALUE;
...
static void sih6889_init(sih_haptic_t *sih_haptic)
{
	...
	/* bst fre select */
	haptic_regmap_update_bits(sih_haptic->regmapp.regmapping,
		SIH6889_REG_ANA_CTRL6,
		SIH6889_ANA_CTRL6_BST_CLK_SEL_O_MASK,
		SIH6889_ANA_CTRL6_BST_CLK_SEL_O_1M);
	...
	*/
	...
	/* f0 pre init */
	sih6889_f0_pre_init(sih_haptic);
...
haptic_func_t sih_6889_func_list = {
	...
	.set_ram_data = sih6889_set_ram_data,
...
#ifdef OPLUS_FEATURE_CHG_BASIC
	.parse_dt = sih6889_parse_dt,
	.convert_level_to_vmax = sih6889_convert_level_to_vmax,
#endif
};
