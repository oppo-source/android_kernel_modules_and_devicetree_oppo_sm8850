...
struct test_kit {
	...
	test_kit_gpio_check_func_t unisoc_soc_gpio_check;
...
bool test_kit_unisoc_soc_gpio_test(struct test_feature *feature, char *buf, size_t len)
{
	struct test_kit_soc_gpio_info *gpio_info;
	size_t index = 0;
	size_t use_size = 0;
	...
	if (feature == NULL) {
		pr_err("feature is NULL\n");
		index += scnprintf(buf + index, len - index, "feature is NULL");
		return false;
	}
	if (g_test_kit == NULL) {
		pr_err("g_test_kit is NULL\n");
		index += scnprintf(buf + index, len - index, "g_test_kit is NULL");
		return false;
	}
	if (g_test_kit->unisoc_soc_gpio_check == NULL) {
		pr_err("unisoc_soc_gpio_check is NULL\n");
		index += scnprintf(buf + index, len - index, "gpio check func is NULL");
		return false;
	}

	gpio_info = feature->cfg->test_info;
	while (gpio_info->name) {
		if (!g_test_kit->unisoc_soc_gpio_check((void *)gpio_info,
			buf + index, len - index, &use_size))
			pass = false;
		index += use_size;
		if (index >= len) {
...
EXPORT_SYMBOL(test_kit_unisoc_soc_gpio_test);
...
static __init int test_kit_driver_init(void)
{
	...
	test_kit->unisoc_soc_gpio_check = test_kit_unisoc_gpio_check;
