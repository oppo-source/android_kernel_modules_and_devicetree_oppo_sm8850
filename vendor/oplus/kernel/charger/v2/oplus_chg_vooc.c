...
static void oplus_vooc_fcs_suspend_adapter(struct oplus_chg_vooc *chip, bool boot_completed,
				   bool suspend, int icl_ma)
{
	if (oplus_chg_get_fcs_support_flags() && !boot_completed && is_wired_icl_votable_available(chip))
...
static void oplus_reset_adapter(struct oplus_chg_vooc *chip)
{
	...
	bool boot_completed = oplus_comm_get_boot_completed();
	...
	oplus_vooc_fcs_suspend_adapter(chip, boot_completed, true, SVOOC_SUSPEND_ICL1_MA);
	...
	oplus_vooc_fcs_suspend_adapter(chip, boot_completed, false, 0);
	...
	if (chip->switch_retry_count == 0) {
		...
		if (chg_type == OPLUS_CHG_USB_TYPE_QC2 ||
			...
			if (is_wired_charge_suspend_votable_available(chip)) {
				...
				boot_completed = oplus_comm_get_boot_completed();
				if (oplus_chg_get_fcs_support_flags() && !boot_completed && is_wired_icl_votable_available(chip))
