...
struct oplus_chg_vooc {
	...
	int efficient_vooc_normal_mid_temp;
					...
					int *temp_cur_range, int *batt_temp_status)
{
	...
	} else if (chip->spec.vooc_normal_mid_temp != -EINVAL &&
		   vbat_temp_cur < chip->efficient_vooc_normal_mid_temp) {
		if (temp_cur_range)
			*temp_cur_range = FASTCHG_TEMP_RANGE_NORMAL_MID;
		if (batt_temp_status)
			*batt_temp_status = BAT_TEMP_NORMAL_MID;
	} else { /*25C-43C*/
...
static int oplus_check_vooc_temp_cur_range(struct oplus_chg_vooc *chip)
{
	...
	bool normal_low_mid_inval;
	int ret;

	little_cool_high_inval = (chip->spec.vooc_little_cool_high_temp == -EINVAL);
	normal_low_mid_inval = (chip->spec.vooc_normal_mid_temp == -EINVAL);

/*
oplus_fastchg_temp_rang 						normal_low_mid_inval
...
FASTCHG_TEMP_RANGE_NORMAL_LOW,			5		-1	FASTCHG_TEMP_RANGE_NORMAL_LOW,		4
FASTCHG_TEMP_RANGE_NORMAL_MID,			6		-2	FASTCHG_TEMP_RANGE_NORMAL_LOW,		4
...
/*
oplus_fastchg_temp_rang 						normal_low_mid_inval && normal_low_mid_inval
FASTCHG_TEMP_RANGE_INIT = 0,			0		-0
...
FASTCHG_TEMP_RANGE_NORMAL_LOW,			5		-2	FASTCHG_TEMP_RANGE_NORMAL_LOW,		3
FASTCHG_TEMP_RANGE_NORMAL_MID,			6		-3	FASTCHG_TEMP_RANGE_NORMAL_LOW,		3
...
*/
	if (chip->config.voocphy_support == NO_VOOCPHY || chip->config.voocphy_support == ADSP_VOOCPHY) {
		if (normal_low_mid_inval && little_cool_high_inval) {
			if (chip->vooc_temp_cur_range >= FASTCHG_TEMP_RANGE_NORMAL_MID)
				ret = chip->vooc_temp_cur_range - 3;
			else if (chip->vooc_temp_cur_range >= FASTCHG_TEMP_RANGE_LITTLE_COOL_HIGH &&
			    chip->vooc_temp_cur_range < FASTCHG_TEMP_RANGE_NORMAL_MID)
				ret = chip->vooc_temp_cur_range - 2;
			else
				ret = chip->vooc_temp_cur_range - 1;
		} else if (normal_low_mid_inval) {
			if (chip->vooc_temp_cur_range >= FASTCHG_TEMP_RANGE_NORMAL_MID)
				ret = chip->vooc_temp_cur_range - 2;
			else
				ret = chip->vooc_temp_cur_range - 1;
...
FASTCHG_TEMP_RANGE_NORMAL_LOW,			5		-2	BCC_TEMP_RANGE_NORMAL_LOW,		3
FASTCHG_TEMP_RANGE_NORMAL_MID,			6		-3	BCC_TEMP_RANGE_NORMAL_LOW,		3
...
static void oplus_vooc_reset_temp_range(struct oplus_chg_vooc *chip)
{
	...
	chip->efficient_vooc_normal_mid_temp = spec->vooc_normal_mid_temp;
	...
	chg_info("[%d %d %d %d %d %d %d]\n", chip->efficient_vooc_little_cold_temp,
		 ...
		 chip->efficient_vooc_normal_mid_temp,
		 chip->efficient_vooc_normal_high_temp);
}
...
static void oplus_vooc_rang_rise_update(struct oplus_chg_vooc *chip)
{
	...
	if (pre_vooc_temp_rang == FASTCHG_TEMP_RANGE_WARM ||
		...
		oplus_vooc_reset_temp_range(chip);
		chip->efficient_vooc_normal_high_temp -= VOOC_TEMP_RANGE_THD;
	} else if (pre_vooc_temp_rang == FASTCHG_TEMP_RANGE_NORMAL_MID) {
		chip->fastchg_batt_temp_status = BAT_TEMP_NORMAL_HIGH;
		chip->vooc_temp_cur_range = FASTCHG_TEMP_RANGE_NORMAL_HIGH;
		oplus_vooc_reset_temp_range(chip);
		chip->efficient_vooc_normal_mid_temp -= VOOC_TEMP_RANGE_THD;
	} else if (pre_vooc_temp_rang == FASTCHG_TEMP_RANGE_NORMAL_LOW) {
		if (chip->spec.vooc_normal_mid_temp != -EINVAL) {
			if (oplus_get_cur_ui_soc(chip) <= chip->spec.vooc_high_soc) {
				oplus_set_fast_status(chip, CHARGER_STATUS_SWITCH_TEMP_RANGE);
				chip->fastchg_batt_temp_status = BAT_TEMP_EXIT;
				chip->vooc_temp_cur_range = FASTCHG_TEMP_RANGE_INIT;
				oplus_vooc_reset_temp_range(chip);
				chip->efficient_vooc_normal_low_temp -= VOOC_TEMP_RANGE_THD_HIGH;
			} else {
				chip->fastchg_batt_temp_status = BAT_TEMP_NORMAL_MID;
				chip->vooc_temp_cur_range = FASTCHG_TEMP_RANGE_NORMAL_MID;
				oplus_vooc_reset_temp_range(chip);
					...
					int cur_temp)
{
	...
	case FASTCHG_TEMP_RANGE_NORMAL_HIGH: /*35~43*/
		if (chip->spec.vooc_normal_mid_temp != -EINVAL)
			rang_start = chip->efficient_vooc_normal_mid_temp;
		else
		...
		if (chip->spec.vooc_normal_high_temp != -EINVAL)
			...
			rang_end = chip->spec.vooc_over_high_temp;
		break;
	case FASTCHG_TEMP_RANGE_NORMAL_MID:
		rang_start = chip->efficient_vooc_normal_low_temp;
		rang_end = chip->efficient_vooc_normal_mid_temp;
		break;
				   ...
				   struct device_node *node)
{
	...
	rc = of_property_read_s32(node, "oplus_spec,vooc_normal_mid_temp",
				  &spec->vooc_normal_mid_temp);
	if (rc < 0) {
		chg_err("oplus_spec,vooc_normal_mid_temp reading failed, rc=%d\n",
			rc);
		spec->vooc_normal_mid_temp =
			default_spec_config.vooc_normal_mid_temp;
	}
