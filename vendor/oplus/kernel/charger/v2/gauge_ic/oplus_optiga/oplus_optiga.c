...
static int oplus_optiga_guage_init(struct oplus_chg_ic_dev *ic_dev)
{
	if (ic_dev == NULL) {
	...
	chg_info("oplus_optiga_guage_init, ic_dev->online = %d\n", ic_dev->online);
	return 0;
}

static int oplus_optiga_guage_exit(struct oplus_chg_ic_dev *ic_dev)
{
	if (!ic_dev->online)
		return 0;

	ic_dev->online = false;
	chg_info("oplus_optiga_guage_exit, ic_dev->online = %d\n", ic_dev->online);

	return 0;
}

static int oplus_optiga_guage_get_batt_auth(struct oplus_chg_ic_dev *ic_dev, bool *pass)
{
	if (ic_dev == NULL) {
		chg_err("oplus_chg_ic_dev is NULL");
		return -ENODEV;
	...
	*pass = oplus_optiga_get_external_auth_hmac();
	chg_info("hmac = %d\n", *pass);
...
struct oplus_chg_ic_virq optiga_guage_virq_table[] = {
...
/*	ret = optiga_authenticate();
	...
	if (!ret) {
		...
		chg_info("get_external_auth_hmac success.\n");
	}
	...
	ret = of_property_read_u32(g_oplus_optiga_chip->dev->of_node, "oplus,ic_type", &ic_type);
	if (ret < 0) {
		chg_err("can't get ic type, ret=%d\n", ret);
		goto error;
	}
	ret = of_property_read_u32(g_oplus_optiga_chip->dev->of_node, "oplus,ic_index", &ic_index);
	if (ret < 0) {
		chg_err("can't get ic index, ret=%d\n", ret);
		goto error;
	}
	ic_cfg.name = g_oplus_optiga_chip->dev->of_node->name;
	snprintf(ic_cfg.manu_name, OPLUS_CHG_IC_MANU_NAME_MAX - 1, "gauge-optiga");
	ic_cfg.type = ic_type;
	...
	chg_info(" register %s\n", g_oplus_optiga_chip->dev->of_node->name);
	chg_info(" oplus_optiga_probe sucess\n");
	return 0;
...
static __exit void oplus_optiga_driver_exit(void)
{
	platform_driver_unregister(&oplus_optiga_driver);
}

oplus_chg_module_register(oplus_optiga_driver);
