...
static int oplus_mt6375_gauge_get_gauge_c_soc(struct oplus_chg_ic_dev *ic_dev, int *c_soc)
{
	int rc;

	if (c_soc == NULL) {
		chg_err("c_soc is NULL\n");
		return -EINVAL;
	}
	if (g_gauge_chip == NULL || g_gauge_chip->gauge_ops == NULL) {
		chg_err("g_gauge_chip is null.\n");
	...
	if (g_gauge_chip->gauge_ops->get_gauge_c_soc == NULL)
		return -EINVAL;

	rc = g_gauge_chip->gauge_ops->get_gauge_c_soc(c_soc);
	if (rc < 0) {
		chg_err("failed to get c_soc from mtk\n");
		*c_soc = 0;
		return -EINVAL;
	}
	return 0;
				...
				enum oplus_chg_ic_func func_id)
{
	...
	case OPLUS_IC_FUNC_GAUGE_GET_C_SOC:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_GAUGE_GET_C_SOC,
					       oplus_mt6375_gauge_get_gauge_c_soc);
		break;
	default:
