...
enum {
	...
	FASTCHG_TEMP_RANGE_NORMAL_LOW, /*16~21*/
	FASTCHG_TEMP_RANGE_NORMAL_MID, /*21~25*/
	FASTCHG_TEMP_RANGE_NORMAL_HIGH, /*25~35*/
	FASTCHG_TEMP_RANGE_WARM, /*35~43*/
...
static const char * const temp_region_text[] = {
	...
	[VOOCPHY_BATT_TEMP_NORMAL_MID]		= "normal_mid",
...
static void oplus_voocphy_reset_temp_range(struct oplus_voocphy_manager *chip)
{
	if (chip != NULL) {
		...
		chip->vooc_normal_mid_temp = chip->vooc_normal_mid_temp_default;
	}
}
...
void oplus_voocphy_get_soc_and_temp_with_enter_fastchg(struct oplus_voocphy_manager *chip)
{
	...
	case FASTCHG_TEMP_RANGE_NORMAL_MID :
		...
		sys_curve_temp_idx = BATT_SYS_CURVE_TEMP_NORMAL_MID;
		chip->batt_temp_plugin = VOOCPHY_BATT_TEMP_NORMAL_MID;
		break;
	...
	default:
		if (batt_temp < chip->vooc_little_cold_temp) { /* 0-5C */
			...
			sys_curve_temp_idx = BATT_SYS_CURVE_TEMP_LITTLE_COOL_HIGH;
			chip->batt_temp_plugin = VOOCPHY_BATT_TEMP_LITTLE_COOL_HIGH;
		} else if (batt_temp < chip->vooc_normal_low_temp) { /* 21-25C */
			chip->vooc_temp_cur_range = FASTCHG_TEMP_RANGE_NORMAL_LOW;
			chip->fastchg_batt_temp_status = BAT_TEMP_NORMAL_LOW;
			sys_curve_temp_idx = BATT_SYS_CURVE_TEMP_NORMAL_LOW;
			chip->batt_temp_plugin = VOOCPHY_BATT_TEMP_NORMAL;
		} else if (chip->vooc_normal_mid_temp != -EINVAL && batt_temp < chip->vooc_normal_mid_temp) { /* 25-35C */
			chip->vooc_temp_cur_range = FASTCHG_TEMP_RANGE_NORMAL_MID;
			chip->fastchg_batt_temp_status = BAT_TEMP_NORMAL_MID;
			sys_curve_temp_idx = BATT_SYS_CURVE_TEMP_NORMAL_MID;
			chip->batt_temp_plugin = VOOCPHY_BATT_TEMP_NORMAL_MID;
...
static const char * const strategy_temp[] = {
	...
	[BATT_SYS_CURVE_TEMP_NORMAL_MID]	= "strategy_temp_normal_mid",
					     ...
					     struct device_node *node)
{
	...
	if (chip->vooc_normal_mid_temp == -EINVAL)
	...
	if (rc == full_voltage_size) {
		...
		if (rc) {
			...
			if (chip->vooc_normal_mid_temp == -EINVAL) {
				...
				chip->full_voltage[VOOCPHY_BATT_TEMP_NORMAL_HIGH] = chip->full_voltage[VOOCPHY_BATT_TEMP_NORMAL_MID];
				chip->full_voltage[VOOCPHY_BATT_TEMP_NORMAL_MID].vol_1time = -EINVAL;
				chip->full_voltage[VOOCPHY_BATT_TEMP_NORMAL_MID].vol_ntime = -EINVAL;
			}
		}
	}  else {
		...
		for (j = 0; j < BATT_SYS_CURVE_MAX; j++) {
			...
			if (rc < 0) {
				if (j == BATT_SYS_CURVE_TEMP_WARM || j == BATT_SYS_CURVE_TEMP_LITTLE_COOL_HIGH ||
				    j == BATT_SYS_CURVE_TEMP_NORMAL_MID) {
					continue;
				} else {
...
static int oplus_voocphy_parse_batt_curves(struct oplus_voocphy_manager *chip)
{
	...
	rc = of_property_read_u32(node, "oplus_spec,vooc_normal_mid_temp",
	                          &chip->vooc_normal_mid_temp );
	if (rc) {
		chip->vooc_normal_mid_temp  = -EINVAL;
	} else {
		voocphy_dbg("oplus_spec,vooc_normal_low_mid_temp is %d\n", chip->vooc_normal_mid_temp );
	}
	chip->vooc_normal_mid_temp_default = chip->vooc_normal_mid_temp ;
