...
static int oplus_ufcs_get_cable_info(struct oplus_ufcs *chip, u8 *cable_info, int size)
{
	...
	rc = oplus_chg_ic_func(chip->ufcs_ic, OPLUS_IC_FUNC_UFCS_GET_CABLE_INFO, cable_info, size);
...
static int oplus_ufcs_set_fcs_icl(int icl_ma)
{
...
static void oplus_ufcs_switch_check_work(struct work_struct *work)
{
	...
	if (chip->oplus_ufcs_adapter) {
		if (UFCS_DEVICE_INFO_DEV_VENDOR(chip->dev_info) == UFCS_OPLUS_DEV_ID) {
			...
			if (rc < 0) {
				...
				if (UFCS_CABLE_INFO_CURR_MAX(chip->cable_info) <= 0) {
				...
				} else if (chip->config.target_vbus_mv <= UFCS_CABLE_INFO_VOL_MAX(chip->cable_info)) {
					vote(chip->ufcs_curr_votable, CABLE_MAX_VOTER, true, UFCS_CABLE_INFO_CURR_MAX(chip->cable_info), false);
					oplus_ufcs_push_emark_power(chip, UFCS_POWER(chip->config.target_vbus_mv, UFCS_CABLE_INFO_CURR_MAX(chip->cable_info)));
				}
			}
		}
	}
	...
	if (oplus_chg_get_fcs_support_flags()) {
		...
		oplus_ufcs_set_fcs_icl(UFCS_FCS_ICL1_MA);
	} else {
		if (is_wired_suspend_votable_available(chip))
			vote(chip->wired_suspend_votable, UFCS_VOTER, true, 1, false);
	}
...
#define ADSP_UFCS_START_PDO_DELAY_MS	100
	...
	if ((cp_vin >= target_vbus && cp_vin <= (target_vbus + chip->config.upper_compensation_mv))
		...
		if (oplus_chg_get_fcs_support_flags()) {
			oplus_ufcs_set_fcs_icl(UFCS_FCS_ICL2_MA);
