...
int ufcs_get_cable_info(struct ufcs_class *class, u8 *cable_info, int size)
{
	int rc;
	int cable_size;
	struct ufcs_event *event;
	...
	cable_size = size > UFCS_CABLE_INFO_SIZE ? UFCS_CABLE_INFO_SIZE : size;
	memmove(cable_info, class->cable_info, cable_size);
...
int ufcs_intf_get_cable_info(struct ufcs_dev *ufcs, u8 *cable_info, int size)
{
	struct ufcs_class *class;
	...
	return ufcs_get_cable_info(class, cable_info, size);
}
