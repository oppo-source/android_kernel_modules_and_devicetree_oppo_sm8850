...
static struct sde_irq_type sde_irq_intf_map[] = {
	...
	{ SDE_IRQ_TYPE_INTF_PANEL_VSYNC, -1, SDE_INTR_PANEL_VSYNC, -1},
	{ SDE_IRQ_TYPE_INTF_ESYNC_MDP_VSYNC, -1, SDE_INTR_ESYNC_MDP_VSYNC, -1},
};
...
void __iomem *regmap1 = NULL;
void __iomem *regmap2 = NULL;
void __iomem *regmap3 = NULL;
void __iomem *regmap4 = NULL;
void __iomem *regmap5 = NULL;
void __iomem *regmap6 = NULL;
void __iomem *regmap7 = NULL;
void __iomem *regmap8 = NULL, *regmap9 = NULL, *regmap10 = NULL, *regmap11 = NULL, *regmap12 = NULL;
		...
		void *arg)
{
	...
	if (regmap1 && regmap2)
		SDE_EVT32(0x1111, readl_relaxed(regmap1), readl_relaxed(regmap2));
	...
	if (diff_time && diff_time < SDE_INTR_STORM_TIME_THRESHOLD_NS) {
		counter++;
		if (regmap3 && regmap4) {
			hist_status = readl_relaxed(regmap3);
			if (hist_status & 0x200000 || hist_status & 0x400000 ||
				...
				clear_status = readl_relaxed(regmap4);
				clear_status |= 0xf33000;
				writel_relaxed(clear_status, regmap4);
			}
		}
		if (regmap5 && regmap6) {
			ltm0_status = readl_relaxed(regmap5);
			if (ltm0_status & 0x40) {
				clear_status = readl_relaxed(regmap6);
				clear_status |= 0x40;
				writel_relaxed(clear_status, regmap6);
			}
		}
		if (regmap7 && regmap8) {
			ltm1_status = readl_relaxed(regmap7);
			if (ltm1_status & 0x40) {
				clear_status = readl_relaxed(regmap8);
				clear_status |= 0x40;
				writel_relaxed(clear_status, regmap8);
			}
		}
		if (regmap9 && regmap10) {
			ltm2_status = readl_relaxed(regmap9);
			if (ltm2_status & 0x40) {
				clear_status = readl_relaxed(regmap10);
				clear_status |= 0x40;
				writel_relaxed(clear_status, regmap10);
			}
		}
		if (regmap11 && regmap12) {
			ltm3_status = readl_relaxed(regmap11);
			if (ltm3_status & 0x40) {
				clear_status = readl_relaxed(regmap12);
				clear_status |= 0x40;
				writel_relaxed(clear_status, regmap12);
			}
		}
		...
		struct sde_mdss_cfg *m)
{
	...
	regmap1 = ioremap(SDE_REG_INTR2_STATUS_ADDR, 0X4);
	regmap2 = ioremap(SDE_REG_INTR_STATUS_ADDR, 0X4);
	regmap3 = ioremap(SDE_REG_HIST_STATUS_ADDR, 0x4);
	regmap4 = ioremap(SDE_REG_HIST_CLEAR_ADDR, 0x4);
	regmap5 = ioremap(SDE_REG_LTM0_STATUS_ADDR, 0x4);
	regmap6 = ioremap(SDE_REG_LTM0_CLEAR_ADDR, 0x4);
	regmap7 = ioremap(SDE_REG_LTM1_STATUS_ADDR, 0x4);
	regmap8 = ioremap(SDE_REG_LTM1_CLEAR_ADDR, 0x4);
	regmap9 = ioremap(SDE_REG_LTM2_STATUS_ADDR, 0x4);
	regmap10 = ioremap(SDE_REG_LTM2_CLEAR_ADDR, 0x4);
	regmap11 = ioremap(SDE_REG_LTM3_STATUS_ADDR, 0x4);
	regmap12 = ioremap(SDE_REG_LTM3_CLEAR_ADDR, 0x4);
