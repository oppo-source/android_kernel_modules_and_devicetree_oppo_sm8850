...
#define OPLUS_ADFR_CONFIG_SOFTWARE_MARKER                    (BIT(14))
...
#define OPLUS_ADFR_GET_SOFTWARE_MARKER_CONFIG(config)        ((config) & OPLUS_ADFR_CONFIG_SOFTWARE_MARKER)
...
static bool oplus_adfr_software_marker_is_enabled(void *oplus_adfr_params)
{
	struct oplus_adfr_params *p_oplus_adfr_params = oplus_adfr_params;
	...
	if (!oplus_adfr_is_supported(p_oplus_adfr_params)) {
		ADFR_DEBUG("adfr is not supported, software_marker is also not supported\n");
		return false;
	}

	return (bool)(OPLUS_ADFR_GET_SOFTWARE_MARKER_CONFIG(p_oplus_adfr_params->config));
}
...
int oplus_adfr_irq_handler(void *sde_encoder_phys, unsigned int irq_type)
{
	...
	if (irq_type == OPLUS_ADFR_RD_PTR) {
		...
		/* software corner mark */
		if (oplus_adfr_software_marker_is_enabled(p_oplus_adfr_params)) {
			delta = rd_ptr_timestamp_us - wr_ptr_timestamp_us;
			if (delta < 200) {
				p_oplus_adfr_params->test_te.refresh_rate = mode->timing.refresh_rate;
			} else if (delta < 45000 && p_oplus_adfr_params->sa_min_fps <= 30) {
...
enum hrtimer_restart oplus_adfr_test_te_timer_handler(struct hrtimer *timer)
{
	...
	if (oplus_adfr_software_marker_is_enabled(p_oplus_adfr_params)) {
		ADFR_INFO("use software_marker, no need to init test te timer handler");
		goto end;
	}
...
static irqreturn_t oplus_adfr_test_te_irq_handler(int irq, void *data)
{
	...
	if (oplus_adfr_software_marker_is_enabled(p_oplus_adfr_params)) {
		ADFR_INFO("use software_marker, no need to init test te irq handler");
		return IRQ_HANDLED;
	}
...
int oplus_adfr_register_test_te_irq(void *dsi_display)
{
	...
	if (oplus_adfr_software_marker_is_enabled(p_oplus_adfr_params)) {
		ADFR_INFO("use software_marker, no need to register test te irq");
		return 0;
	}
...
int oplus_adfr_adjust_osync_tearcheck(void *sde_encoder_phys)
{
	...
	*/
	...
	if(p_oplus_adfr_params->osync_sync_threshold_start != tc_cfg.sync_threshold_start) {
		...
		cmd_enc->qsync_threshold_lines = tc_cfg.sync_threshold_start;
		if (phys_enc->has_intf_te && phys_enc->hw_intf->ops.update_tearcheck[disp_op]) {
			phys_enc->hw_intf->ops.update_tearcheck[disp_op](phys_enc->hw_intf, &tc_cfg);
		} else if (phys_enc->hw_pp->ops.update_tearcheck[disp_op]) {
			phys_enc->hw_pp->ops.update_tearcheck[disp_op](phys_enc->hw_pp, &tc_cfg);
		}
