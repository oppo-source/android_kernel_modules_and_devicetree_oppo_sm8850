...
int oplus_display_panel_set_LGD_value(void *buf)
{
	...
	if (value == 1) {
		panel->oplus_panel.lgd_status = 1;
	} else {
		panel->oplus_panel.lgd_status = 0;
	}
	...
	OPLUS_DSI_INFO("Set LGD Value = %d\n", value);

	if (panel->oplus_panel.lgd_support == false) {
		OPLUS_DSI_ERR("panel don't support LGD\n");
		rc = -EINVAL;
		goto unlock;
	}
	...
	if (value == 1) {
		...
		if (rc) {
			OPLUS_DSI_ERR("send DSI_CMD_SET_SWITCH_LGD_ON failed, retry\n");
			goto unlock;
		}
	} else {
		rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_SWITCH_LGD_OFF, false);
		if (rc) {
			OPLUS_DSI_ERR("send DSI_CMD_SET_SWITCH_LGD_OFF failed, retry\n");
			goto unlock;
		}
	}
...
{
	int rc = 0;
	unsigned int *lgd_value;
	struct dsi_display *display;

	if (!buf) {
		OPLUS_DSI_ERR("buf is null\n");
		return -EINVAL;
	}
	...
	*lgd_value = LGD_Value;
	OPLUS_DSI_INFO("Get LGD Value = %d\n", *lgd_value);
