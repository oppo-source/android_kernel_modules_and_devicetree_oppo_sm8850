...
static const struct panel_ioctl_desc panel_ioctls[] = {
	...
	PANEL_IOCTL_DEF(PANEL_IOCTL_GET_PANEL_BTBSN, oplus_display_ioctl_get_panel_btbsn),
	...
	PANEL_IOCTL_DEF(PANEL_IOCTL_SET_LGD_VALUE, oplus_display_panel_set_LGD_value),
	PANEL_IOCTL_DEF(PANEL_IOCTL_GET_LGD_VALUE, oplus_display_panel_get_LGD_value),
};
...
static int oplus_display_set_apollo_backlight_value(void *data)
{
	...
	OPLUS_DSI_INFO("panel_level = %d, apollo_level = %d\n",
...
static int oplus_export_dmabuf(int buf_size)
{
	...
	if (!vaddr) {
		retcode = -ENOMEM;
		OPLUS_DSI_ERR("alloc_pages fail\n");
		goto err_dmabuf;
	}
	...
	if (IS_ERR(dmabuf)) {
		retcode = -EINVAL;
		OPLUS_DSI_ERR("dma_buf_export fail\n");
	...
	OPLUS_DSI_INFO("buf_size = %d bytes, p_panel_backlight = %p, vaddr = %p\n",
...
int oplus_display_panel_init(void)
{
	...
	if (rc < 0) {
		OPLUS_DSI_ERR("dmabuf alloc fail\n");
		goto err_device_create;
	}
