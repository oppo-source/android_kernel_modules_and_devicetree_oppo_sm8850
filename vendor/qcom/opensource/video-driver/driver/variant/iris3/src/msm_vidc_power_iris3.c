...
static int msm_vidc_calc_freq_iris3_legacy(struct msm_vidc_inst *inst,
...
static int msm_vidc_calc_freq_iris3_new(struct msm_vidc_inst *inst,
					struct vidc_clock_scaling_data *clock_scaling_data)
{
	u64 freq = 0, nom_freq;
	...
	i_vpr_p(inst, "%s: filled len %d, required freq %llu, fps %u, mbpf %u\n",
		__func__, clock_scaling_data->data_size, freq, fps, mbpf);
...
static int
msm_vidc_calc_freq_iris3(struct msm_vidc_inst *inst,
			 struct vidc_clock_scaling_data *clock_scaling_data)
{
	if (ENABLE_LEGACY_POWER_CALCULATIONS)
		return msm_vidc_calc_freq_iris3_legacy(inst, clock_scaling_data);
	else
		return msm_vidc_calc_freq_iris3_new(inst, clock_scaling_data);
}
...
int msm_vidc_scale_clocks_iris3(struct msm_vidc_inst *inst)
{
	...
	if (inst->power.buffer_counter < DCVS_WINDOW ||
		...
		msm_vidc_calc_freq_iris3(inst, clock_data);
