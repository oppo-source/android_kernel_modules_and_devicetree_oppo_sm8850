...
/* 1000x */
static u32 sun_en_readfactor[8] = {1000, 1500, 1750, 1875, 1000, 2000, 2000, 1000};
/* 1000x */
		...
		struct api_calculation_input codec_input)
{
	...
	if (codec_input.decoder_or_encoder == CODEC_DECODER) {
		compression_factor->dpb_cf_y =
			dpbopb_ubwc30_cr_table_cratio_iris35[cr_index_entry][cr_index_y];
		compression_factor->dpb_cf_cbcr =
			dpbopb_ubwc30_cr_table_cratio_iris35[cr_index_entry][cr_index_c];
		...
		if (frame_width < 3840) {
			compression_factor->ipb_cr =
				ipblossless_ubwc30_cr_table_cratio_iris35[cr_index_entry][cr_index_uni];
			compression_factor->ipb_cr_y =
				ipblossless_ubwc30_cr_table_cratio_iris35[cr_index_entry][cr_index_y];
		} else {
			compression_factor->ipb_cr =
				ipblossy_ubwc30_cr_table_cratio_iris35[cr_index_entry][cr_index_uni];
			compression_factor->ipb_cr_y =
				ipblossy_ubwc30_cr_table_cratio_iris35[cr_index_entry][cr_index_y];
		}

		compression_factor->dpb_cf_y =
			rpb_ubwc30_cr_table_cratio_iris35[cr_index_entry][cr_index_y];
...
static int calculate_bandwidth_decoder_iris35(
		struct api_calculation_input codec_input,
		struct api_calculation_bw_output *codec_output)
{
	...
	lcu_per_frame =
		calculate_number_lcus_iris35(frame_width, frame_height, frame_lcu_size);
	...
	frame420_y_bw_linear_8bpp =
		((calculate_number_ubwctiles_iris35(frame_width, frame_height, 32, 8) *
	...
	if (codec_input.codec == CODEC_AV1) {
		...
		av1_tile_numbers = av1_num_tiles_iris35[av1tile_index_entry][av1tile_complexity];
...
static int calculate_bandwidth_encoder_iris35(
		struct api_calculation_input codec_input,
		struct api_calculation_bw_output *codec_output)
{
	/* common control parameters */
	...
	large_bw_calculation_fp = (large_bw_calculation_fp *
		sun_en_readfactor[codec_input.hierachical_layer]);
	...
	large_bw_calculation_fp = ((codec_input.bitdepth == CODEC_BITDEPTH_8) ?
		frame420_y_bw_linear_8bpp : frame420_y_bw_no_ubwc_tile_10bpp) *
		reconstructed_write_bw_factor_rd *
		sun_en_writefactor[codec_input.hierachical_layer] /
		sun_en_frame_num_parallel;
			...
			if (codec_input.linear_ipb == 0) {
				if (codec_input.complexity_setting == 0) /* pwc */
					codec_output->ipb_rd_total_noc =
						...
						en_original_compression_factor_rgba_pwd_iris35 - 1) /
						en_original_compression_factor_rgba_pwd_iris35;
				else
					codec_output->ipb_rd_total_noc =
					(large_bw_calculation_fp * 100 +
					en_original_compression_factor_rgba_avg_iris35 - 1) /
					en_original_compression_factor_rgba_avg_iris35;
			}
		}
	} else {
		...
		struct api_calculation_bw_output *codec_output)
{
	...
	if (codec_input.decoder_or_encoder == CODEC_DECODER) {
		rc = calculate_bandwidth_decoder_iris35(codec_input, codec_output);
	} else if (codec_input.decoder_or_encoder == CODEC_ENCODER) {
		rc = calculate_bandwidth_encoder_iris35(codec_input, codec_output);
	} else {
