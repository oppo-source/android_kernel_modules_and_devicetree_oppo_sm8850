...
/* 100x */
static u32 dpbopb_ubwc30_cr_table_cratio_iris36[7][12] = {
...
/* 100x */
static u32 rpb_ubwc30_cr_table_cratio_iris36[7][12] = {
...
/* 100x */
static u32 ipblossy_ubwc30_cr_table_cratio_iris36[7][12] = {
...
/* 100x */
static u32 ipblossless_ubwc30_cr_table_cratio_iris36[7][12] = {
...
/* 100x */
static u32 en_original_compression_factor_rgba_pwd_iris36 = 243;
/* 100x */
static u32 en_original_compression_factor_rgba_avg_iris36 = 454;

static u32 av1_num_tiles_iris36[7][3] = {
	{2, 1, 1},
	{4, 2, 2},
	{4, 2, 2},
	{4, 2, 2},
...
/* 1000x */
static u32 nordau_en_readfactor[8] = {1000, 1500, 1750, 1875, 1000, 2000, 2000, 1000};
/* 1000x */
static u32 nordau_en_writefactor[8] = {1000, 500, 500, 500, 1000, 0, 1000, 1000};
static u32 nordau_en_frame_num_parallel = 1;

u32 calculate_number_lcus_iris36(u32 width, u32 height, u32 lcu_size)
{
...
u32 calculate_number_ubwctiles_iris36(
		...
		struct api_calculation_input codec_input)
{
	...
	if (codec_input.decoder_or_encoder == CODEC_DECODER) {
		compression_factor->dpb_cf_y =
			dpbopb_ubwc30_cr_table_cratio_iris36[cr_index_entry][cr_index_y];
		compression_factor->dpb_cf_cbcr =
			dpbopb_ubwc30_cr_table_cratio_iris36[cr_index_entry][cr_index_c];
		compression_factor->opb_cf_ycbcr =
			dpbopb_ubwc30_cr_table_cratio_iris36[cr_index_entry][cr_index_uni];
		...
		if (frame_width < 3840) {
			compression_factor->ipb_cr =
				ipblossless_ubwc30_cr_table_cratio_iris36[cr_index_entry]
					[cr_index_uni];
			...
			compression_factor->ipb_cr =
				ipblossy_ubwc30_cr_table_cratio_iris36[cr_index_entry]
					[cr_index_uni];
			compression_factor->ipb_cr_y =
				ipblossy_ubwc30_cr_table_cratio_iris36[cr_index_entry]
					[cr_index_y];
		...
		compression_factor->dpb_cf_y =
			rpb_ubwc30_cr_table_cratio_iris36[cr_index_entry][cr_index_y];

		compression_factor->dpb_cf_cbcr =
			rpb_ubwc30_cr_table_cratio_iris36[cr_index_entry][cr_index_c];
...
static int calculate_bandwidth_decoder_iris36(
		struct api_calculation_input codec_input,
		struct api_calculation_bw_output *codec_output)
{
	...
	lcu_per_frame =
		calculate_number_lcus_iris36(frame_width, frame_height, frame_lcu_size);
	...
	frame420_y_bw_linear_8bpp =
		((calculate_number_ubwctiles_iris36(frame_width, frame_height, 32, 8) *
		256 * codec_input.frame_rate + 999) / 1000 + 999) / 1000;

	frame420_y_bw_no_ubwc_tile_10bpp =
		((calculate_number_ubwctiles_iris36(frame_width, frame_height, 48, 4) *
		256 * codec_input.frame_rate + 999) / 1000 + 999) / 1000;
	...
	if (codec_input.codec == CODEC_AV1) {
		...
		av1_tile_numbers = av1_num_tiles_iris36[av1tile_index_entry][av1tile_complexity];
...
static int calculate_bandwidth_encoder_iris36(
		struct api_calculation_input codec_input,
		struct api_calculation_bw_output *codec_output)
{
	/* common control parameters */
	...
	if (codec_input.ipb_yuvrgb == 0) {
		frame420_y_bw_linear_8bpp =
			((calculate_number_ubwctiles_iris36(frame_width, frame_height,
	...
	large_bw_calculation_fp = (large_bw_calculation_fp *
		nordau_en_readfactor[codec_input.hierachical_layer]);
	...
	large_bw_calculation_fp = ((codec_input.bitdepth == CODEC_BITDEPTH_8) ?
		frame420_y_bw_linear_8bpp : frame420_y_bw_no_ubwc_tile_10bpp) *
		reconstructed_write_bw_factor_rd *
		nordau_en_writefactor[codec_input.hierachical_layer] /
		nordau_en_frame_num_parallel;
		...
		if (codec_input.ipb_yuvrgb == 0) { /* yuv */
			...
			if (codec_input.linear_ipb == 0) {
				if (codec_input.complexity_setting == 0) /* pwc */
					codec_output->ipb_rd_total_noc =
						(large_bw_calculation_fp * 100 +
						 en_original_compression_factor_rgba_pwd_iris36
						 - 1) /
						en_original_compression_factor_rgba_pwd_iris36;
				else
					codec_output->ipb_rd_total_noc =
					(large_bw_calculation_fp * 100 +
					en_original_compression_factor_rgba_avg_iris36 - 1) /
					en_original_compression_factor_rgba_avg_iris36;
			}
		}
	} else {
		...
		struct api_calculation_bw_output *codec_output)
{
	...
	if (codec_input.decoder_or_encoder == CODEC_DECODER) {
		rc = calculate_bandwidth_decoder_iris36(codec_input, codec_output);
	} else if (codec_input.decoder_or_encoder == CODEC_ENCODER) {
		rc = calculate_bandwidth_encoder_iris36(codec_input, codec_output);
	} else {
