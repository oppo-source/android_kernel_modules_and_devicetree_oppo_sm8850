...
static u32 codec_mbspersession_iris36;
 ...
 */
static u32 frequency_table_iris36[2][5] = {
...
#define DECODER_VPP_FW_OVERHEAD_IRIS36_AV1D                                            ((80000*3)/2)
#define DECODER_VPP_FW_OVERHEAD_IRIS36_NONAV1D                                         ((60000*3)/2)

 /* Tensilica cycles */
#define DECODER_VPP_FW_OVERHEAD_IRIS36                                                  (0)

/* Tensilica cycles; this is measured in Lahaina 1stage with FW profiling */
#define DECODER_VPPVSP1STAGE_FW_OVERHEAD_IRIS36                                         (93000)

#define DECODER_VSP_FW_OVERHEAD_IRIS36 \
	(DECODER_VPPVSP1STAGE_FW_OVERHEAD_IRIS36 - DECODER_VPP_FW_OVERHEAD_IRIS36)

/* Tensilica cycles; encoder has ARP register */
#define ENCODER_VPP_FW_OVERHEAD_IRIS36                                                  (69000*3/2)

#define ENCODER_VPPVSP1STAGE_FW_OVERHEAD_IRIS36 \
	(ENCODER_VPP_FW_OVERHEAD_IRIS36 + DECODER_VSP_FW_OVERHEAD_IRIS36)

#define DECODER_SW_OVERHEAD_IRIS36                                                      (489583)
#define ENCODER_SW_OVERHEAD_IRIS36                                                      (489583)

/* Video IP Core Technology: pipefloor and pipe penlaty */
// static u32 encoder_vpp_target_clk_per_mb_iris36[2] = {320, 675};
static u32 decoder_vpp_target_clk_per_mb_iris36 = 200;

/*
 * These pipe penalty numbers only applies to 4 pipe
 * For 2pipe and 1pipe, these numbers need recalibrate
 */
static u32 pipe_penalty_iris36[3][3] = {
	/* NON AV1 */
	{1059, 1059, 1059},
 ...
 */
static u32 bitrate_table_iris36_2stage_fp[5][10] = {
	/* h264 cavlc */
 ...
 */
static u32 bitrate_table_iris36_1stage_fp[5][10] = { /* 1-stage assume IPPP */
	/* h264 cavlc */
	{0, 220, 220, 220, 220, 220, 220, 220, 220, 220},
	/* h264 cabac */
...
static u32 calculate_number_mbs_iris36(u32 width, u32 height, u32 lcu_size)
{
		...
		struct api_calculation_freq_output *codec_output)
{
	...
	/*
	 * Ignore fw_sw_vsp_offset, as this is baked into the reference bitrate
	 * tables. As a consequence remove x1000 multiplier as well.
	 */
	...
	u8 bitrate_entry = get_bitrate_entry(pixle_count);

	input_bitrate_fp = ((u32)(codec_input.bitrate_mbps * 100 + 99)) / 100;

	if (codec_input.vpu_ver == VPU_VERSION_IRIS33) {
		frequency_table_value = frequency_table_iris36;
		bitrate_table_2stage_value = bitrate_table_iris36_2stage_fp;
	}
	...
	if ((pixle_count >= fp_pixel_count_bar0) &&
		...
		if ((codec_input.codec == CODEC_H264) ||
			...
			if (codec_input.vsp_vpp_mode == CODEC_VSPVPP_MODE_2S) {
				...
				vsp_hw_min_frequency = vsp_hw_min_frequency +
					(bitrate_table_iris36_1stage_fp[codec][0] *
					fw_sw_vsp_offset - 1);
				vsp_hw_min_frequency = DIV_ROUND_UP(vsp_hw_min_frequency,
					(bitrate_table_iris36_1stage_fp[codec][0]) *
						fw_sw_vsp_offset);
		...
		if (codec_input.vsp_vpp_mode == CODEC_VSPVPP_MODE_2S) {
			...
			vsp_hw_min_frequency = vsp_hw_min_frequency +
				(bitrate_table_iris36_1stage_fp[codec][bitrate_entry] *
				fw_sw_vsp_offset - 1);
			vsp_hw_min_frequency = DIV_ROUND_UP(vsp_hw_min_frequency,
				(bitrate_table_iris36_1stage_fp[codec][bitrate_entry]) *
					fw_sw_vsp_offset);
...
static u32 calculate_pipe_penalty(struct api_calculation_input codec_input)
{
	...
	if (codec_input.vpu_ver == VPU_VERSION_IRIS33)
		pipe_penalty_value = pipe_penalty_iris36;
		...
		struct api_calculation_freq_output *codec_output)
{
	...
	u32 decoder_vpp_fw_overhead = DECODER_VPP_FW_OVERHEAD_IRIS36;

	codec_mbspersession_iris36 =
		calculate_number_mbs_iris36(codec_input.frame_width,
	...
	if (codec_input.decoder_or_encoder == CODEC_DECODER) { /* decoder */
		vpp_hw_min_frequency = ((decoder_vpp_target_clk_per_mb_iris36) *
			(codec_mbspersession_iris36) + codec_input.pipe_num - 1) /
		...
		if (codec_input.codec == CODEC_AV1)
			decoder_vpp_fw_overhead = DECODER_VPP_FW_OVERHEAD_IRIS36_AV1D;
		else
			decoder_vpp_fw_overhead = DECODER_VPP_FW_OVERHEAD_IRIS36_NONAV1D;
		...
		if (codec_input.vsp_vpp_mode == CODEC_VSPVPP_MODE_2S) {
			...
			decoder_vpp_fw_overhead =
				...
				(codec_mbspersession_iris36 *
				decoder_vpp_target_clk_per_mb_iris36 / codec_input.pipe_num));
			...
			fmin_measured_fwoverhead = fmin +
				(((DECODER_VPPVSP1STAGE_FW_OVERHEAD_IRIS36 *
		...
		tensilica_min_frequency = (DECODER_SW_OVERHEAD_IRIS36 * 10 + 14) / 15;
		...
		if (codec_input.vsp_vpp_mode == CODEC_VSPVPP_MODE_2S) {
			...
			encoder_vpp_fw_overhead =
				DIV_ROUND_UP((ENCODER_VPP_FW_OVERHEAD_IRIS36 * 10 *
				codec_input.frame_rate), 15);

			encoder_vpp_fw_overhead =
				DIV_ROUND_UP((encoder_vpp_fw_overhead * 1000),
				(codec_mbspersession_iris36 * encoder_vpp_target_clk_per_mb /
				codec_input.pipe_num));
		...
		tensilica_min_frequency = (ENCODER_SW_OVERHEAD_IRIS36 * 10 + 14) / 15;
		tensilica_min_frequency = (tensilica_min_frequency + 999) / 1000;
