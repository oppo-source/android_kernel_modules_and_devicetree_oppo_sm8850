	...
	struct cam_isp_context *ctx_isp, void *evt_data)
{
	...
	if (ctx_isp->frame_id == 1) {
		...
		if (!atomic_read(&ctx_isp->last_applied_default) || atomic_read(&ctx_isp->apply_in_progress))
#else
...
#ifdef OPLUS_FEATURE_CAMERA_COMMON
		/* Other functions should handle this */
	...
	struct cam_context *ctx, struct cam_config_dev_cmd *cmd)
{
	...
	if ((packet_opcode == CAM_ISP_PACKET_UPDATE_DEV)
		...
		CAM_INFO(CAM_ISP, "post flushed [%d]", ctx_isp->num_inits_post_flush);
...
static int __cam_isp_ctx_issue_resume_util(struct cam_context *ctx)
{
	 ...
	 */
	 if ((!rc) && (ctx->ctx_crm_intf && ctx->ctx_crm_intf->notify_msg)) {
		struct cam_req_mgr_notify_msg msg = {0};
