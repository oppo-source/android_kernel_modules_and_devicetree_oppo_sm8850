...
static int cam_vfe_bus_ver3_handle_mc_comp_done_top_half(uint32_t evt_id,
	...
	void      *priv,  void  *data)
{
	...
	debug_data = (struct cam_vfe_core_debug_data *)data;
	ver3_hw_info = bus_priv->bus_hw_info;
	...
	if (debug_data->overflow_status) {
		...
		while (tmp) {
			if (tmp & 0x1) {
				c_reg = &ver3_hw_info->bus_client_reg[i];
				CAM_ERR(CAM_ISP, "VFE[%u] Bus Overflow %s",
					soc_info->index, c_reg->name);
			}
			tmp = tmp >> 1;
			i++;
		}
...
static const char *cam_vfe_bus_ver3_wm_mode_to_string(enum cam_vfe_bus_wr_wm_mode wm_mode)
{
	switch (wm_mode) {
	case CAM_VFE_WM_LINE_BASED_MODE:
		return "LINE_BASED";
	case CAM_VFE_WM_FRAME_BASED_MODE:
		return "FRAME_BASED";
	case CAM_VFE_WM_INDEX_BASED_MODE:
		return "INDEX_BASED";
	default:
		return "INVALID_WM_MODE";
	}
}
...
static bool cam_vfe_bus_ver3_can_be_secure(uint32_t out_type)
{
	...
	case CAM_VFE_BUS_VER3_VFE_OUT_DS2:
	...
	uint32_t res_type, uint32_t  *index)
{
	...
	case CAM_ISP_IFE_OUT_RES_RDI_4:
		...
		break;
	case CAM_ISP_IFE_OUT_RES_STATS_TMC_BHIST:
		vfe_out_type = CAM_VFE_BUS_VER3_VFE_OUT_STATS_TMC_BHIST;
		break;
	case CAM_ISP_IFE_OUT_RES_STATS_AF_BHIST:
...
{
	rsrc_data->cfg.width = CAM_VFE_RDI_BUS_DEFAULT_WIDTH;
	rsrc_data->cfg.height = 0;
	rsrc_data->cfg.stride = CAM_VFE_RDI_BUS_DEFAULT_STRIDE;
	rsrc_data->cfg.en_cfg =
		(rsrc_data->common_data->common_reg->wm_mode_val[CAM_VFE_WM_FRAME_BASED_MODE] <<
			rsrc_data->common_data->common_reg->wm_mode_shift) |
			(1 << rsrc_data->common_data->common_reg->wm_en_shift);
}
	...
	struct cam_vfe_bus_ver3_wm_resource_data  *rsrc_data)
{
	...
	rsrc_data->cfg.pack_fmt = PACKER_FMT_VER3_PLAIN_128;
	...
	case CAM_FORMAT_MIPI_RAW_10:
		if (rsrc_data->use_wm_pack) {
			...
			rsrc_data->cfg.width = ALIGNUP((rsrc_data->cfg.width * 5) / 4, 16) / 16;

		break;
	case CAM_FORMAT_MIPI_RAW_6:
		if (rsrc_data->wm_mode == CAM_VFE_WM_LINE_BASED_MODE)
			rsrc_data->cfg.width = ALIGNUP((rsrc_data->cfg.width * 3) / 4, 16) / 16;
	...
	case CAM_FORMAT_MIPI_RAW_12:
		if (rsrc_data->use_wm_pack) {
			...
			rsrc_data->cfg.width = ALIGNUP((rsrc_data->cfg.width * 3) / 2, 16) / 16;
	...
	case CAM_FORMAT_MIPI_RAW_14:
		if (rsrc_data->use_wm_pack) {
			...
			rsrc_data->cfg.width = ALIGNUP((rsrc_data->cfg.width * 7) / 4, 16) / 16;
	...
	case CAM_FORMAT_PLAIN128:
		if (rsrc_data->wm_mode == CAM_VFE_WM_LINE_BASED_MODE)
			rsrc_data->cfg.width = ALIGNUP((rsrc_data->cfg.width * 16), 16) / 16;

		break;
	case CAM_FORMAT_PLAIN32_20:
		if (rsrc_data->wm_mode == CAM_VFE_WM_LINE_BASED_MODE)
			rsrc_data->cfg.width = ALIGNUP((rsrc_data->cfg.width * 4), 16) / 16;
	...
	case CAM_FORMAT_YUV422_10:
		if (rsrc_data->wm_mode == CAM_VFE_WM_LINE_BASED_MODE) {
			rsrc_data->cfg.en_cfg =
				(common_reg->wm_mode_val[CAM_VFE_WM_LINE_BASED_MODE] <<
				common_reg->wm_mode_shift) | (1 << common_reg->wm_en_shift);
			rsrc_data->cfg.width = ALIGNUP((rsrc_data->cfg.width * 5) / 4, 16) / 16;
		}
	...
	default:
		CAM_ERR(CAM_ISP, "VFE:%u Unsupported RDI format %d",
			rsrc_data->common_data->core_index, rsrc_data->cfg.format);
		return -EINVAL;
	}
	...
	enum cam_vfe_bus_plane_type                plane)
{
	...
	rsrc_data->cfg.en_cfg = (common_reg->wm_mode_val[CAM_VFE_WM_LINE_BASED_MODE] <<
			common_reg->wm_mode_shift) | (1 << common_reg->wm_en_shift);
	CAM_DBG(CAM_ISP, "pack_fmt: %d", rsrc_data->cfg.pack_fmt);

	return 0;
}

static int cam_vfe_bus_ver3_config_port(
	...
	enum cam_vfe_bus_plane_type plane)
{
	...
	case CAM_VFE_BUS_VER3_VFE_OUT_DS4_DISP:
	case CAM_VFE_BUS_VER3_VFE_OUT_DS16_DISP:
		rsrc_data->cfg.height = rsrc_data->cfg.height / 2;
		rsrc_data->cfg.width  = rsrc_data->cfg.width / 2;
		rsrc_data->cfg.en_cfg = (common_reg->wm_mode_val[CAM_VFE_WM_LINE_BASED_MODE] <<
			common_reg->wm_mode_shift) | (1 << common_reg->wm_en_shift);
		break;
	case CAM_VFE_BUS_VER3_VFE_OUT_STATS_HDR_BE:
	...
	case CAM_VFE_BUS_VER3_VFE_OUT_PREPROCESS_RAW:
		...
		rsrc_data->cfg.height = 0;
		rsrc_data->cfg.stride = 1;
		rsrc_data->cfg.en_cfg = (common_reg->wm_mode_val[CAM_VFE_WM_FRAME_BASED_MODE] <<
			common_reg->wm_mode_shift) | (1 << common_reg->wm_en_shift);
		break;
	case CAM_VFE_BUS_VER3_VFE_OUT_STATS_CAF:
		rsrc_data->cfg.en_cfg = (common_reg->wm_mode_val[CAM_VFE_WM_FRAME_BASED_MODE] <<
	...
	case CAM_VFE_BUS_VER3_VFE_OUT_SPARSE_PD:
		...
		rsrc_data->cfg.en_cfg = (common_reg->wm_mode_val[CAM_VFE_WM_LINE_BASED_MODE] <<
			common_reg->wm_mode_shift) | (1 << common_reg->wm_en_shift);
		if (rsrc_data->cfg.format != CAM_FORMAT_PLAIN8)
			/* LSB aligned */
			rsrc_data->cfg.pack_fmt |= (1 <<
	...
	case CAM_VFE_BUS_VER3_VFE_OUT_PREPROCESS_2PD:
		...
		break;
	case CAM_VFE_BUS_VER3_VFE_OUT_LTM_STATS:
		rsrc_data->cfg.stride = ALIGNUP(rsrc_data->cfg.width * 4, 16);
		rsrc_data->cfg.en_cfg = (common_reg->wm_mode_val[CAM_VFE_WM_LINE_BASED_MODE] <<
			common_reg->wm_mode_shift) | (1 << common_reg->wm_en_shift);
		break;
	case CAM_VFE_BUS_VER3_VFE_OUT_IR:
	...
	enum cam_vfe_bus_ver3_comp_grp_type   *comp_grp_id)
{
	...
	if (ver3_bus_priv->bus_hw_info->valid_wm_mask &&
		(!(BIT_ULL(rsrc_data->index) & ver3_bus_priv->bus_hw_info->valid_wm_mask))) {
	...
	if ((rsrc_data->cfg.en_cfg >> rsrc_data->common_data->common_reg->wm_mode_shift) !=
		rsrc_data->common_data->common_reg->wm_mode_val[rsrc_data->wm_mode]) {
		CAM_ERR(CAM_ISP,
			"VFE:%u WM:%d %s en_cfg: 0x%x does not have the required wm mode: %s value",
			rsrc_data->common_data->core_index, rsrc_data->index, wm_res->res_name,
			rsrc_data->cfg.en_cfg,
		...
		for (i = 0; i < CAM_ISP_MULTI_CTXT_MAX; i++) {
			...
			if (rc) {
				CAM_ERR(CAM_ISP,
					...
					rsrc_data->common_data->core_index, wm_res->res_name, i);
				goto end;
			}
		}
...
static inline void cam_vfe_bus_ver3_stop_wm_util(
	struct cam_vfe_bus_ver3_wm_resource_data   *rsrc_data)
{
	/* Disable WM */
...
static int cam_vfe_bus_ver3_stop_wm(struct cam_isp_resource_node *wm_res)
{
	...
	CAM_DBG(CAM_ISP, "Stop VFE:%u WM:%d %s hw_ctxt_mask:0x%x",
		rsrc_data->common_data->core_index, rsrc_data->index,
		wm_res->res_name, rsrc_data->out_rsrc_data->dst_hw_ctxt_id_mask);
	...
	if (rsrc_data->out_rsrc_data->mc_based || rsrc_data->out_rsrc_data->cntxt_cfg_except) {
		for (i = 0; i < CAM_ISP_MULTI_CTXT_MAX; i++) {
			...
			rsrc_data->mc_data[i].cfg.updated = false;
		}
	}
	...
	struct cam_vfe_bus_ver3_vfe_out_data *out_rsrc_data)
{
	...
	if (ver3_hw_info->valid_wm_mask && (!(BIT_ULL(index) & ver3_hw_info->valid_wm_mask))) {
	...
	struct cam_vfe_bus_ver3_vfe_out_data *vfe_out_data, uint32_t *bus_irq_reg_mask)
{
	...
	struct cam_vfe_bus_ver3_comp_grp_data *comp_grp_rsrc_data =
		vfe_out_data->comp_grp->res_priv;
	struct cam_vfe_bus_ver3_common_data *common_data = comp_grp_rsrc_data->common_data;

	CAM_DBG(CAM_ISP,
		"Start VFE:%u comp_grp:%d streaming state:%d comp_mask:0x%lX",
		comp_grp_rsrc_data->common_data->core_index,
		comp_grp_rsrc_data->comp_grp_type, comp_grp->res_state,
		comp_grp_rsrc_data->composite_mask);
	...
	uint32_t            *comp_grp_id)
{
	...
	struct cam_vfe_bus_ver3_comp_grp_data *comp_grp_rsrc_data = comp_grp->res_priv;
	...
	if (comp_grp_rsrc_data->is_dual && (!comp_grp_rsrc_data->is_master)) {
		CAM_ERR(CAM_ISP, "VFE:%u Invalid comp_grp:%u is_master:%u",
			comp_grp_rsrc_data->common_data->core_index,
			comp_grp_rsrc_data->comp_grp_type,
			comp_grp_rsrc_data->is_master);
		return rc;
	}
	...
	CAM_DBG(CAM_ISP,
		...
		comp_grp_rsrc_data->common_data->core_index, comp_grp_rsrc_data->comp_grp_type,
		CAM_BOOL_TO_YESNO(evt_payload->is_hw_ctxt_comp_done),
		comp_grp_rsrc_data->comp_done_mask, comp_grp_rsrc_data->mc_comp_done_mask,
		status_0, rc);

	*comp_grp_id = comp_grp_rsrc_data->comp_grp_type;
	...
	struct cam_isp_resource_node    *comp_grp)
{
	...
	rsrc_data->mc_comp_done_mask = ver3_hw_info->mc_comp_done_mask[index];
	...
	uint32_t args_size)
{
	 ...
	 */
	if (!(ver3_bus_priv->common_data.supported_irq & CAM_VFE_HW_IRQ_CAP_BUF_DONE)) {
		rsrc_data->common_data->buf_done_controller = acq_args->buf_done_controller;
	...
	if (rsrc_data->out_type == CAM_VFE_BUS_VER3_VFE_OUT_DS2) {
	...
	struct cam_isp_resource_node          *vfe_out)
{
	...
	if (comp_grp_rsrc_data->mc_comp_done_mask) {
		...
		rsrc_data->mc_comp_irq_handle = cam_irq_controller_subscribe_irq(
		...
		if (rsrc_data->mc_comp_irq_handle < 1) {
			CAM_ERR(CAM_ISP, "Subscribe  MC comp IRQ failed for VFE out_res %d, VFE:%u",
	...
	if (rsrc_data->early_done_mask) {
		memset(bus_irq_reg_mask, 0, sizeof(bus_irq_reg_mask));
		bus_irq_reg_mask[CAM_VFE_BUS_VER3_IRQ_REG1] = rsrc_data->early_done_mask;

		CAM_DBG(CAM_ISP,
			"VFE:%u out_type:0x%X %s bus_irq_mask_0:0x%X for Early done",
			rsrc_data->common_data->core_index, rsrc_data->out_type,
			rsrc_data->wm_res[PLANE_Y]->res_name,
			bus_irq_reg_mask[CAM_VFE_BUS_VER3_IRQ_REG1]);

		rsrc_data->early_done_irq_handle = cam_irq_controller_subscribe_irq(
			rsrc_data->bus_priv->common_data.bus_irq_controller,
			CAM_IRQ_PRIORITY_1,
			bus_irq_reg_mask,
			vfe_out,
			vfe_out->top_half_handler,
		...
		if (rsrc_data->early_done_irq_handle < 1) {
			CAM_ERR(CAM_ISP, "VFE:%u Failed to subscribe Early done IRQ",
				rsrc_data->bus_priv->common_data.core_index);
			rsrc_data->early_done_irq_handle = 0;
			return -EFAULT;
		}
	}
	...
	struct cam_isp_resource_node          *vfe_out)
{
	...
	if (vfe_out->irq_handle) {
		rc = cam_irq_controller_unsubscribe_irq(common_data->buf_done_controller,
			vfe_out->irq_handle);
		vfe_out->irq_handle = 0;
	}

	if (rsrc_data->mc_comp_irq_handle) {
		rc = cam_irq_controller_unsubscribe_irq(common_data->mc_comp_buf_done_controller,
			rsrc_data->mc_comp_irq_handle);
		rsrc_data->mc_comp_irq_handle = 0;
	}

	if (rsrc_data->early_done_irq_handle) {
		rc = cam_irq_controller_unsubscribe_irq(common_data->bus_irq_controller,
...
static int cam_vfe_bus_ver3_out_done_top_half_util(uint32_t evt_id,
	struct cam_irq_th_payload *th_payload,
	struct cam_vfe_bus_irq_evt_payload         *evt_payload,
	struct cam_vfe_bus_ver3_vfe_out_data       *out_rsrc_data)
{
	...
	struct cam_irq_th_payload *th_payload)
{
	...
	out_rsrc_data = vfe_out->res_priv;

	rc  = cam_vfe_bus_ver3_get_evt_payload(out_rsrc_data->common_data, &evt_payload);
	if (rc) {
		for (i = 0; i < th_payload->num_registers; i++)
			CAM_INFO_RATE_LIMIT(CAM_ISP, "VFE:%u Bus IRQ status_%d: 0x%X",
				out_rsrc_data->common_data->core_index, i,
				th_payload->evt_status_arr[i]);
		return rc;
	}

	evt_payload->is_hw_ctxt_comp_done = false;
	rc = cam_vfe_bus_ver3_out_done_top_half_util(evt_id, th_payload, evt_payload,
		out_rsrc_data);
	if (rc) {
		CAM_ERR(CAM_ISP, "Failed to process out done top half rc:%d", rc);
		return rc;
	}


	CAM_DBG(CAM_ISP, "VFE:%u Exit", out_rsrc_data->common_data->core_index);
	return rc;
}
	...
	struct cam_irq_th_payload *th_payload)
{
	...
	CAM_DBG(CAM_ISP, "VFE:%u MC comp done res: %s", out_rsrc_data->common_data->core_index,
		out_rsrc_data->wm_res[PLANE_Y]->res_name);
	rc  = cam_vfe_bus_ver3_get_evt_payload(out_rsrc_data->common_data, &evt_payload);
	if (rc) {
		for (i = 0; i < th_payload->num_registers; i++)
			CAM_INFO_RATE_LIMIT(CAM_ISP, "VFE:%u Bus IRQ status_%d: 0x%X",
	...
	evt_payload->is_hw_ctxt_comp_done = true;
	rc = cam_vfe_bus_ver3_out_done_top_half_util(evt_id, th_payload, evt_payload,
		out_rsrc_data);
	if (rc) {
		CAM_ERR(CAM_ISP, "Failed to process out done top half rc:%d", rc);
...
static int cam_vfe_bus_ver3_handle_vfe_out_done_bottom_half(void                *handler_priv,
	void                *evt_payload_priv)
{
	int                                    rc = -EINVAL;
	...
	rc = cam_vfe_bus_ver3_handle_comp_done_bottom_half(rsrc_data, evt_payload, &comp_grp_id);
	CAM_DBG(CAM_ISP,
		"VFE:%u out_type:0x%x mc_based:%s cntxt_cfg_except:%s is_early_done:%s comp_grp_id:%d rc:%d",
		rsrc_data->common_data->core_index, rsrc_data->out_type,
		CAM_BOOL_TO_YESNO(rsrc_data->mc_based),
		CAM_BOOL_TO_YESNO(rsrc_data->cntxt_cfg_except),
		CAM_BOOL_TO_YESNO(evt_payload->is_early_done), comp_grp_id, rc);
	...
	case CAM_VFE_IRQ_STATUS_SUCCESS:
		...
		bufdone_evt_info.is_hw_ctxt_comp = evt_payload->is_hw_ctxt_comp_done;
		bufdone_evt_info.is_early_done = evt_payload->is_early_done;
		evt_info.event_data = (void *)&bufdone_evt_info;

		if (rsrc_data->common_data->event_cb)
			rsrc_data->common_data->event_cb(ctx, evt_id, (void *)&evt_info);
...
static void cam_vfe_bus_ver3_populate_vfe_out(uint32_t index,
	struct cam_vfe_bus_ver3_priv      *bus_priv)
{
	struct cam_isp_resource_node *vfe_out = NULL;

	vfe_out = &bus_priv->vfe_out[index];
	...
	vfe_out->hw_intf = bus_priv->common_data.hw_intf;
	vfe_out->irq_handle = 0;
}

static int cam_vfe_bus_ver3_init_vfe_out_res_dyn(struct cam_vfe_bus_ver3_priv *bus_priv,
	struct cam_vfe_bus_ver3_hw_info *hw_info)
{
	uint64_t i = 0, j = 0;
	uint64_t valid_wm_mask = hw_info->valid_wm_mask;
	struct cam_vfe_bus_ver3_reg_offset_bus_client *c_reg;
	struct cam_isp_resource_node         *vfe_out = NULL;
	struct cam_vfe_bus_ver3_vfe_out_data *out_rsrc_data = NULL;
	struct cam_isp_resource_node    *wm_res = NULL;
	...
	while (valid_wm_mask)  {
		...
		c_reg = &hw_info->bus_client_reg[i];

		if (bus_priv->vfe_out_map_outtype[c_reg->out_type] ==
				CAM_VFE_BUS_VER3_VFE_OUT_MAX) {

			bus_priv->vfe_out_map_outtype[c_reg->out_type] = i;

			vfe_out = &bus_priv->vfe_out[i];
			out_rsrc_data = CAM_MEM_ZALLOC(sizeof(struct cam_vfe_bus_ver3_vfe_out_data),
					GFP_KERNEL);
			if (!out_rsrc_data) {
				rc = -ENOMEM;
				goto free_mem;
			...
			vfe_out->res_priv = out_rsrc_data;
			out_rsrc_data->out_type = c_reg->out_type;
			out_rsrc_data->common_data  = &bus_priv->common_data;
			out_rsrc_data->mc_based = c_reg->mc_based;
			out_rsrc_data->cntxt_cfg_except = c_reg->cntxt_cfg_except;
			out_rsrc_data->num_mid = c_reg->num_mid;
			out_rsrc_data->mid = c_reg->mid;
		...
		wm_res = CAM_MEM_ZALLOC(sizeof(struct cam_isp_resource_node), GFP_KERNEL);
		if (!wm_res) {
			CAM_ERR(CAM_ISP, "VFE:%u init wm[%d] mem allocation failed",
				bus_priv->common_data.core_index, i);
			rc = -ENOMEM;
			goto free_mem;
		}
		out_rsrc_data->wm_res[out_rsrc_data->num_wm] = wm_res;

		out_rsrc_data->num_wm++;
		rc = cam_vfe_bus_ver3_init_wm_resource(
			i, bus_priv, hw_info, wm_res,
			&out_rsrc_data->comp_grp, c_reg->name,
			c_reg->line_based, out_rsrc_data);

		if (rc) {
			CAM_ERR(CAM_ISP, "VFE:%u init wm[%d] failed",
				bus_priv->common_data.core_index, i);
			goto free_mem;
		}
		cam_vfe_bus_ver3_populate_vfe_out(i, bus_priv);
		i++;
		valid_wm_mask >>= 1;
	}

	bus_priv->num_out =  i;
	CAM_DBG(CAM_ISP, "valid out ports %u num_out %u", hw_info->valid_wm_mask,
		bus_priv->num_out);
...
free_mem:
	for (i = 0; i < bus_priv->num_out; i++) {
		...
		if (!out_rsrc_data)
			continue;
		for (j = 0; j < out_rsrc_data->num_wm; j++) {
			CAM_MEM_FREE(out_rsrc_data->wm_res[j]);
			out_rsrc_data->wm_res[j] = NULL;
		}

		CAM_MEM_FREE(out_rsrc_data);
...
static int cam_vfe_bus_ver3_init_vfe_out_static(uint32_t  index,
	struct cam_vfe_bus_ver3_priv                  *ver3_bus_priv,
	struct cam_vfe_bus_ver3_hw_info               *ver3_hw_info)
{
	struct cam_isp_resource_node         *vfe_out = NULL;
	...
	if (!ver3_hw_info->vfe_out_hw_info) {
		CAM_ERR(CAM_ISP, "VFE out hw info not defined");
		return -EINVAL;
	}

	vfe_out_type = ver3_hw_info->vfe_out_hw_info[index].vfe_out_type;
	if (vfe_out_type < 0 ||
		vfe_out_type >= CAM_VFE_BUS_VER3_VFE_OUT_MAX) {
	...
	for (i = 0; i < rsrc_data->num_wm; i++) {
		rsrc_data->wm_res[i] = CAM_MEM_ZALLOC(sizeof(struct cam_isp_resource_node),
					GFP_KERNEL);
		if (!rsrc_data->wm_res[i]) {
	...
	cam_vfe_bus_ver3_populate_vfe_out(index, ver3_bus_priv);

	return 0;

free_mem:
	for (i = i - 1; i >= 0; i--) {
		CAM_MEM_FREE(rsrc_data->wm_res[i]);
		rsrc_data->wm_res[i] =  NULL;
	}
	return rc;
}

static int cam_vfe_bus_ver3_init_vfe_out_resource(
	struct cam_vfe_bus_ver3_priv                  *bus_priv,
	struct cam_vfe_bus_ver3_hw_info               *hw_info)
{
	int rc = 0;
	...
	if (hw_info->support_dyn_offset) {
		rc = cam_vfe_bus_ver3_init_vfe_out_res_dyn(bus_priv, hw_info);
		if (rc)
			CAM_ERR(CAM_ISP, "VFE:%u Failed to update outport info",
				bus_priv->common_data.core_index);
	} else {
		for (i = 0; i < bus_priv->num_out; i++) {
			rc = cam_vfe_bus_ver3_init_vfe_out_static(i, bus_priv,
					hw_info);
			if (rc) {
				CAM_ERR(CAM_ISP,
	...
	uint8_t *wm_name)
{
	...
	restore_ctxt_sel_val = cam_vfe_bus_ver3_get_rd_ctxt_sel(bus_priv, &rd_ctxt);
	...
	void *cmd_args)
{
	...
	for (i = 0; i < bus_priv->num_out; i++) {
		...
		for (j = 0; j < rsrc_data->num_wm; j++) {
			...
			md_wm->width  = wm->cfg.width;
			md_wm->index  = wm->index;
			md_wm->height = wm->cfg.height;
			md_wm->stride = wm->cfg.stride;
			md_wm->en_cfg = wm->cfg.en_cfg;
			md_wm->h_init = wm->cfg.h_init;
			md_wm->format = wm->cfg.format;
...
static int cam_vfe_bus_ver3_update_wm(void *priv, void *cmd_args, uint32_t arg_size)
{
	...
	for (i = 0, j = 0; i < vfe_out_data->num_wm; i++) {
		...
		if ((vfe_out_data->mc_based  || wm_data->out_rsrc_data->cntxt_cfg_except)
			&& (wm_data->mc_data[hw_cntxt_id].cfg.updated))
	...
	if (wm_data->hw_regs->ubwc_regs && (bus_priv->bus_hw_info->ubwc_clients_mask &
		...
		if (((wm_data->out_rsrc_data->mc_based ||
			...
			wm_data->ubwc_cfg_data.ubwc_mode_cfg)
			ubwc_enabled = true;
	}
#endif
		...
		if ((cfg->en_cfg) & (1 << wm_data->common_data->common_reg->frmheader_en_shift))
			cfg->en_cfg &= ~(1 << wm_data->common_data->common_reg->frmheader_en_shift);
		...
		if (update_buf->wm_update->frame_header &&
			...
			CAM_DBG(CAM_ISP,
				...
				bus_priv->common_data.core_index, wm_data->index, cfg->en_cfg,
				update_buf->wm_update->frame_header,
				update_buf->wm_update->local_id);
		}
	...
	bool init_cfg_done)
{
	if ((!wm_data->is_dual) && ((wm_data->cfg.h_init !=
		...
		!init_cfg_done)) {
		wm_data->cfg.h_init = ubwc_generic_plane_cfg->h_init;
	...
	uint32_t arg_size)
{
	...
	for (i = 0; i < vfe_out_data->num_wm; i++) {
		...
		if ((wm_data->wm_mode == CAM_VFE_WM_FRAME_BASED_MODE) ||
			(wm_data->wm_mode == CAM_VFE_WM_INDEX_BASED_MODE))
			wm_data->cfg.offset = stripe_config->offset;
		else
			wm_data->cfg.h_init = stripe_config->offset;

		CAM_DBG(CAM_ISP,
			"VFE:%u out_type:0x%X WM:%d width:%d offset:0x%X h_init:%d",
			vfe_out_data->common_data->core_index, stripe_args->res->res_id,
			wm_data->index, wm_data->cfg.width, wm_data->cfg.offset,
			wm_data->cfg.h_init);
	}
	...
	void                                       *cmd_args)
{
	...
	for (i = 0; i < vfe_out_data->num_wm; i++) {
		...
		wm_data = vfe_out_data->wm_res[i]->res_priv;
		common_reg = wm_data->common_data->common_reg;
		if (vfe_out_data->mc_based) {
			while (context_id_mask) {
				hw_ctxt_id = ffs(wm_config->context_id_mask) - 1;
				if (hw_ctxt_id < 0 || hw_ctxt_id >= CAM_ISP_MULTI_CTXT_MAX) {
					CAM_ERR(CAM_ISP, "VFE[%u] Invalid context id received %d",
						vfe_out_data->common_data->core_index,
						hw_ctxt_id);
					return -EINVAL;
				}

				clear_bit(hw_ctxt_id, &context_id_mask);
				cfg = &wm_data->mc_data[hw_ctxt_id].cfg;
				cfg->updated = true;
			}
		} else {
			cfg = &wm_data->cfg;
	...
	void                                        *cmd_args)
{
	...
	for (i = 0; i < vfe_out_data->num_wm; i++) {
		...
		wm_data->cfg.en_cfg = ((wm_config->wm_mode << common_reg->wm_mode_shift) |
			(wm_config->virtual_frame_en << common_reg->virtual_frm_en_shift) |
			(1 << common_reg->wm_en_shift));
		wm_data->cfg.width  = wm_config->width;

		if (i == PLANE_C)
			wm_data->cfg.height = wm_config->height / 2;
		else
			wm_data->cfg.height = wm_config->height;
		...
		if ((wm_config->packer_format != CAM_FORMAT_BASE) &&
			...
			CAM_DBG(CAM_ISP,
				...
				vfe_out_data->common_data->core_index, wm_data->index,
				wm_config->packer_format, wm_data->cfg.pack_fmt);
		}
		...
		CAM_DBG(CAM_ISP,
			...
			vfe_out_data->common_data->core_index, wm_data->index,
			vfe_out_data->wm_res[i]->res_name, CAM_BOOL_TO_YESNO(update_wm_mode),
			CAM_BOOL_TO_YESNO(wm_data->update_wm_format), wm_data->cfg.en_cfg,
			wm_data->cfg.height, wm_data->cfg.width, wm_data->cfg.stride);
	}
	...
	struct cam_vfe_bus_ver3_priv *bus_priv, uint32_t rd_ctxt, uint32_t restore_ctxt_sel_val)
{
	...
	if (!(common_reg->capabilities & CAM_VFE_COMMON_CAP_SPLIT_CTXT_RD_WR_SEL)) {
		...
		reg_val = (rd_ctxt << bus_priv->common_data.common_reg->mc_read_sel_shift) | reg_val;
	} else {
	...
	struct cam_isp_hw_get_res_for_mid *get_res)
{
	struct cam_vfe_bus_ver3_vfe_out_data          *out_rsrc_data = NULL;
	struct cam_vfe_bus_ver3_wm_resource_data      *wm_data = NULL;
	struct cam_vfe_bus_ver3_reg_offset_bus_client *c_reg = NULL;
	uint32_t                                       meta_mid = 0;
	uint32_t                                       img_mid = 0;
	uint32_t                                       num_wm = 0;
...
int cam_vfe_bus_ver3_get_out_res_mid_info(struct cam_vfe_bus_ver3_priv *bus_priv,
	struct cam_isp_hw_get_res_for_mid       *get_res)
{
	...
	if (i == bus_priv->num_out || !out_data) {
		CAM_ERR(CAM_ISP,
	...
	void *cmd_args, uint32_t arg_size)
{
	...
	if (bus_priv->bus_hw_info->support_dyn_offset) {
		rc = cam_vfe_bus_ver3_dump_wm_mid_info(bus_priv, get_res);
		get_res->dump_handled = true;
	} else {
		rc = cam_vfe_bus_ver3_get_out_res_mid_info(bus_priv, get_res);
		get_res->dump_handled = false;
	}
	...
	struct cam_vfe_bus                  **vfe_bus)
{
	...
	bus_priv->vfe_out = CAM_MEM_ZALLOC((sizeof(struct cam_isp_resource_node) *
		CAM_VFE_BUS_VER3_VFE_OUT_MAX), GFP_KERNEL);
