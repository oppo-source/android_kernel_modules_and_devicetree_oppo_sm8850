	...
	void *priv, void *cmd_args, uint32_t arg_size)
{
	...
	reg_idx = inject_params->reg_unit - (CAM_SFE_IRQ_BUS_RD_REG * REG_SHIFT);
	...
	inject_reg = &bus_rd_hw_info->common_reg.irq_reg_info.irq_reg_set[reg_idx];
	if (!inject_reg) {
	...
	cam_io_w_mb(0x10, soc_info->reg_map[SFE_CORE_BASE_IDX].mem_base +
		bus_rd_hw_info->common_reg.irq_reg_info.global_irq_cmd_offset);
