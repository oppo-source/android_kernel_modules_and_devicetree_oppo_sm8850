...
static bool msm_usbc_swap_gnd_mic(struct snd_soc_component *component, bool active)
{
...
#ifdef OPLUS_ARCH_EXTENDS
	// add oplus typec switch
	if (!pdata->wcd_usbss_handle && !pdata->oplus_switch_handle)
#else
	...
	if (pdata->oplus_switch_handle) {
		ret = typec_switch_switch_event(pdata->oplus_switch_handle, TYPEC_SWITCH_MIC_GND_SWAP);
		if (ret == 0) {
			return true;
		}
		return false;
			...
			struct msm_asoc_mach_data *pdata, int event)
{
	...
	switch (event) {
		...
		case SND_SOC_DAPM_POST_PMD:
			...
			if (level_shifter_supply) {
				ret = dmic_regulator_enable(level_shifter_supply, false);
				if (ret) {
...
static int msm_asoc_machine_probe(struct platform_device *pdev)
{
	...
	if (!pdata->oplus_switch_handle) {
		dev_dbg(&pdev->dev, "property %s not detected in node %s\n",
			"oplus-switch-handle", pdev->dev.of_node->full_name);
	}
	if ((pdata->wcd_usbss_handle) || (pdata->oplus_switch_handle))
#else
	if (pdata->wcd_usbss_handle)
